...
class MXMQTTMessage:
    def __init__(self) -> None:
        self._protocol_type: Union[MXProtocolType.Base,
                                   MXProtocolType.Super,
                                   MXProtocolType.WebClient] = None
...
class MXExecuteMessage(MXMQTTReceiveMessage):
    def __init__(self, msg: mqtt.MQTTMessage) -> None:
        ...
        elif not all([isinstance(arg.get('order', None), int) for arg in self.arguments]) or not all([isinstance(arg.get('value', None), (int, float, str, bool)) for arg in self.arguments]):
            self.payload_error = True

    def tuple_arguments(self) -> tuple:
        self.arguments = sorted(
            self.arguments, key=lambda x: int(x['order']))
...
class MXExecuteResultMessage(MXMQTTSendMessage):
    def __init__(self, function, scenario: str, request_ID: str = '', error: MXErrorCode = MXErrorCode.UNDEFINED) -> None:
        ...
        if self.request_ID:
            self.topic = self._protocol_type.value % (
                self.function.get_name(), self.function.get_thing_name(), self.function.get_middleware_name(), self.request_ID)
        else:
            self.topic = (self._protocol_type.value %
                          (self.function.get_name(), self.function.get_thing_name(), '', '')).rstrip('/')

        self.payload = dict(error=self._error.value,
