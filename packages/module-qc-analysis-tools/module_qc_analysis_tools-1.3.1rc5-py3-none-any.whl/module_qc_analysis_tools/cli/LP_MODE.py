...
def main(
    ...
    for filename in sorted(allinputs):
        ...
        for inputDF in inputDFs:
            ...
            data.add_parameter(
                "LP_VINA", round(calculated_data["VinA"]["Values"][0], 4)
            )
            data.add_parameter(
                "LP_VIND", round(calculated_data["VinD"]["Values"][0], 4)
            )
            data.add_parameter(
                "LP_VOFFS", round(calculated_data["Vofs"]["Values"][0], 4)
            )
            data.add_parameter(
                "LP_IINA", round(calculated_data["IinA"]["Values"][0], 4)
            )
            data.add_parameter(
                "LP_IIND", round(calculated_data["IinD"]["Values"][0], 4)
            )
            data.add_parameter(
