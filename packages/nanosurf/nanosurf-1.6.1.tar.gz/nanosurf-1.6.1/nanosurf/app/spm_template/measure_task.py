...
class MeasureTask(nsf.frameworks.qt_app.nsf_thread.SPMWorker):
    ...
    def do_work(self):
        ...
        if self.connect_to_controller():
            ...
            # loop and measure until all samples are measured or a user abort is detected
            self.count = 0
            while (self.count < self.par_sample_points) and not self.is_stop_request_pending():
                new_value = self.lu_adc_in.attribute.current_input_value.value

                self.result.monitor_stream.channels[0].value[self.count] = new_value
                self.result.last_index = self.count
