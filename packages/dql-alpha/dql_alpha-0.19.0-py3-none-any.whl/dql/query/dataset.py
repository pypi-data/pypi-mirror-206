...
from dql.data_storage.sqlite import compile_statement, quote_schema
...
class StartingStep(ABC):
    ...
    @abstractmethod
    def apply(self, conn: "DBAPIConnection") -> "QueryGenerator":
        ...
...
class Step(ABC):
    ...
    @abstractmethod
    def apply(self, query: "Executable", conn: "DBAPIConnection") -> "QueryGenerator":
...
class UDFSignal(Step):
    ...
    def apply(self, query, conn):
        if self.name in DATASET_CORE_COLUMN_NAMES:
            ...
            return self.overwrite_existing(query, conn)
        return self.new_column(query, conn)

    def overwrite_existing(self, query, conn):
        """Handler for udfs populating core columns (e.g. the checksum column)."""
        tbl, col = self.udf_results_table(query, conn)
    ...
    def new_column(self, query, conn):
    ...
    def udf_results_table(self, query, conn):
        """
        ...
        for row in results:
            ...
            update = tbl.insert().values(id=row.id, **signal)
...
class RowGenerator(Step):
    ...
    def __init__(self, generator: "Generator", catalog: "Catalog", engine: "Engine"):
        self.generator = generator
        self.catalog = catalog
        self.engine = engine

    def clone(self):
        return self.__class__(self.generator, self.catalog, self.engine)
    ...
    def apply(self, query, conn):
        ...
        for row in results:
            for new_entry in self.generator(self.catalog, row):
                execute_stmt(tbl.insert().values(**new_entry))
...
class SQLQuery:
    ...
    def apply_steps(self):
        ...
        with engine.connect() as connection:
            ...
            query = self.starting_step.apply(conn)
...
class DatasetQuery(SQLQuery):
    def __init__(
        ...
        engine: "Engine" = data_storage.engine
        starting_step: StartingStep
        if path:
            ...
            table_obj = sqlalchemy.Table(
                table, sqlalchemy.MetaData(), autoload_with=engine
            )
            starting_step = QueryStep(table_obj)
        else:
            raise ValueError("must provide path or name")
        super().__init__(engine=engine, starting_step=starting_step)
    ...
    def generate(self, generator: "Generator"):
        query = self.clone()
        steps = query.steps
        steps.append(RowGenerator(generator, self.catalog, self.engine))
        return query
    ...
    def save(self, name: str):
        ...
        old = quote_schema(temp_tbl)
        new = quote_schema(table_name)
        with self.engine.connect() as connection:
            conn = connection.connection.driver_connection
            conn.execute(f"ALTER TABLE {old} RENAME TO {new}")  # type: ignore[union-attr] # noqa: E501
