            ...
            if np.any(done):
                ...
                for env_id in env_ind_global:  # enumerate env
                    ...
                    for length in new_trajactory_len:  # use different mid goal to resample
                        ...
                        trajectory.obs.guess_scene_id = trajectory.obs.cur_scene_id
                        trajectory.obs.desired_goal_id = new_goals
                        trajectory.obs_next.guess_scene_id = trajectory.obs_next.cur_scene_id
                        trajectory.obs_next.desired_goal_id = new_goals
                        trajectory.obs.is_relabeled = np.repeat([1], length + 1, axis=0)
                        trajectory.obs_next.is_relabeled = np.repeat([1], length + 1, axis=0)
