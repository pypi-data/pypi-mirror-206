...
class FormatNames:
    def __init__(self, splitter: str, matcher: FormatStandardise, write_directory: Union[Path, str],
                 qc_validation: Union[Path, str]):

        # Initialise the matcher and how to split the names
        self._std = matcher
    ...
    def write(self, database_name: str):
        """Write the database and the log to disk"""
        write_json(self.database, self._write_directory, f'Cleaned_{database_name}')
        self.log.write(database_name, self._write_directory)

    def standardise(self, csv_path: Path, name_i: int, data_start_i: int, merge_ambiguity: bool, file_index: int,
                    total_files: int):
        ...
        place_dict = {place: self.rename_place(place, raw_csv.file_name) for place in unique_names}

        # rename all locations
        cleaned = [[place_dict[simplify_string(row[name_i])]] + row[data_start_i:] for row in raw_csv.row_data]
        ...
        self.database[raw_csv.file_name] = {row[0]: {h: row[i+1] for i, h in enumerate(raw_csv.headers[data_start_i:])}
                                            for row in cleaned}
        print(f"Processed {file_index} / {total_files}: {raw_csv.file_name} at {terminal_time()}")

    def rename_place(self, place, year):
        """
