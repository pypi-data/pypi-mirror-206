...
from eulith_web3.signing import Signer
...
class EulithData:
    ...
    def get_enable_safe_tx(self, owner_threshold: int, owners: List[str]) -> (bool, TxParams):
        try:
            params = {
    ...
    def submit_enable_safe_tx_hash(self, tx_hash: str) -> bool:
        try:
            params = {
                'tx_hash': tx_hash
            }
...
class v0:
    ...
    def submit_enable_module_signature(self, authorized_trading_address: str, signer: Signer) -> bool:
        ...
        tx_hash = safe.get_transaction_hash(self.ew3.to_checksum_address(sa), 0, enable_data,
                                            0, 0, 0, 0, '0x0000000000000000000000000000000000000000',
                                            '0x0000000000000000000000000000000000000000', 0)

        signature = signer.sign_msg_hash(tx_hash)
        compatible_v = signature.v + 27
        last_two_chars = hex(compatible_v)[-2:]
        signature = str(signature)[:-2] + last_two_chars

        status, result = self.ew3.eulith_data.submit_safe_signature(aa, str(signer.address),
                                                                    signature, f'0x{tx_hash.hex()}', 'enable_module')
    ...
    def enable_armor(self, owner_threshold: int, owners: List[str], override_tx_params: TxParams = None) -> bool:
        """
        ...
        """
        status, result = self.ew3.eulith_data.get_enable_safe_tx(owner_threshold, owners)
        if not status:
            raise EulithRpcException(result)
        ...
        status = self.ew3.eulith_data.submit_enable_safe_tx_hash(h.hex())
        return status
