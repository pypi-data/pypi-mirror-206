...
from amazon_sagemaker_jupyter_scheduler.cron_util import EventBridgeCronExpressionParser
from amazon_sagemaker_jupyter_scheduler.file_uploader import S3FileUploader, SM_UI_EMPTY_FILE_STRING
...
from amazon_sagemaker_jupyter_scheduler.image_metadata_resolver import (
...
CHANNEL_NAME_INPUT_ARTIFACTS = "sagemaker_headless_execution_vanilla"
TRAINING_JOB_INPUT_FOLDER = "/opt/ml/input/data"

class ModelConverter:
    def __init__(self, sagemaker_client: SageMakerAsyncBoto3Client, logger):
        self.internal_metadata_util = ImageMetadataResolver(
            InternalMetadataAdapter(), sagemaker_client
        )
        self.log = logger
    ...
    async def to_create_training_job_input(
        ...
        training_image_metadata = (
            await self.internal_metadata_util.resolve_image_metadata(
                get_region_name(),
                runtime_environment_parameters.sm_image
            )
        )
        ...
        output = {
            "AlgorithmSpecification": {
                "TrainingImage": training_image_metadata.app_image_uri,
            ...
            "Environment": {
                ...
                JobEnvironmentVariableName.SM_EXECUTION_INPUT_PATH.value: f"{TRAINING_JOB_INPUT_FOLDER}/{CHANNEL_NAME_INPUT_ARTIFACTS}",
                **runtime_environment_parameters.customer_environment_variables,
            },
            "InputDataConfig": [
                {
                    "ChannelName": CHANNEL_NAME_INPUT_ARTIFACTS,
                    "DataSource": {
                        "S3DataSource": {
                            "S3DataType": "S3Prefix",
                            "S3Uri": os.path.join(
    ...
    def to_pipeline_seach_input(self, query: ListJobDefinitionsQuery, default_filters = None) -> Dict:
        ...
        filters = self.get_common_resource_tags_filters()
    ...
    def to_jupyter_describe_job_definition_output(
        ...
        return DescribeJobDefinition(
            ...
            schedule=str(
                EventBridgeCronExpressionParser().parse(
