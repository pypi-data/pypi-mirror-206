...
def process_manifest(pdf_in_file: str, source_file_id: str, vendor_files: dict, log: Logger):
    pdf_text = extract_pdf_text(pdf_in_file)
    manifest = extract_test_data(pdf_text)
    manifest.update(extract_patient_data(pdf_text))

    manifest["reportFile"] = pdf_in_file
    manifest["sourceFileId"] = source_file_id
    manifest["resources"] = [
        {"fileName": vendor_files["pdfFile"]},
        {"fileName": vendor_files["somaticVcfFile"]},
        {"fileName": vendor_files["germlineVcfFile"]},
        {"fileName": vendor_files["somaticBamFile"]},
        {"fileName": vendor_files["germlineBamFile"]},
        {"fileName": vendor_files["xmlFile"]},
    ]
