...
class ArtemisAggregator:
    ...
    async def aggregate(self):
        conn = Connection([(self.queue_host, self.queue_port)], heartbeats=(10000, 10000))
        ...
        conn.send(f"topic.poisson_pill.{self.url}", "", headers={"type": "poisson_pill"})
        conn.disconnect()
...
class ArtemisProcessor:
    ...
    async def _call_pipeline_with_ack(self,
    ...
    async def processor(self):
        ...
        conn = Connection(
            [(self.queue_host, self.queue_port)], reconnect_attempts_max=-1, heartbeats=(10000, 10000)
        )
        ...
        try:
            ...
            while True:
                if (
                    ...
                    and (self.pills_to_die is None or listener.pills >= self.pills_to_die)
                ...
                try:
                    ...
                    while (
                        len(pending_extracts) < self.queue_size and not listener.messages.empty()
                    ):
