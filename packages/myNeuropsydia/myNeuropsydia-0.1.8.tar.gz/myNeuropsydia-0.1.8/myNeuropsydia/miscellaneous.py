...
def line(left_x=-5, left_y=0, 
         right_x=5, right_y=0, 
         line_color="black", thickness=1):
    ...
    *Dependencies*
    ...
    if left_x == right_x or left_y == right_y:
        pygame.draw.line(screen, color(line_color), 
                         (left_x, left_y), 
                         (right_x, right_y), 
                         thickness)
...
def rectangle(x=0, y=0, 
              width=10, height=10, 
              line_color="black", thickness=1, fill_color=None):
...
def right_arrow (x=0, y=0,
    ...
    left_tri = left + width_rec
    left_rec = left
    
    pygame.draw.polygon(screen, 
                        color(_color), 
                        [(left_tri,top), 
                         (left_tri+width_tri,top-height/2),
                         (left_tri,top-height), 
                         (left_tri,top-(2/3)*height),
                         (left_rec,top-(2/3)*height),
                         (left_rec,top-(1/3)*height),
                         (left_tri,top-(1/3)*height)],
                        0)


def left_arrow (x=0, y=0,
            width=6, height=3,
            _color='black'):
     ...
     left_tri = left
     left_rec = left + width_rec
     
     pygame.draw.polygon(screen, 
                         color(_color), 
                         [(left_rec,top), 
                          (left_tri,top-height/2),
                          (left_rec,top-height), 
                          (left_rec,top-(2/3)*height),
                          (left_rec+width_rec,top-(2/3)*height),
                          (left_rec+width_rec,top-(1/3)*height),
                          (left_rec,top-(1/3)*height)],
                         0)   


def countdown(style="circle", duration=3000, width=5, 
              reverse=False, background="white", 
              write_seconds=True, write_color="white", write_outline="black", 
              color_fade=False, color_start="red", color_end="green", 
              sound=False, melody=[1000, 1500]):
    ...
    for i in range(duration_frames):
        ...
        if write_seconds==True:
            write("%i" %((duration/1000+1)-(builtin_time.perf_counter()-t0)), size = 2, color=write_color,outline=True, outline_color=write_outline)
...
def start_screen(name="test", path="./Logo/", 
                 extension=".png", authors="", language="en"):
...
def end_screen(name="test", success=True, 
               path="./Logo/", extension=".png", 
               authors="", language="en"):
