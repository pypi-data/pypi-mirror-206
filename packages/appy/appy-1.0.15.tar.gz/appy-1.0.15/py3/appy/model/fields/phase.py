...
class Page:
    ...
    def __init__(self, name, phase='main', show=True, showSave=True,
        ...
        # * one of the standard Appy icons, residing in appy/ui/static, set the
        #   icon name, for example "help.svg" ;
        # * an icon being in your app's "static" folder, if it is a SVG file,
        #   use its name (ie, "myimage.svg"); if it is another format (ie, PNG),
        #   use string "<app_name>/<icon_name>";
        ...
        #
        #   For this latter case, here are some tips.
        #   1) If you want to build the URL to a standard SVG Appy icon, or to
        #      a SVG icon being in your app's "static" folder, use, ie:
        #
        #                      o.buildSvg('help')
        #
        #      ("o" is any instance of any Appy class;
        #       in the example, the file to use is help.svg, byt the ".svg"
        #       suffix does not need to be mentioned.)
        #
        #   2) If you want to build the URL to a non-SVG icon being in your
        #      app's "static" folder, use, ie:
        #
    ...
    def setIcon(self, icon):
        ...
        if callable(icon):
            ...
            self.icon, self.iconBase, self.iconRam = iconParts(icon)
...
class UiPhase:
    ...
    view = Px('''
     ...
     css='''
      .phase { margin:|phaseMargin|; display:flex; flex-wrap:wrap;
               gap:|pageGap|; width:fit-content }

      .phase > div { height:|pageHeight|; width:|pageWidth|; text-align:center;
        text-transform:|pageTextTransform|; font-weight:|pageTextWeight|;
        display:flex !important; align-items:center; justify-content:center;
        padding:|pagePadding|; background-color:|phaseBgColor|;
        position:relative; padding:0 5px; color:|phaseColor|;
        border-radius:|pageRadius|; font-size:|pageFontSize|;
      ...
      .pagePicto, .phase form { display:|pageDisplay| }
      .phaseC { margin:|phaseCMargin|; border-left:1px solid |phaseBorderColor|}
      .phaseC > div { height:|pageCHeight|; width:|pageCWidth|;
        font-size:|pageCFontSize|; padding:|pageCPadding|;
        text-transform:|pageCTextTransform|;
        border-radius:|pageCRadius| }
      ...
      .pictoT { position:|epictoPosition|; top:8px; right:8px;
                width:|epictoWidth|; padding: |epictoPadding| }
      .pictoC, .pictoB { width:|pictoCWidth|; height:|pictoCHeight|;
                         margin:|pictoCMargin| }
      .filtered { filter:invert(66%) }

      .currentPage { background-color:|phaseBgcColor| !important;
                     color:|phaseCcolor| }
      .currentPage img { filter:|phaseFilter| }
      .currentPage div { color:|phaseCcolor| }
    ...
    def showNavigation(self, c):
        '''Must siblings navigation be shown within this phase ?'''
        # There may be no siblings to navigate to
        nav = c.req.nav
        if not nav or nav == 'no': return
        # Navigation must not be shown on "edit", or on a phase not being "main"
        if c.isEdit or self.name != 'main': return
        # Navigation may already be shown in the page preamble, if present
        if 'p' in c.pageLayout.layoutString: return
        return True
      ...
      <x if="not singlePhase and not isEdit">
       <table cellpadding="0" cellspacing="0">
        ...
        <tr for="phase in phases.all.values()"
          ...
          style=":'display:%s' % ('table-row' if loop.phase.nb==0 else 'none')">
         <td>:phase.view</td>
        </tr>
       </table>
       <script>:'initTab(%s,%s)' % \
