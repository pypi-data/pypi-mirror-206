...
class TestCapAmount(TestCase):
    def setUp(self) -> None:
        ...
        cap_configuration.amount = Decimal("100")
        cap_configuration.discount = Decimal("100")
    ...
    def test_cap_config_required_fields(self):
        ...
        # No limit amount
        cap_config.amount = None
        self.assertRaises(ValidationError, cap_config.full_clean)
        # limit of 0
        cap_config.amount = Decimal(0)
        self.assertRaises(ValidationError, cap_config.full_clean)
        cap_config.amount = Decimal("1")
        cap_config.full_clean()
        # No discount
        cap_config.discount = None
        self.assertRaises(ValidationError, cap_config.full_clean)
        # Basically the same as no discount
        cap_config.discount = Decimal(0)
        self.assertRaises(ValidationError, cap_config.full_clean)
        # Cannot have more than 100% discount
        cap_config.discount = Decimal("101")
        self.assertRaises(ValidationError, cap_config.full_clean)
        # 100% ok
        cap_config.discount = Decimal("100")
        cap_config.full_clean()
    ...
    def test_reset(self):
        ...
        cap_configuration.amount = Decimal(200)
        cap_configuration.discount = Decimal(50)
        ...
        self.assertEqual(cap.amount, 100)
        self.assertEqual(cap.discount, 100)
        ...
        self.assertEqual(cap_after_reset.amount, 200)
        self.assertEqual(cap_after_reset.discount, 50)
