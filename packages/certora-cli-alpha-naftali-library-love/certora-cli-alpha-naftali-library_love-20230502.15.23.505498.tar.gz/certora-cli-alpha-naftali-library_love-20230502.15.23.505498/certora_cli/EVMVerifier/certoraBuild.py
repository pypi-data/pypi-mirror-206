...
from Shared.certoraUtils import get_certora_config_dir, get_certora_build_file, get_certora_verify_file, \
    ...
    get_certora_verify_file_cvl1, find_filename_in, PRE_AUTOFINDER_BACKUP_DIR
...
class ContractInSDC:
    ...
    all_funcs: List[Func] = field(default_factory=list)

    @property
    def original_file_name(self) -> str:
        return Path(self.original_file).name

    def as_dict(self) -> Dict[str, Any]:
        """
        :return: A dictionary representation of this SDC, including all attributes and their values
        """
        if self.compiler_collector.smart_contract_lang == CompilerLangVy():
        ...
        return {
            ...
            "srcmap": self.srcmap,
            "varmap": self.varmap,
            "constructorSrcmap": self.constructor_srcmap,
            ...
            "compilerName": self.compiler_collector.compiler_name,
            "compilerVersion": compiler_version,
            "optimizationFlags": self.compiler_collector.optimization_flags
        }
    ...
    def __hash__(self) -> int:
        return hash((self.name, self.original_file, self.address))
...
class SDC:
    ...
    """
    primary_contract: ContractInSDC
    compiler_collector: CompilerCollector
    primary_contract_address: str
    sdc_origin_file: str
    original_srclist: Dict[Any, Any]
    report_srclist: Dict[Any, Any]
    sdc_name: str
    contracts: List[ContractInSDC]  # Not transferred to dict
    library_addresses: List[str]
    state: Dict[str, str] = field(default_factory=dict)
    struct_linking_info: Dict[str, str] = field(default_factory=dict)
    legacy_struct_linking: Dict[str, str] = field(default_factory=dict)
    prototypes: List[str] = field(default_factory=list)

    def as_dict(self) -> Dict[str, Any]:
        return {
            "primary_contract": self.primary_contract.as_dict(),
            ...
            "state": self.state,
            "structLinkingInfo": self.struct_linking_info,
            "legacyStructLinking": self.legacy_struct_linking,
...
class CertoraBuildGenerator:
    ...
    def collect_for_file(self,
        ...
        for primary_contract in contracts_in_file:
            ...
            sdc = SDC(CertoraBuildGenerator.get_primary_contract_from_sdc(contracts_in_sdc, primary_contract),
                      compiler_collector,
                      primary_contract_address,
                      build_arg_contract_file,
                      srclist,
    ...
    @staticmethod
    def get_sdc_key(contract: ContractInSDC) -> str:
        return f"{contract.name}_{contract.address}"

    @staticmethod
    def get_primary_contract_from_sdc(contracts: List[ContractInSDC], primary: str) -> ContractInSDC:
        for x in contracts:
            if x.name == primary:
                return x
        assert False, "Didn't find the primary contract in the list"
    ...
    @staticmethod
    def get_fresh_backupdir() -> Path:
        """
        returns a non-existing directory for backing-up of sources pre-autofinder generation.
        ...
        """
        base = get_certora_sources_dir() / PRE_AUTOFINDER_BACKUP_DIR
        candidate = base
        folder_id = 0
    ...
    def build(self, context: CertoraContext) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            if context.method and (build_arg_contract_file in context.verified_contract_files):
                ...
                if context.mode == Mode.VERIFY or context.mode == Mode.ASSERT:
                    ...
                    sdc_with_verified_contract_name = next(
                        curr_sdc for curr_sdc in sdc_pre_finders if
                        curr_sdc.primary_contract.name == verified_contract_name)
            ...
            # in any case we want to have a backup of the .certora_sources.
            # We will call the folder .pre_autofinders and it will be inside .certora_sources
            # Why? Because otherwise we'll get a zipInput file with all the autofinders, which breaks
            # idempotency. We want the true original sources too!
            # But here it gets even weirder. If we compile contract A, B, C, and we successfully
            ...
            # We therefore need a new backup dir to checkpoint every successful autofinder generation.
            # It also tells us we cannot have full parallel compilation of all contracts.
            backup_dir = self.get_fresh_backupdir()
            shutil.copytree(get_certora_sources_dir(), backup_dir,  # directory should not exist!
                            ignore=shutil.ignore_patterns(PRE_AUTOFINDER_BACKUP_DIR,
                                                          f"{PRE_AUTOFINDER_BACKUP_DIR}.*"))  # skip previous backups
            ...
            if compiler_lang == CompilerLangSol():
                ...
                if not success:
                    ...
                    shutil.copytree(backup_dir, get_certora_sources_dir(), dirs_exist_ok=True,
                                    ignore=shutil.ignore_patterns(PRE_AUTOFINDER_BACKUP_DIR,
                                                                  f"{PRE_AUTOFINDER_BACKUP_DIR}.*"))
                    # self.build_source_tree(sources, context, overwrite=True)
            ...
            for added_finders, sdc in added_finders_to_sdc:
                ...
                if sdc.primary_contract.name in self.input_config.prototypes:
                    sdc.prototypes += self.input_config.prototypes[sdc.primary_contract.name]
                ...
                for library_address in sdc.library_addresses:
                    ...
                    sdc_lib = SDC(library_contract,
                                  ...
                                  [library_contract],
                                  [])
                    self.SDCs[self.get_sdc_key(sdc_lib.primary_contract)] = sdc_lib

                self.SDCs[self.get_sdc_key(sdc.primary_contract)] = sdc
        ...
        if self.input_config.struct_link is not None:
            ...
            for link in links:
                ...
                if not reference_to_replace_with_link.isnumeric() and not is_hex(reference_to_replace_with_link):
                    self.SDCs[source_to_update].struct_linking_info[reference_to_replace_with_link] = dst_address
                else:
                    if is_hex(reference_to_replace_with_link):
                    ...
                    build_logger.debug(f"STRUCT Linking {src_contract} ({source_to_update}) to {dst_address} in slot "
                                       f"{resolved_src_slot}")
                    self.SDCs[source_to_update].legacy_struct_linking[resolved_src_slot] = dst_address
        ...
        if self.input_config.verify is not None:
            ...
            self.certora_verify_struct = {
                ...
                "specfilesToImportDecls": {
                    path_updater(k): list(v) for k, v in
                    vq_spec_with_imports.spec_files_to_orig_imports.items()
                },
                "importFilesToOrigRelpaths": {
                    path_updater(k): str(Path(k).relative_to(Path().resolve())) for k in
                    vq_spec_with_imports.abspath_imports_to_locs.keys()
                }
            }
        ...
        if contract is not None:
            if len(self.build_generator.get_matching_sdc_names_from_SDCs(contract)) == 0:
                fatal_error(
                    ...
                    f"[{','.join(map(lambda x: x[1].primary_contract.name, self.build_generator.SDCs.items()))}]")
