...
@app.command()
def how(ctx: typer.Context, input_action_qry: str, force_response= None):
    ctx.ensure_object(AppState)
    state = ctx.obj
    ...
    if force_response:
        ...
        try:
            spinner.start()
            response = routeQuery(tier=state.env["TIER"], query_type="suggest", query={"query_text": input_action_qry}, user = {"uid": state.env["UID"], "idToken": state.idToken })
        except Exception as e:
            spinner.stop()
            typer.echo(f"{colorutils.colors.FAIL} Encountered error when querying OpenAI API: {e}. \n Exiting...")
            raise typer.Exit()
    ...
    if action == "y":
        ...
        what(ctx=ctx, command_str=response)        
        how(ctx=ctx, input_action_qry="", force_response=response)

    elif action == "a":

        alt_resp = alternatives(ctx=ctx, command_str=response)
        alt_action = typer.prompt(f"\nSwitch to alternative command: {colorutils.colors.BOLD}{alt_resp}{colorutils.colors.ENDC}? [y/N]", type=str)
...
@app.command()
def chat(ctx: typer.Context, user_text: str):
    ctx.ensure_object(AppState)
    state = ctx.obj
    ...
    try:
        chat_response = routeQuery(tier=state.env["TIER"], query_type="chat", query={"query_text": user_text}, user = {"uid": state.env["UID"], "idToken": state.idToken })

    except Exception as e:
        spinner.stop()
        typer.echo(f"{colorutils.colors.FAIL} Encountered error communicating with server. Please try again later or contact support with this error: {e}. \n Exiting...")
        raise typer.Exit()
    else:
        spinner.stop()
...
@app.command()
def alternatives(ctx: typer.Context, command_str: str):
    ctx.ensure_object(AppState)
    state = ctx.obj
    ...
    alt_response = ""
    try:
        alt_response = routeQuery(tier=state.env["TIER"], query_type="alternatives", query={"query_text": command_str}, user = {"uid": state.env["UID"], "idToken": state.idToken })
...
@app.command() 
def what(ctx: typer.Context, command_str: str):
    ctx.ensure_object(AppState)
    state = ctx.obj
    ...
    try: 
        response = routeQuery(tier=state.env["TIER"], query_type="explain", query={"query_text": command_str}, user = {"uid": state.env["UID"], "idToken": state.idToken })
        ...
        # if stop_reason != "stop":
        #     typer.echo(f"{colorutils.colors.WARNING} Output stopped prematurely:{colorutils.colors.ENDC} {stop_reason}.")
...
def manage(ctx: typer.Context):
    ...
    if not checkTier(state, 2):
        ...
        try:
            ...
            typer.echo("Any changes to your account will be reflected in the Platitude.ai tool shortly. Thanks for being a subscriber! \n")
...
def subscribe(ctx: typer.Context):
   ...
   {colorutils.colors.BOLD}                     [1]                     [2]                     [3]          {colorutils.colors.ENDC}
            ...
            typer.echo("Once you checkout, Platitude Terminal will be ready to use immediately! You can manage your account at any time using `ai manage`.\n")
