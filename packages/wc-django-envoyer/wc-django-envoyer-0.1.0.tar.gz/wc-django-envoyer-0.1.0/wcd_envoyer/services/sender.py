...
TemplatedMessagesGroup = Tuple[BaseMessagingBackend, ChannelConfig, TemplatedMessages]
...
class Sender(KwargsInjector):
    ...
    def send_messages_groups(
        ...
        for backend, config, messages in messages_groups:
            backend.send(config, list(messages), context=context)

    def send_messages(self, messages: Iterable[Message], context: dict = {}):
        backends = self.get_backends()
        configs = self.get_channel_configs(backends.keys())

        self.send_messages_groups(
            [
                (backends[channel], configs[channel], messages)
    ...
    def send(
        ...
        self.send_messages_groups(messages_groups, context=context)

    def resend(self, messages: Iterable[Message], context: dict = {}):
        self.send_messages(messages, context=context)
