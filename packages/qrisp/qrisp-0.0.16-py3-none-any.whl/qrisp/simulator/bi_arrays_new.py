...
# the SparseBiArray class. The DenseBiArray class is an interface to numpy arrays, that
# also use the restriction of the dimension being a power of two. For instance does the
# DenseBiArray class support constant time reshaping and axis swapping. The reshaping
# and axis swappings are "collected" and executed together once the array is required
# for contraction.
...
# ank n tensor interface of the "sparse" package.
...
# last two digits of the bitstring to the front
# After the axis swap we have
...
class SparseBiArray(BiArray):
    ...
    # init_object can be
    # - a numpy array
    # - a tuple of arrays where init_object[0] is an array of non-zero indices and
    #     init_object[1] the corresponding data
    ...
    def __init__(self, init_object, shape=None, contraction_counter=0):
        ...
        if isinstance(init_object, np.ndarray):
            ...
            # np.around(raveled_array, 8, out = raveled_array)
    ...
    # This method checks if the sparse array has a thread object, which means that its
    # data is under construction. If so, it waits until the thread is finished
    def catch_up(self):
    ...
    def swapaxes(self, i, j):
        ...
        # these two intervals have to be swapped. However, in order to make this method
        # constant in time, the actual swapping of the indices is not performed here -
        # rather we log the permutation of the indices.
        ...
        # example [0,1,2,3,4,5,6,7]. In order to log the axis swap, we perform this
        # permutation on the .index_bit_permutation attribute, i.e. after the
        # application of this method, we want it to be [5,6,7,2,3,4,0,1].
    ...
    def move_index_to_front(self, index):
        ...
        # The logic for creating the self.index_bit_permutation attribute is similar
        # as in swapaxes.
    ...
    # Calculate the squared norm, i.e. the sesquilinear scalar product of self with
    # itself.
...
# about wether it's treating a sparse array or a dense array. Furthermore, some of the
# multithreading techniques from the SparseBiArray class are deployed.
class DenseBiArray(BiArray):
    ...
    def apply_swaps(self):
        ...
        # dimension two and permute the axes.
        ...
        if False:
            ...
            # sigma = [abs(len(index_perm) - 1 - i) for i in sigma[::-1]]
            ...
            # self.data = self.data.ravel()[permutated_indices]
    ...
    # ie. identify contraction axes, move to the front, transpose one matrix
    ...
    def contract(self, other, axes_self, axes_other):
        ...
        # Therefore, we perform n = contraction_size random experiments with
        # probability p_z. The result entry is zero if all products are zero, so
        ...
        res = DenseBiArray(
            np.array([self.data.flatten()[0] * other.data.flatten()[0]]),
...
def tensordot(a, b, axes, contract_sparsity_threshold=0.01):
    ...
    if len(axes[0]) == 0:
        ...
        if isinstance(a, SparseBiArray) and isinstance(b, SparseBiArray):
            ...
            def mp_wrapper():
                ...
                # and sizes 16, 8  respectively the nz_indices of the result can be
                # ordered in a matrix:
                # [1,2,3] + 2*16
                # [1,2,3] + 3*16
                # [1,2,3] + 6*16
            ...
            res = DenseBiArray(
                np.array([a.data.flatten()[0] * b.data.flatten()[0]]),
    ...
    # Convert between arrays if necessary
    # if True:
    # if False:
    if (
        res_sparsity > contract_sparsity_threshold or a.size * b.size < 2**12
    ) and not a.size * b.size / contraction_size > 2**32:
