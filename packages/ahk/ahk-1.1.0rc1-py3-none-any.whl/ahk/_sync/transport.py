...
SyncIOProcess: TypeAlias = asyncio.subprocess.Process  # unasync: remove
...
class SyncAHKProcess:
    ...
    def drain_stdin(self) -> None:  # unasync: remove
        assert self._proc is not None
...
def sync_create_process(runargs: List[str]) -> asyncio.subprocess.Process:  # unasync: remove
...
class Transport(ABC):
    ...
    def send_nonblocking(  # unasync: remove
        self, request: RequestMessage, engine: Optional[AHK] = None
    ) -> FutureResult[
        Union[None, Tuple[int, int], int, str, bool, Window, List[Window], List[Control]]
    ]:
...
class DaemonProcessTransport(Transport):
    ...
    def _sync_run_nonblocking(  # unasync: remove
        self, proc: Communicable, script_bytes: Optional[bytes], timeout: Optional[int] = None
    ) -> FutureResult[str]:
        loop = asyncio.get_running_loop()

        def f() -> str:
            stdout, stderr = proc.acommunicate(script_bytes, timeout)
