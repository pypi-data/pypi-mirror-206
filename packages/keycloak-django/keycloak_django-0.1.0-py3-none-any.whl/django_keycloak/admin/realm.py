...
from django_keycloak.services.synchronize import RolePermissionKeyclock
...
def synchronize_permissions(modeladmin, request, queryset):
    for realm in queryset:
        try:
            ...
            #     client=realm.client)
            # print(modeladmin.name)
            rolepermissionkeyclock = RolePermissionKeyclock(realm=realm)
            rolepermissionkeyclock.synchronize_permissions_roles()
            print(realm)
