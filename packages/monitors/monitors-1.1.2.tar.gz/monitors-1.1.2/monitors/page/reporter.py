...
from monitors.page.classifier import ClassifierResult, SingleClassifierResult
from monitors.page import toolbox
from monitors.page import constants
from monitors.page.cutter import VideoCutResult, VideoCutRange
from monitors.page.video import VideoFrame
...
def gen_report(results=None, report_path=None, report_name='monitors_summary.html'):
    """
    gen_report
    ...
    template = env.get_template("summary_template.html", template_dir_path)
    html = template.render({"results": format_list})
    output_file = os.path.join(report_path, report_name)
    with io.open(output_file, 'w', encoding="utf-8") as f:
        f.write(html)
    logger.info(f"save custom summary report to {output_file}")
