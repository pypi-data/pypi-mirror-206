...
class RespondersLaterBid(Hand):
    ...
    def suggested_bid(self):
        ...
        if self.opener_bid_one.name == '2C':
            ...
            bid = self.next_level_bid(self.opener_bid_three.denomination, '3809')
        ...
        elif (self.opener_bid_one.is_major and self.suit_holding[self.opener_bid_one.denomination] >=3 and
                    self.openers_last_bid.name == '3NT'):
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3860')
        elif self._opener_bids_three_nt_after_change_of_major():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3802')
        ...
        # Misc bids
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3808')

        elif self._two_card_support_for_openers_six_card_suit_twelve_points():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3812')
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3814')
            ...
            bid = self.bid_to_game(self.opener_bid_one.denomination, '3820')

        # elif (self.openers_last_bid.denomination == self.bid_one.denomination and
        #         len(self.bid_history) >= 10 and
        #         not self.bid_two.is_pass and
        #         12 <=self.hcp <= 15 and
        #         len(self.bid_history) >= 10 and
        #         self.nt_level <= 3):
        #     bid = self.suit_bid(4, self.bid_one.denomination, '3852')
        ...
        # 3NT bids
            ...
            bid = self.next_level_bid(self.opener_bid_three.denomination, '3822')
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3817')
        ...
        # Pass bids
        # elif self.openers_penultimate_bid.name == '5NT':
        #     bid = Pass('3801')
            ...
            bid = Pass('3823')
        elif self.openers_last_bid.denomination == self.openers_first_bid.denomination and self.nt_level == 3:
            bid = Pass('3843')
        elif self.openers_last_bid.is_pass:
            bid = Pass('3846')
    ...
    def _after_my_4nt(self):
        ...
        if aces == 4:
            ...
            bid = self.suit_bid(6, self.opener_bid_one.denomination, '3827')

        # elif aces >= 3 and self.kings >= 3:
        #     bid = self.nt_bid(5, '3828')
        # elif self.openers_last_bid.denomination == Blackwood(self.cards, self.board).agreed_suit:
        elif self.openers_last_bid.denomination == self.diamond_suit and self.suit_preference() == self.diamond_suit:
            bid = Pass('3847')
        elif self.nt_level == 5:
    ...
    def _opener_has_rebid_second_suit(self):
        ...
        if self.hcp >= 15:
            ...
            if (self.shape[0] >= 6 and
                ...
                suit = self.opener_bid_one.denomination
            bid = self.next_level_bid(suit, '3837')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
