...
# DO NOT IMPORT ANYTHING FROM monitors.page HERE
# MAKE TOOLBOX STATIC
...
def find_all_cases(base, mark, types):
    """

    :param base: path
    :param types: dirs or files
    :param mark: eg: '.mp4'
    :return: files or dirs list
    """
    for root, ds, fs in os.walk(base, topdown=True):
        if types in ["dirs"]:
            for f in ds:
                if f.endswith(f'{mark}'):
...
def copyfile(src_file, dst_path):
    """
    copy files
    :param src_file: Source file address
    :param dst_path: target file address
    :return:
    """
    if not os.path.isfile(src_file):
        logger.info(f"{src_file} not exist!")
    else:
        f_path, f_name = os.path.split(src_file)
        if not os.path.exists(dst_path):
            os.makedirs(dst_path)
        shutil.copy(src_file, dst_path + f_name)
        logger.info(f"copy {src_file} -> {dst_path + f_name}")
