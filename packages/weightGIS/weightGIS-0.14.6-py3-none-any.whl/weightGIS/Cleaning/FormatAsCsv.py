...
class FormatAsCsv:
    ...
    def _extract_place(self, place: str, p_values: dict) -> List[List[str]]:
        ...
        for date in self._dates:
            values = [self._extract_value(attr, date, p_values) for attr in self._attrs]
            if sum([1 for v in values if v == "NA"]) != len(values):
                place_data.append(self._format_names(place, date) + values)
