...
# Needs to be integrated with react  ### Latest @Pratyush
@app.route('/api/v1/evaluate')
def evaluate_stream():
    session_id = request.args.get('session_id')
    mode = request.args.get('mode')
    track_num = int(request.args.get('track_num'))
    rewards_streamer = RewardsStreamer(
        session_id=session_id
    )
...
def push_model():
    ...
    s3 = boto3.client(
        's3',
        aws_access_key_id="AKIA3WLRZ2QR7B7HDFEJ",
        aws_secret_access_key="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"
    )
 
    def uploadDirectory(path, bucketname):
        for root, dirs, files in os.walk(path):
            for file in files:
                s3.upload_file(os.path.join(root,file), bucketname, "Models/" + user_id + "/" + file)
 
    json_object = json.dumps(user)
    with open("user.json", "w") as outfile:
        outfile.write(json_object)
 
    s3.upload_file("user.json", "clash-of-models", "Models/" + user_id + "/" + "user.json")
 
    path = os.path.join(os.path.join(utils.get_home_path(), ".rewards_ai"), session_id)
...
@app.route('/api/v1/get_user_list')
def get_user_list():
    s3 = boto3.client(
        's3',
        aws_access_key_id="AKIA3WLRZ2QR7B7HDFEJ",
    ...
    for user in list(set_of_users):
        ...
        text = result["Body"].read().decode()
        text = json.loads(text)
        score = text["evaluation"]["1"]
        try:
            temp["score"] = score["record"]
        except:
            temp["score"] = 0
