...
class DataContainer:
    ...
    def _generate_parameter_representation(self) -> None:
        spherical_harmonic_parameters = SphericalHarmonicParameters(
            self._stack.geometry,
            l_max=0)
        reconstruction_output = ReconstructionOutput(
            residual_gradient=np.ones(np.prod(self._stack.geometry.volume_shape)),
            reconstruction_projection=np.zeros_like(self._stack.data.ravel()),
            residual=0.0,
            residual_per_projection=np.zeros(len(self._stack), dtype=np.float64))
        reconstruction_input = ReconstructionInput(np.ones(np.prod(self._stack.geometry.volume_shape)))
        projection_parameters = ProjectionParameters(
            self._stack.geometry,
            integration_step_size=1. / 3.)
        self._reconstruction_parameters = ReconstructionParameters(
            settings=np.ones(3),
            data=self._stack.data.ravel(),
            diode=self._stack.diode.ravel(),
            projection_weights=self._stack.weights.ravel(),
    ...
    def correct_for_transmission(self) -> None:
        ...
        if self._generated_parameters:
            self._reconstruction_parameters.data = (
                self._reconstruction_parameters.data.reshape(-1, len(self.geometry.detector_angles)) /
