...
from .metrics import Metric, getClassForMetric
...
class Experiment(NetworkObject):
    ...
    def createMetrics(self, values: List[Tuple[str, str, str]]) -> List[Metric]:
        """
            ...
            values : List[Tuple[str, str, str]]
                Metric meta in this format ("name", "x_label", "y_label")
        ...
        metrics: List[Metric] = []
        for name, xLabel, yLabel in values:
            clazz = getClassForMetric(name)

            if clazz is not None:
                metric = clazz.create(name, xLabel, yLabel)
                metrics.append(metric)

            else:
                metric = Metric.create(name, xLabel, yLabel)
                metrics.append(metric)
    ...
    def startCustomExperiment(
        ...
        """
            ...
            Returns
            -------
            Optional[Self] -> if response is failed returns None, otherwise returns Experiment object
