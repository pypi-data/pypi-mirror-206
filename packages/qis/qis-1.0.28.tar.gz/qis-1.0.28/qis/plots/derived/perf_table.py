...
def get_ra_perf_table(prices: Union[pd.DataFrame, pd.Series],
    ...
    if drop_index:
        data = data.set_index(column_header, drop=True)
    return data
...
def plot_ra_perf_table(prices: Union[pd.DataFrame, pd.Series],
    ...
    plot ra perf table
    """
    df = get_ra_perf_table(prices=prices,
...
def plot_ra_perf_table_benchmark(prices: pd.DataFrame,
    ...
    for perf_column in perf_columns:
        ...
        data[perf_column.to_str(**kwargs)] = dfs.series_to_str(ds=ra_perf_table[perf_column.to_str()],
                                                               var_format=perf_column.to_format(**kwargs))
