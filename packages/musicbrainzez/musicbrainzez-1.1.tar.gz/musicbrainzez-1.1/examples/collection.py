...
musicbrainzez.set_useragent(
    "python-musicbrainzez-example",
    "0.1",
    "https://github.com/alastair/python-musicbrainzez/",
)

def show_collections():
    """Fetch and display the current user's collections.
    """
    result = musicbrainzez.get_collections()
    print('All collections for this user:')
    for collection in result['collection-list']:
        # entity-type only available starting with musicbrainzez 0.6
...
def show_collection(collection_id, ctype):
    ...
    """
    if ctype == "release":
        result = musicbrainzez.get_releases_in_collection(
                                                collection_id, limit=0)
    elif ctype == "artist":
        result = musicbrainzez.get_artists_in_collection(
                                                collection_id, limit=0)
    elif ctype == "event":
        result = musicbrainzez.get_events_in_collection(
                                                collection_id, limit=0)
    elif ctype == "place":
        result = musicbrainzez.get_places_in_collection(
                                                collection_id, limit=0)
    elif ctype == "recording":
        result = musicbrainzez.get_recordings_in_collection(
                                                collection_id, limit=0)
    elif ctype == "work":
        result = musicbrainzez.get_works_in_collection(
                                                collection_id, limit=0)
...
def show_releases(collection):
    result = musicbrainzez.get_releases_in_collection(collection_id, limit=25)
    ...
    while len(release_list) > 0:
        ...
        result = musicbrainzez.get_releases_in_collection(collection_id,
...
if __name__ == '__main__':
    ...
    # Call musicbrainzez.auth() before making any API calls that
    # require authentication.
    musicbrainzez.auth(username, password)
