...
class PerformSelectionDistributable(object):  # implements IDistributable
    ...
    def work_count(self):
        return (
            self.feature_selection_strategy.num_folds + 1
    ...
    def work_sequence(self):
        ...
        for fold_idx in range(self.feature_selection_strategy.num_folds):
            yield lambda fold_idx=fold_idx: self.dowork(
                fold_idx
            )  # the 'fold_idx=fold_idx is need to get around a strangeness in Python
        yield lambda output_prefix=self.output_prefix: self.feature_selection_strategy.linreg_entire_dataset(
            output_prefix
        )
    ...
    def reduce(self, result_sequence):
        ...
        if (self.strategy == "insample_cv") or (self.strategy == "lmm_full_cv"):
            if (
                len(ll_cv) != len(mse_cv)
                or len(mse_cv) != self.feature_selection_strategy.num_folds
            ):
                raise Exception(
                    "These should be the same. len(ll_cv)={0}, len(mse_cv)={1}, self.feature_selection_strategy.num_folds={2}".format(
                        len(ll_cv),
                        len(mse_cv),
        ...
        if mse_cv is not None:
            (
                ...
                best_delta_mse,
                best_mse,
                best_delta_mse_interp,
                best_mse_interp,
            ) = self.feature_selection_strategy.reduce_result(
                mse_cv,
                self.k_values,
                self.delta_values,
                self.strategy,
        ...
        if self.select_by_ll:
            best_k, best_delta, best_obj, best_delta_interp, best_obj_interp = (
                best_k_ll,
                best_delta_ll,
                best_ll,
                best_delta_ll_interp,
    ...
    def dowork(self, fold_idx):
        ...
        for i_k, k in enumerate(self.k_values):
            self.k_values[i_k] = min(
                self.k_values[i_k], self.feature_selection_strategy.snpreader.sid_count
            )
        max_k = max(
            [1]
            + [
        ...
        split_iterator = self.feature_selection_strategy.setup_linear_regression(
            max_k, start=fold_idx, stop=None
        )
        ...
        for k_idx, k in enumerate(self.k_values):
            ...
            logging.info(
                "reporter:status:processing fold={0}, k={1}".format(fold_idx, k)
            )
            ...
            if (
                k == self.feature_selection_strategy.snpreader.sid_count
                or k >= self.feature_selection_strategy.num_snps_in_memory
            ):
                if k == self.feature_selection_strategy.snpreader.sid_count:
                    ...
                    subset = self.feature_selection_strategy.snpreader[
                        :, top_k_feat_idx
                    ]
                    K = subset.kernel(
                        self.feature_selection_strategy.standardizer,
                        blocksize=self.feature_selection_strategy.blocksize,
                    )
            ...
            if self.strategy == "lmm_full_cv":
                ...
                for delta_idx, delta_act in enumerate(self.delta_values):
                    ...
                    mse_cv1[k_idx, delta_idx] = mean_squared_error(
                        fold_data["y_test"], out
                    )
                    ll_cv1[k_idx, delta_idx] = model.nLLeval_test(
                        fold_data["y_test"],
                        res["beta"],
                        sigma2=res["sigma2"],
                        delta=delta,
                ...
                def f(x):
                    if k:
                        ...
                        delta_corr = x
                    myres = model.nLLeval(
                        delta=delta_corr, REML=REML, penalty=self.penalty
                    )
                ...
                if 0:  # old code without brent search
                    for delta_idx, delta_act in enumerate(self.delta_values):
                        ...
                        res = model.nLLeval(
                            delta=delta, REML=REML, penalty=self.penalty
                        )
                ...
                ll_cv1[k_idx] = model.nLLeval_test(
                    fold_data["y_test"],
                    resmin[0]["beta"],
                    sigma2=resmin[0]["sigma2"],
                    delta=resmin[0]["delta_corr"],
                    Kstar_star=K_test_test,
                )
...
def build_kernel_blocked(
    snpreader, snp_idx=None, blocksize=10000, alt_snpreader=None, allowlowrank=False
):
    ...
    if snp_idx != None:
        ...
        logging.info(
            ...
            % (len(snp_idx), blocksize)
        )
    else:
        current_size = num_snps
        logging.info(
            "constructing K from all %i SNPs (for %i individuals)" % (num_snps, N)
        )

    ct = 0
    ts = time.time()

    if (not allowlowrank) and alt_snpreader.snp_count < N:
