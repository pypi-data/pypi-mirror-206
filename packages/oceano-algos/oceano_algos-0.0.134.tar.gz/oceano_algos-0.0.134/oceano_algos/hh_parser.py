...
__all__ = ['TextPreprocessor', 'preprocess_string', 'preprocess_series', 'HeadHunterScrapper',
           ...
           'get_url_vs_similarity', 'Telegram', 'send_vacancy', 'send_recommended_vacancies_to_chatid',
...
from .texts import welcome_text, after_rocket_text, settings_text, start_search_text, stop_search_text, no_recommended_vacancies_text
import telebot
...
class Telegram:
    def __init__(self, postgres_creds, token="6137609275:AAEJcRfFHrvHW792-gXA_kCDC-d2meVE0YM"):
        ...
        self.chatid_vs_settings = self.pc.download(f"SELECT * FROM main_schema.chatid_vs_settings")
...
"""
        ...
        # button2   =  InlineKeyboardButton(text="⚙️ Изменить или остановить поиск", url=url)
        # keyboard.add(button2)

        button3 = InlineKeyboardButton(text="➡️ Показать еще", callback_data="show_more")
        keyboard.add(button3)

        m = self.bot.send_message(chat_id, text, parse_mode="HTML", reply_markup=keyboard)

    except:
        pass

    # try to delete previous message if it's '⏳ Сейчас посмотрю...':
    try:
        if self.bot.get_message(chat_id, m.message_id-1).text == '⏳ Сейчас посмотрю...':
            self.bot.delete_message(chat_id, m.message_id-1)
    except:
        pass
...
def send_recommended_vacancies_to_chatid(self, chat_id):
    ...
    # filter vacancies:
    filtered_vacancies = url_vs_similarity[max_responses_filter & min_salary_filter & experience_filter].sort_values('similarity', ascending=False)
    ...
    if filtered_vacancies.shape[0] > 0:
        ...
        self.bot.send_message(chat_id, no_recommended_vacancies_text())
...
def send_recommended_vacancies_to_chatids_with_status_1(self):
    ...
    for chat_id in chat_ids:
        self.send_recommended_vacancies_to_chatid(chat_id)