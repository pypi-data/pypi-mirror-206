...
class LMM(object):
    ...
    def __init__(
        ...
        self.numcalls = 0
        self.setX(
            X=X, regressX=regressX, linreg=linreg
        )  # set the covariates (needs to be first)
    ...
    def setSU_fromG(self):
        ...
        if k:
            if (not self.forcefullrank) and (k < N):
                ...
                [self.U, self.S, V] = self._xp.linalg.svd(
                    PxG, full_matrices=False, compute_uv=True
                )  # , N x min, min, min x k
                logging.info("Ending SVD")
                inonzero = self._xp.arange(len(self.S))[
                    self.S > 1e-10
                ]  # This 'arange' trick allows this indexing to work whether the svd is "full_matrix" or not.
    ...
    def findH2_2K(
        ...
        **kwargs
        ...
        def f(x, resmin=resmin, **kwargs):
            if noG1:
                res = self.nLLeval_2K(
                    h2_1=0.0, i_up=i_up, i_G1=i_G1, UW=UW, UUW=UUW, h2=x, **kwargs
                )
            else:
                res = self.innerLoop_2K(
                    h2=x,
                    i_up=i_up,
                    i_G1=i_G1,
                    UW=UW,
    ...
    def find_log_delta(
        self, sid_count=1, min_log_delta=-5, max_log_delta=10, nGrid=10, **kwargs
    ):
        """
        ...
        def f(x, resmin=resmin, **kwargs):
            h2 = 1.0 / (
                np.exp(x) * sid_count + 1
            )  # We convert from external log_delta to h2 and then back again so that this
    ...
    def findH2(
        self, nGridH2=10, minH2=0.0, maxH2=0.99999, estimate_Bayes=False, **kwargs
    ):
        """
        ...
        if self.Y.shape[1] > 1:
            ...
            (evalgrid, resultgrid) = evalgrid1D(
                f,
                evalgrid=None,
                nGrid=nGridH2,
                minval=minH2,
    ...
    def posterior_h2(self, nGridH2=1000, minH2=0.0, maxH2=0.99999, **kwargs):
        ...
        def f(x):
            ...
            if resmin[0] is None:
                resmin[0] = {
                    "nLL": res["nLL"],
                    "h2": np.zeros_like(res["nLL"]) + res["h2"],
                }
            else:
                for i in range(self.Y.shape[1]):
    ...
    def nLLeval_2K(
        ...
        if (
            subset
        ):  # if G1 is a complete subset of G, then we don't need to subtract and add separately
            h2_1 = h2_1 - h2
    ...
    def nLLeval(
        ...
        if np.any(h2 < 0.0) or np.any(h2 >= 1.0):
            assert (
                P == 1
            ), "Expect h2 to be out of range only when looking at one phenotype at a time"
            return {"nLL": 3e20, "h2": h2, "scale": scale}
        ...
        if weightW is not None:
            ...
            weightW = (
                weightW * h2
            )  # Christoph: fixes bug with h2_1 parameterization in findA2 and/or findH2 and/or innerLoop
    ...
    def get_Sd_etc(self, Sd, denom, h2, logdelta, delta, scale, weightW):
        ...
        if delta is not None:
            ...
            assert (
                weightW is None
            ), "weightW should be none when used with delta or logdelta parameterization, which support only a single Kernel"
        else:
    ...
    def nLLcore(
        ...
        if Usnps is not None:
            ...
            for pheno_index in range(P):
                snpsKsnps[:, pheno_index] = self.computeAKA(
                    Sd[:, pheno_index : pheno_index + 1],
                    denom=denom[pheno_index],
                    UA=Usnps,
                    UUA=UUsnps,
                )
                snpsKY[:, pheno_index : pheno_index + 1] = self.computeAKB(
                    Sd[:, pheno_index],
                    denom=denom[pheno_index],
                    UA=Usnps,
        ...
        if UW is not None and weightW_nonz.any():  # low rank updates
            assert (
                P == 1
            ), "currently no support for this code path and multiple phenotypes"
            multsign = False
            absw = np.sqrt(absw)
            ...
            if multsign:
                WW = np.eye(num_exclude) + self.computeAKB(
                    Sd=Sd, denom=denom, UA=UW, UUA=UUW, UB=UW_, UUB=UUW_
                )
            else:
                WW = np.diag(signw) + self.computeAKB(
                    Sd=Sd, denom=denom, UA=UW, UUA=UUW, UB=UW, UUB=UUW
                )
            ...
            UWY = U_WW.T.dot(WY)
            WY = UWY / np.lib.stride_tricks.as_strided(
                S_WW, (S_WW.size, UWY.shape[1]), (S_WW.itemsize, 0)
            )
            # compute S_WW^{-1} * UWy
            ...
            if Usnps is not None:
                Wsnps = self.computeAKB(
                    Sd=Sd, denom=denom, UA=UW, UUA=UUW, UB=Usnps, UUB=UUsnps
                )
                UWsnps = U_WW.T.dot(Wsnps)
                Wsnps = UWsnps / np.lib.stride_tricks.as_strided(
                    S_WW, (S_WW.size, UWsnps.shape[1]), (S_WW.itemsize, 0)
                )
    ...
    def computeAKB(self, Sd, denom, UA, UB, UUA=None, UUB=None):
        ...
        assert (UUA is None) == (
            UUB is None
        ), "Expect UUA and UUB to either both be given or both not"
        assert len(Sd.shape) == 1 or (
            len(Sd.shape) == 2 and Sd.shape[1] == 1
        ), "expect Sd to be 1-D"
    ...
    def set_beta(self, Y):
        ...
        if self.X is None:
            ...
            if self.Xdagger is None:
                if self.X.shape[1]:
                    self.Xdagger = self._xp.linalg.pinv(
                        self.X
                    )  # SVD-based, and seems fast
                else:
