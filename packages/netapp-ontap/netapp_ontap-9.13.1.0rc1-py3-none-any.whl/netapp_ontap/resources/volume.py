...
class VolumeSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    tags = fields.List(fields.Str, data_key="_tags")
...
* promote"""
    ...
    cloud_write_enabled = fields.Boolean(
...
This feature is only available on volumes in FabricPools on FSx or Cloud Volumes ONTAP."""
...
Example: vol_cs_dept"""
    ...
    optimize_aggregates = fields.Boolean(
...
* ordinal"""
    ...
    snapshot_directory_access_enabled = fields.Boolean(
...
* restricted"""
    ...
    r""" The style of the volume. If "style" is not specified, the volume type is determined based on the specified aggregates or license. Specifying a single aggregate, without "constituents_per_aggregate", creates a flexible volume. Specifying multiple aggregates, or a single aggregate with "constituents_per_aggregate", creates a FlexGroup volume. When the UDO License is installed, and no aggregates are specified, the system automatically provisions a FlexGroup volume on system selected aggregates. Specifying a volume "style" creates a volume of that type. For example, if the style is "flexvol", you must specify a single aggregate. If the style is "flexgroup", the system either uses the specified aggregates or automatically provisions aggregates if there are no specified aggregates. The style "flexgroup_constiutent" is not supported when creating a volume.<br>flexvol &dash; flexible volumes and FlexClone volumes<br>flexgroup &dash; FlexGroup volumes<br>flexgroup_constituent &dash; FlexGroup constituents.
    ...
    if RECLINE_INSTALLED:
        ...
        def volume_show(
            ...
            Args:
                ...
                cloud_write_enabled: Indicates whether or not cloud writes are enabled on the volume. NFS writes to this volume are sent to the cloud directly instead of the local performance tier. This feature is only available on volumes in FabricPools on FSx or Cloud Volumes ONTAP. 
    ...
    if RECLINE_INSTALLED:
        ...
        async def volume_modify(
            ...
            Args:
                ...
                aggressive_readahead_mode: Specifies the aggressive readahead mode enabled on the volume. When set to \"file_prefetch\", on a file read, the system aggressively issues readaheads for all of the blocks in the file and retains those blocks in a cache for a finite period of time. This feature is only available on FabricPool volumes on FSx for ONTAP and Cloud Volumes ONTAP.
                query_aggressive_readahead_mode: Specifies the aggressive readahead mode enabled on the volume. When set to \"file_prefetch\", on a file read, the system aggressively issues readaheads for all of the blocks in the file and retains those blocks in a cache for a finite period of time. This feature is only available on FabricPool volumes on FSx for ONTAP and Cloud Volumes ONTAP.
                anti_ransomware_state: The Anti-ransomware state of the volume. If no \"anti_ransomware_state\" property is specified, the volume inherits the value from its parent SVM's \"anti_ransomware_default_volume_state\" property. If this value is \"disabled\", Anti-ransomware is disabled on the volume. If this value is \"enabled\", Anti-ransomware is enabled on the volume and alerts are raised if any suspect is detected for those volumes. If this value is \"dry_run\", Anti-ransomware is enabled in the dry-run or learning mode on the volume. The \"dry_run\" state is same as the \"enabled\" state except that the analytics data is used here for learning. No alerts are raised for any detections or violations. If this value is \"paused\", Anti-ransomware is paused on the volume. Additionally, three more states are available, which are only valid for GET. If this value is \"disable_in_progress\", Anti-ransomware monitoring is being disabled and a cleanup operation is in effect. If this value is \"enable_paused\", Anti-ransomware is paused on the volume from its earlier enabled state. If this value is \"dry_run_paused\", Anti-ransomware monitoring is paused on the volume from its earlier dry_run state. For POST, the valid Anti-ransomware states are only \"disabled\", \"enabled\" and \"dry_run\", whereas for PATCH, \"paused\" is also valid along with the three valid states for POST.
                ...
                cloud_write_enabled: Indicates whether or not cloud writes are enabled on the volume. NFS writes to this volume are sent to the cloud directly instead of the local performance tier. This feature is only available on volumes in FabricPools on FSx or Cloud Volumes ONTAP. 
                query_cloud_write_enabled: Indicates whether or not cloud writes are enabled on the volume. NFS writes to this volume are sent to the cloud directly instead of the local performance tier. This feature is only available on volumes in FabricPools on FSx or Cloud Volumes ONTAP. 
                comment: A comment for the volume. Valid in POST or PATCH.
