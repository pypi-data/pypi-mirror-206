...
# The OVF file is a widely used, human-readable format to store them
#
# The ovf package can be installed conveniently via pip
# We also put a copy in the "misc" directory
...
def parse_ovf(fil,parse_params=False):
    assert os.path.isfile(fil), 'File {} not found!'.format(fil)    
    ...
    with open(filename,mode) as fw:
        if mode=='w':
            ...
            if 'nsegment' in params.keys():
                fw.write('#\n# Segment count: {}\n#\n'.format(params['nsegment']))
        spins = spins.reshape(params['nsegment'],-1,valuedim)
        for iseg in range(params['nsegment']): write_one_segment(fw,params,spins[iseg])
...
if __name__=='__main__':
    print ('Processing with the spin vector fields in ovf format')
