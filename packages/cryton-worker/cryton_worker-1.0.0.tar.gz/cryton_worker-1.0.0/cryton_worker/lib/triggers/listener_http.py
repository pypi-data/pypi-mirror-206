...
class HTTPListener(Listener):
    def __init__(self,  main_queue: Queue, host: str, port: int):
        """
    ...
    def start(self) -> None:
        ...
        if self._stopped:
            ...
            self._process = Process(target=self._app.run,
                                    kwargs={co.LISTENER_HOST: self._identifiers[co.LISTENER_HOST],
                                            co.LISTENER_PORT: self._identifiers[co.LISTENER_PORT], "quiet": True})
            self._process.start()
            self._stopped = False
