...
coverage: ## check code coverage quickly with the default Python
	coverage run --source mvn -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/source/mvn.rst
	rm -f docs/source/modules.rst
	sphinx-apidoc -o docs/source/ mvn
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
version: ## display git versioning tags and the actual current version from versioneer
	@git tag | grep "mvn-v" | sed -e "s|mvn-v||g"
	@echo "----------------"
	@python -c "import mvn; print(mvn.__version__+' [← current]')"
