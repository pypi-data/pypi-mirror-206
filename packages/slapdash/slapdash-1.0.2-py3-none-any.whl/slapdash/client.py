...
class RequestClient(ComClient):
    ...
    def __call__(self, method_name: str, **kwargs):
        req = requests.get(self._url + method_name,
                           timeout=self._timeout, params=kwargs)
        try:
            return req.json()
    ...
    def remote_call(self, method_name: str, **kwargs):
        resp = requests.post(self._url + re.sub('\]', '', re.sub('\.|\[', '/', method_name)),  # noqa
                             timeout=self._timeout, params=kwargs)
        if resp.ok:
            return resp.json()
...
class ParamArray(list):
    ...
    def __getitem__(self, index: int):
        ...
        if isinstance(item, dict):
            return make_class(self._client,
                              name,
                              parent=name + '.')
        else:
            return item
...
class Param:
    ...
    def __get__(self, obj, owner):
        ...
        interface = make_class(obj._client, name=self._name,
                               parent=self._name + '.')
...
def make_function(client: ComClient, name: str, args: list, doc: str = None, **kwargs):
    def _func(self, *args, **kwargs):
        ...
        kwargs = {k: v for k, v in sorted(
            kwargs.items(), key=lambda i: keywords.index(i[0]))}
...
def make_class(client: ComClient, name: str, props=None, parent='', **kwargs):
    ...
    for prop_name, prop in props.items():
        if prop['type'] == 'method':
            ...
            setattr(customClientClass,
                    method_name, make_function(client, **prop))

        else:
            setattr(customClientClass, prop['name'], Param(
                call_name=parent + prop_name, **prop))
...
class Client:
    def __new__(cls,
        ...
        if client_type is not None:
            client = client_type(hostname=hostname, port=port, timeout=timeout, *args, **kwargs)
        else:
