...
def upload_s3_ml_script(environment, config, aws_access_key_id = '', aws_secret_access_key = '', stepfunction_job = 'mlops_test_step_01',  bucket = 'dwe-mlops-hub-repository',  ssm_path = '/mlops/{0}/database/mongo'  ):
...
def get_config_and_tar(environment, model_id,  aws_access_key_id = '', aws_secret_access_key = '', stepfunction_job = 'mlops_test_step_01',  bucket = 'dwe-mlops-hub-repository',  ssm_path = '/mlops/{0}/database/mongo'  ):
...
#         print( image_config )
        ...
        get_version_list = list( collection.find({'model_id' : model_id }, {'script_ver' : 1, '_id' : 0}).sort([("_id", pymongo.DESCENDING)]).limit(1) )
        get_version = get_version_list[0]['script_ver'] + 1 
        
        
        
        script_path = '{0}/{1}/{2}/{3}'.format(environment, image_config['model_name'], get_version, 'sourcedir.tar.gz')
        ...
        s3 = aws_connection('s3', aws_access_key_id, aws_secret_access_key)
        src_key = '{0}/{1}/{2}/sourcedir.tar.gz'.format( environment, image_config['model_name'], str(image_config['script_ver']) ) 
        print( src_key )
        dst_key = '{0}/{1}/{2}/sourcedir.tar.gz'.format( environment, image_config['model_name'], str(get_version) ) 
        print( dst_key )
