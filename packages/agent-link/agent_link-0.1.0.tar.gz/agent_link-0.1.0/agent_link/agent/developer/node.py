...
import requests
from core.design.extract_figma_frames import extract_figma_frames 
import eventlet
from core.gen.react_tailwind_typescript.create import create_react_app_with_tailwind_typescript
from core.tools.copy_directory.copydir import copy_directory
...
extract_figma_frames(figma_file_key, access_token)

create_react_app_with_tailwind_typescript(project_dir)
...
async def process_specifications(ctx, specifications, github_repo_url, unique_dir, user_id):
    ...
    def generate_code(filename, specifications, created_files, design_specifications):
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a code generation assistant. Generate only pure code without comments or additional information."},
                {"role": "user", "content": prompt}
            ],
        )
