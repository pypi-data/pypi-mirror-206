...
def download_logo() -> np.ndarray:
    """
    Download the logo.png file to userprofile/.aij/logo.png
    """
    # get the user profile path
    user_profile = os.environ['USERPROFILE']
    _sep = os.path.sep
    logo_home = user_profile + _sep + '.aij' + _sep + 'logo.png'
    ...
    if logo_remote_response.status_code == 200 and not os.path.exists(logo_home):
        # write the csv file using pandas
        with open(logo_home, 'wb') as f:
            f.write(logo_remote_response.content)
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        logo = cv2.imread(get_logo_path(), cv2.IMREAD_UNCHANGED)
