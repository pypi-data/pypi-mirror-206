...
def get_channel_id(slack_client: WebClient, channel_name: str):
    try:
        ...
        logger.log(message="Error: " + str(e), to_terminal=True, log_level="error")
        sys.exit(1)
...
def send_slack(file_to_send: str, customer_name: str = None):
    ...
    channel_id = get_channel_id(channel_name=slack_channel_name, slack_client=slack_client)

    try:
        response = slack_client.files_upload_v2(
            channel=channel_id,
            file=file_to_send,
            title=f"{customer_name}: File upload for SD Uploader" if customer_name else "File upload for SD Uploader",
            filename=os.path.basename(file_to_send),
        )

        if response and response['ok'] or response.status_code == 200:
            logger.log(message="File uploaded successfully.", to_terminal=True, log_level="info")
        else:
            logger.log(message="File upload failed.", to_terminal=True, log_level="error")

    except SlackApiError as e:
        logger.log(message="Error: " + str(e), to_terminal=False, log_level="error")
        logger.log(message="Please find error message in the logfile", to_terminal=True, log_level="info")

def send_slack_entrypoint():
    config = yaml_parser.parse_config()

    user_email = config['robin_email']
    ...
    send_slack(zip_file, customer_name=user_email)
