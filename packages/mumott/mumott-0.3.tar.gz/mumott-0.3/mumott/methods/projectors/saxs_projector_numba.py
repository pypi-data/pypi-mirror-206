...
from ...core.john_transform import john_transform, john_transform_adjoint
...
class SAXSProjectorNumba(Projector):
    ...
    def forward(self,
        ...
        """
        assert np.allclose(field.shape[:-1], self._geometry.volume_shape)
        self._update()
        if indices is None:
            return self._forward_stack(field)
        elif indices.size == len(self._geometry):
            return self._forward_stack(field)
    ...
    def adjoint(self,
        ...
        -------
            ...
            coefficients.
        """
        assert np.allclose(projections.shape[-3:-1], self._geometry.projection_shape)
        self._update()
