    ...
    if isinstance(batch_source, specs.SparkBatchSourceSpec):
        if not batch_source.supports_time_filtering:
            raise TectonValidationError(
                f"DataSource {data_source.name} does not support time filtering. Must set supports_time_filtering to True and implement time filtering within the data source function."
            )
    else:
        if not batch_source.timestamp_field:
            raise TectonValidationError(
                f"DataSource {data_source.name} does not support time filtering. Must specify the data source timestamp field."
            )
