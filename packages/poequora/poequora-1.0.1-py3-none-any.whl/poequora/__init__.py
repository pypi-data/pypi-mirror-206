...
class Account:
    ...
    @staticmethod
    def get(path: str = 'cookies.txt'):
        cookies = open(path, 'r').read().splitlines()
        return unquote(choice(cookies))
...
class Completion:
    ...
    def create(
        ...
        chunk = None
        for response in client.send_message(_model, prompt, with_chat_break, timeout):
            chunk = response
