...
class FormatExternal:
    def __init__(self, place_reference: Union[str, Path], data_name: str, write_directory: Union[Path, str],
                 qc_validation: Union[Path, str], correction_path: Optional[Union[Path, str]] = None,
                 place_order: Optional[List[int]] = None, alternate_matches: Optional[List[int]] = None,
                 splitter: str = "__"):

        # Set delimiters for complex names, the database name, and the write_directory
        self.data_name = data_name
        self._splitter = splitter
        self._qc_validation = qc_validation
        ...
        self._name_qc = FormatNames(self._splitter, self._matcher, self._write_directory, qc_validation)

    def standardise_names(self, data_directory: Union[str, Path], name_i: int, data_start_i: int, process_i: int = 0,
                          merge_ambiguity=True) -> None:
        ...
        [self._name_qc.standardise(Path(data_directory, file), name_i, data_start_i, merge_ambiguity, i, file_count)
         for i, file in enumerate(files[process_i:], 1)]

        # Write the file and log to disk
        self._name_qc.write(self.data_name)

    def standardise_name(self, name: str, year: str):
        """Standardise a single place"""
        return self._name_qc.rename_place(name, year)
