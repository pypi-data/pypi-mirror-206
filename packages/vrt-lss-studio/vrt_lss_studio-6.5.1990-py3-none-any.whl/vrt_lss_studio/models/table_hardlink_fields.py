...
class TableHardlinkFields(BaseModel):
    ...
    element_type: Optional[StrictStr] = Field('ORDER', description="Element type.")
    ...
    __properties = ["essence_key", "essence_type", "enabled", "pristine", "element_type", "elements_count", "trip_key"]
    ...
    def element_type_validate_enum(cls, v):
        if v is None:
            return v
        if v not in ('ORDER', 'PERFORMER_SHIFT', 'TRANSPORT_SHIFT'):
            raise ValueError("must be one of enum values ('ORDER', 'PERFORMER_SHIFT', 'TRANSPORT_SHIFT')")
        return v
    ...
    def from_dict(cls, obj: dict) -> TableHardlinkFields:
        ...
        _obj = TableHardlinkFields.parse_obj({
            ...
            "element_type": obj.get("element_type") if obj.get("element_type") is not None else 'ORDER',
