...
class Browser(BrowserView):
    ...
    def _end_group_memberships(self, member_var, group_var,
        ...
        if isinstance(member_ids, six_string_types) and member_ids:
            single_member = True
            member_ids = [member_ids]
        group_ids = form.get(group_var, [])
        if isinstance(group_ids, six_string_types) and group_ids:
            single_group = True
            group_ids = [group_ids]
    ...
    def can_view_unitracc_groups(self):
        ...
        if context.Creator() == member.getId():
            ...
            state = workflow_tool.getInfoFor(context, 'review_state', None)
            if state != 'published':
                return True
        ...
        try:
            ...
            with getAdapter('sqlwrapper') as sql:
                ...
                if today is None:
                    ...
                    query_data = None
                    where = None
                    # wir brauchen hier einen Wert:
                    today = datetime(*localtime()[:6])
                    logger.info('"today" for "done" field is %(today)r',
                                locals())
                else:
    ...
    def get_manageable_groups(self, group_id=None, pretty=True, forlist=None):
        ...
        if is_group_manager:
            res = list(map(ggibi, list(group_ids)))
        else:
            res = []
            for group_id in group_ids:
                dic = ggibi(group_id)
