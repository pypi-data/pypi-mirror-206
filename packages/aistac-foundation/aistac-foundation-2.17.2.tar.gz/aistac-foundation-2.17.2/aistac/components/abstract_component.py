...
class AbstractComponent(ABC):
        ...
        if reset_templates:
            ...
            if align_connectors:
                ...
                if self.pm.has_connector(self.TEMPLATE_SOURCE):
                    self.remove_connector_contract( self.TEMPLATE_SOURCE)
                if self.pm.has_connector(self.TEMPLATE_PERSIST):
                    self.remove_connector_contract( self.TEMPLATE_PERSIST)
                ...
                self.add_connector_contract(connector_name=self.TEMPLATE_SOURCE, connector_contract=scc)
                pcc = ConnectorContract(uri=template_path, module_name=template_module, handler=template_persist_handler)
                self.add_connector_contract(connector_name=self.TEMPLATE_PERSIST, connector_contract=pcc)
                # reset the connector
    ...
    def from_env(cls, task_name: str, default_save=None, reset_templates: bool=None, align_connectors: bool=None,
        ...
        if isinstance(uri_pm_repo, str):
            pm_repo, _ = str.split(uri_pm_repo)
            default_save = False
        else:
    ...
    def remove_intent(self, intent_param: [str, dict]=None, level: [int, str]=None, save: bool=None):
        ...
        if isinstance(intent_param, (str, dict)) or isinstance(level, (int, str)):
            ...
            for intent in self.pm.get_intent().keys():
