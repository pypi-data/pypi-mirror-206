...
class ConstructWeights:
    ...
    def construct_base_weights(self, write_dir: Union[str, Path], write_name: str = 'BaseWeights') -> None:
        ...
        for c, (shape, record) in enumerate(zip(self.base.polygons, self.base.records)):
            ...
            # Strip the Match, as it's not JSON serializable, then add to the base weights
            match_weights = {date: self._format_match_data(date_values) for date, date_values in match_weights.items()}
    ...
    @staticmethod
    def _format_match_data(date_values):
        """Polygons are not json serializable, so stip them when adding to base list"""
        return {gid: {place_attr: v for place_attr, v in gid_values.items() if place_attr != "Match"}
                for gid, gid_values in date_values.items()}
