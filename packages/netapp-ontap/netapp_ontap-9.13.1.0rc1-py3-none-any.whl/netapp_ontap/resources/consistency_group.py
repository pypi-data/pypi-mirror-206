...
class ConsistencyGroupSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    metric = fields.Nested("netapp_ontap.resources.consistency_group_metrics.ConsistencyGroupMetricsSchema", data_key="metric", unknown=EXCLUDE)
...
An NVMe namespace must be mapped to an NVMe subsystem to grant access to the subsystem's hosts. Hosts can then access the NVMe namespace and perform I/O using the NVMe over Fabrics protocol."""
    ...
    replication_relationships = fields.List(fields.Nested("netapp_ontap.models.consistency_group_replication_relationships.ConsistencyGroupReplicationRelationshipsSchema", unknown=EXCLUDE), data_key="replication_relationships")
...
This is the dedicated consistency group Snapshot copy policy, not an aggregation of the volume granular Snapshot copy policy."""
    ...
    statistics = fields.Nested("netapp_ontap.models.consistency_group_statistics.ConsistencyGroupStatisticsSchema", data_key="statistics", unknown=EXCLUDE)
...
The total number of volumes across all child consistency groups contained in a consistency group is constrained by the same limit."""
    ...
    """application,consistency_groups,luns,namespaces,parent_consistency_group.links,parent_consistency_group.name,parent_consistency_group.uuid,provisioning_options,qos,restore_to,snapshot_policy.name,snapshot_policy.uuid,volumes,"""
    ...
    """application,clone,consistency_groups,luns,name,namespaces,parent_consistency_group.links,parent_consistency_group.name,parent_consistency_group.uuid,provisioning_options,qos,snapshot_policy.name,snapshot_policy.uuid,svm.name,svm.uuid,tiering,volumes,"""
