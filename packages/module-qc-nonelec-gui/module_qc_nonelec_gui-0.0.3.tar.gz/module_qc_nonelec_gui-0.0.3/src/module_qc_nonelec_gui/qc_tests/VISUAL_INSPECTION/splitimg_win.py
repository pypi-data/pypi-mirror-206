...
class SplitImageWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.img_bgr_line, h, w, d = read_img(
        ...
        img_bgr_line, h_line, w_line, d_line = read_img(path_line)
        scale = min(600.0 / float(h_line), 600.0 / float(w_line))
        resize_img = cv2.resize(img_bgr_line, dsize=None, fx=scale, fy=scale)
