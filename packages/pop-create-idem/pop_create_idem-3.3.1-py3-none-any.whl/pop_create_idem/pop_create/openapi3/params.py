        ...
        for p in parameters:
            ...
            if p.get("nested_member", None):
                ...
                if nested_params.get("primitive", None):
                    ...
                    param_type = hub.pop_create.openapi3.params.type(p.get("type"))
                    nested_member = {
...
def parse_response_mappings(hub, schema_ref: str, all_schemas: dict) -> Dict[str, str]:
    ...
    parameters: list = all_schemas.get(schema_ref)
    if parameters:
        # e.g. a get pet request would get the following response mapping
        # {
        #     "id": "id",
        #     "category": "category",
        #     "name": "name",
        ...
        for p in parameters:
            if "idem_name" in p and p.get("idem_name"):
                response_mapping[p.get("idem_name")] = p.get("actual_name", "")
