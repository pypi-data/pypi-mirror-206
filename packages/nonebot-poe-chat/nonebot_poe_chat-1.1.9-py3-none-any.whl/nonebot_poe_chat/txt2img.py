...
class Txt2Img:
    ...
    async def draw_img(self, title: str, text: str, template: Union[str, dict] = "mi") -> Image.Image:
        """异步绘制给定模板模板下的图片"""
    ...
    async def draw(self, title: str, text: str, template: Union[str, dict] = "mi") -> Union[str,str]:
        """返回一个base64的图片和一个链接"""
        self.raw_text = text
        out_img,url = await self.draw_img(title, text, template)
        return await img2b64(out_img),url
    ...
    async def draw_text(self, title: str, text: str) -> Image.Image:
        ...
        text_height = self.text_font_size * (text_rows+1) + (text_rows - 1) * self.text_line_space
        if title:
