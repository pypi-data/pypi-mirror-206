...
# def get_process_uid(pid: int) -> int:
#     """
#     Returns the uid of the process with the given identifier.

#     Raises 'NotImplementedError' if running in a non-Linux platform as it relies on the Linux proc filesystem.
#     Raises 'FileNotFoundError' if no such process.
#     Raises 'PermissionError' if there is no sufficient permissions.

#     :return: The uid of the process with the given pid
#     """
#     if platform.system() != "Linux":
#         # TODO(raz): support this with psutil.Process().eniron() probably
#         raise NotImplementedError(
...
#                 real, effective, saved, fs = (int(_) for _ in line.split()[1:])
#                 return real

#     raise RuntimeError(f"Can't query uid of process {pid}")


# def get_uids_to_usernames() -> Dict[int, str]:
#     """
#     Parses /etc/passwd and returns a mapping between uids and usernames.
#     """
...
def terminate(pid: int) -> None:
    """
    Terminates the running process with the given pid by sending the signal SIGKILL to it.
