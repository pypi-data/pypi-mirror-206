...
def _write_sp_from_poly_or_circle(cygnss_l1_path: Optional[str], date_list: Union[list[dt.date], np.ndarray],
    ...
    if save_below_thresh:
        ...
        df_low = _export_sp_points(pnt_list_low, klm_low_path, lyr_options, save_klm, low_hex_color, save_csv)
    else:
        df_low = None
    df_high = _export_sp_points(pnt_list_high, klm_high_path, lyr_options, save_klm, sp_high_color_hex, save_csv)
...
def _export_sp_points(pnt_list: list[dict], klm_out_file_path: str, lyr_options, save_klm: bool, sp_color_hex: str, save_csv: bool) -> Optional[
    pd.DataFrame]:
    df_data = None
    if not pnt_list:
