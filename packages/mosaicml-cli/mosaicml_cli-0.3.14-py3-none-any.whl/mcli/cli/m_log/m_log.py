...
from mcli.sdk import Run, delete_runs, follow_run_logs, get_inference_deployment_logs, get_run_logs
...
def _get_run_logs(run: Run, follow: bool, rank: Optional[int], failed: bool,
                  conf: MCLIConfig) -> Tuple[Optional[str], int]:
    ...
    **kwargs,
    ...
    try:
        ...
        if isinstance(submissions[0], Run):
            last_line, err = _get_run_logs(submissions[0], follow, rank, failed, conf)
            if err == 1:
                return 1
