...
from bigeye_sdk.model.dbt_schema import SimpleDbtSchema, SimpleDbtColumn, relationships_test_names, SimpleDbtModel
from bigeye_sdk.model.protobuf_enum_facade import MetricStatus, SimpleLookbackType

from bigeye_sdk.log import get_logger
from bigeye_cli.functions import write_metric_info, cli_client_factory, write_debug_queries
from bigeye_sdk.generated.com.bigeye.models.generated import MetricInfoList, MetricInfo, TableList, Table, \
    MetricConfiguration
from bigeye_sdk.model.metric_facade import SimpleUpsertMetricRequest
from bigeye_sdk.model.protobuf_message_facade import SimpleAutoThreshold, SimpleLookback, SimpleMetricDefinition
...
def dbt_tests_to_metrics(
        ...
        warehouse_name: str = typer.Option(
            ...
            , "--warehouse_name"
            , "-wn"
            , help="The name of the source to deploy metrics"),
        schema_name: str = typer.Option(
            ...
            , "--schema_name"
            , "-sn"
            , help="The name of the schema to deploy metrics"),
        schema_file: str = typer.Option(
            ...
            , "--schema_file"
            , "-sf"
            , help="The path to the dbt schema file"),
        use_autothresholds: bool = typer.Option(
            False
            , "--use_auto"
            , "-auto"
            , help="Use autothresholds over default constant"),
):
    """Convert tests in a dbt schema.yml to metrics in Bigeye"""
    dbt_schema = SimpleDbtSchema.parse_file(path=schema_file)
    client = cli_client_factory(bigeye_conf)
    source = client.get_sources_by_name(source_names=[warehouse_name])[warehouse_name]
    dbt_tables: Dict[str, List[SimpleMetricDefinition]] = {t.name: t.model_tests for t in dbt_schema.models}
    tables: Dict[str, Table] = {t.name.lower(): t for t in
                                client.get_tables(warehouse_id=[source.id], schema=[schema_name]).tables if
                                t.name.lower() in dbt_tables.keys()}

    default_lookback_type: bool = [ac.boolean_value for ac in client.get_advanced_configs()
    ...
    for t, tests in dbt_tables.items():
        try:
            ...
            log.error(f"Table {t} was not found in {source.name}.{schema_name}. Check that Bigeye has access to the dataset.")
            continue
            ...
            if bigeye_table.metric_time_column:
                ...
                try:
                    if test.metric_name == "dbt_test_relationships":
                        template = [t for t in client.get_all_metric_templates()
                                    if t.source.name.lower() == warehouse_name.lower() and
                                    t.name.lower() in relationships_test_names][0]
                        test.metric_type.template_id = template.id
                        for p in test.parameters:
                            if p.key == "lookup_table":
                                sdb = schema_name.split(".")
                                p.string_value = f'"{sdb[0]}"."{p.string_value}"' if len(sdb) == 1 \
                                    else f'"{sdb[0]}"."{sdb[1]}"."{p.string_value}"'
                except IndexError:
                    log.error(
                        f"No template found in source {warehouse_name} with any of the names: {relationships_test_names}")
                ...
                log.info(f"Creating metric {test.metric_name} for table {source.name}.{schema_name}.{t}")
                client.upsert_metric(metric_configuration=mc)
