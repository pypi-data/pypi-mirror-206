...
from .utils import APPNAMES, DEFAULTSECTION
...
@click.command()
@click.argument('appname', metavar="APPNAME", type=click.Choice(APPNAMES), required=False)
@click.argument('prjname', required=False)
@click.option('--ini-file', type=click.File('r+'), required=False,
...
@click.pass_context
def startsite(ctx, appname, prjname, ini_file, batch, dev_repos, shared_env,
    ...
    if ini_file:
        ...
        try:
            ...
            try:
                ...
                front_end = None
            try:
                use_django_settings = main_cfg.get('getlino', 'use_django_settings')
            except NoOptionError as e:
                use_django_settings = False
        except (NoSectionError, NoOptionError) as e:
            with open(Path(__file__).parent / 'templates' / 'lino.ini', 'r') as f:
                click.echo(
                    ("INI must contain a 'getlino' section (with optional 'front_end').\nSee DEFAULT ini format: \n\n"
                     + f.read()), color=ctx.color)
            ctx.exit()
    ...
    if ini_file:
        ...
        def clone_pull_install_repo(repo_nn, repo_remote, settings_module=None):
            clonable = True
            ...
            if clonable:
                ...
                for f in ['setup.py', 'pyproject.toml']:
                    if os.path.exists(join(project_dir, repo_nn, f)):
                        shutil.move(join(project_dir, repo_nn), join(get_dev_repos_dir(), repo_nn))
                        change_directory_into_dev_repos()
                        i.run_in_env(envdir, "pip install -q -e {}".format(repo_nn))
                        break
            ...
            if os.path.exists(join(project_dir, repo_nn)):
                ...
                if settings_module is not None:
                    return "{}.{}.{}.{}".format(local_prefix, prjname, repo_nickname, settings_module)
        ...
        if front_end and isinstance(front_end, str):
            ...
            if all((u.scheme, u.netloc)):
                ...
                clone_pull_install_repo(front_end.split('.git')[0].split('/')[-1], front_end)
    ...
    if asgi_server:
        if reinstall:
            ...
            if cfgp.has_section('getlino_asgi') and cfgp.has_option('getlino_asgi', 'upstream_port'):
                try:
                    upstream_port = cfgp.getint('getlino_asgi', 'upstream_port')
                ...
                if p != prjname:
                    ...
                    if os.path.isdir(p):
                        ...
                        if os.path.exists(lini):
                            cfgp.read(lini)
                            try:
                                up = cfgp.getint('getlino_asgi', 'upstream_port')
                                used_ports.add(up)
                            except (NoSectionError, NoOptionError) as e:
                                pass
                            except ValueError as e:
