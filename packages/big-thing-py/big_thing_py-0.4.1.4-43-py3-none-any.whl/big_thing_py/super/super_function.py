...
class MXSuperFunction(MXFunction):
    ...
    def _request_subschedule(self, subschedule_request: MXSubScheduleRequest) -> None:
        ...
        MXLOG_DEBUG(
            f'[SUB_SCHEDULE {subschedule_msg._status.value.upper()} START] {subschedule_msg.sub_service_name}|{subschedule_msg.target_thing_name}|{subschedule_msg.target_middleware_name}', 'cyan')
    ...
    def _super_schedule_wrapper(self, super_schedule_request: MXSuperScheduleRequest, hierarchical_service_table: Dict[str, List[MXService]], timeout: float = 10):
        ...
        MXLOG_DEBUG(
            f'[SUPER_SCHEDULE START] {self._name} by scenario {scenario}.', 'green')
        ...
        try:
            ...
            MXLOG_DEBUG(
                f'[SUPER_SCHEDULE END] {self._name} by scenario {scenario}. duration: {super_schedule_request.duration():.4f} Sec', 'green')
    ...
    def _super_execute_wrapper(self, super_execute_request: MXSuperExecuteRequest):
        ...
        try:
            ...
            MXLOG_DEBUG(f'[SUPER_EXECUTE TIMEOUT] {super_execute_request._action_type.value} of {self._name} \
                    by scenario {requester_middleware_name}|{scenario} timeout...', 'red')
            ...
            print_error(e)
            MXLOG_DEBUG(f'[SUB_EXECUTE FAILED] {super_execute_request._action_type.value} of {self._name} \
                by scenario {requester_middleware_name}|{scenario} failed...', 'red')
            ...
            MXLOG_DEBUG(
                f'[SUPER_EXECUTE END] {self._name} by scenario {scenario}. duration: {super_execute_request.duration():.4f} Sec', 'green')
