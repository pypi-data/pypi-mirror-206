...
from kelvin.sdk.datatype.base_messages import Recommendation
from kelvin.sdk.datatype.krn import KRNAssetMetric, KRNWorkload
...
class Connection:
    ...
    def _make_messages(self, topic: str, payload: bytes) -> List[Message]:
        ...
        if not isinstance(message.resource, KRNAssetMetric):
            logger.error("Expected to receive messages with asset/metric resource")
            return []
        ...
        asset_name = message.resource.asset
        name = message.resource.metric
        data_type = message.type.icd
    ...
    def _make_payloads(self, message: Message) -> List[MessagePayloadTuple]:
        ...
        if not isinstance(message.resource, KRNAssetMetric):
            logger.error("Expected to emit message with asset/metric resource")
            return []
        ...
        for item in items:
            ...
            if output_name != name:
                message_ = message.copy(deep=True)
                cast(KRNAssetMetric, message_.resource).metric = output_name
