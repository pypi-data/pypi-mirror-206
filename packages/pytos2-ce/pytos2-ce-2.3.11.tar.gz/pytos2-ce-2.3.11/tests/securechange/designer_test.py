...
class TestNewRuleAnyService:
    ...
    def test_any_service(self, rule):
        ...
        assert isinstance(rule.services[0], IPService)
...
class TestUpdateRuleInstruction:
    ...
    def test_attributes(self, instruction):
        ...
        assert instruction.source_objects is instruction.sources
...
def services_mock(st, st_devices_mock):
    responses.add(
        ...
        json=json.load(open(f"tests/securetrack/json/services/1.json")),
    )
