...
class Blackwood(Hand):
    ...
    def __init__(self, hand_cards, board):
        ...
        # print('b', self.bid_history)
        # print('l', self.responders_last_bid)
        self.agreed_suit = self._get_agreed_suit()
        index = 0
        # for call in self.bid_history[::4]:
        #     if len(self.bid_history) >= index+3:
        #         print(f'{call:<4} {self.bid_history[index + 2]:<4}')
        #         index += 4
        #     else:
        #         print(f'{call:<4}')
        # print('a', self.agreed_suit)
        if 0:
            xxx = input('-->')
            if xxx == 'q':
                quit()
    ...
    def _get_agreed_suit(self):
        ...
        opener_index = 0
        for opener_call in calls[::4]:
            responder_index = 2
            for responder_call in calls[2::4]:
                if responder_call.name == '4NT':
                    if (self.opener_bid_one.is_major and
                            self.suit_holding[self.opener_bid_one.denomination] >= 4):
                        return self.opener_bid_one.denomination
