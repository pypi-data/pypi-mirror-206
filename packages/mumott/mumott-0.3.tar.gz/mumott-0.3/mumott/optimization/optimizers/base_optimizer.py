...
class Optimizer(ABC):
    ...
    def __str__(self) -> str:
        ...
        with np.printoptions(threshold=4, precision=5, linewidth=60, edgeitems=1):
            s += ['{:18} : {}'.format('ObjectiveFunction', self._objective_function.__class__.__name__)]
    ...
    def _repr_html_(self) -> str:
        ...
        with np.printoptions(threshold=4, edgeitems=2, precision=2, linewidth=40):
            s += ['<tr><td style="text-align: left;">ObjectiveFunction</td>']
            s += [f'<td>{1}</td><td>{self._objective_function.__class__.__name__}</td></tr>']
            h = hex(hash(self))
