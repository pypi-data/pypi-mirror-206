...
from oblv.exceptions import AuthenticationError, BadRequestError, HTTPClientError, ParamValidationError
...
from oblv.auth import authenticate
...
class TestAuth(unittest.TestCase):
    ...
    def getInvalidCredentialResponse():
        res = Response(401, json={KEY_MESSAGE: BAD_REQUEST_MESSAGE})
        return res

    def getSuccessResponse():
        res = Response(200, content='{"token": "'+USER_TOKEN+'", "user_id": "'+USER_ID+'"}'
        )
        return res
    ...
    def test_success_request(self, sync):
        ...
        self.assertEqual(authenticate(self.random_api_key), client)
