...
def create_orchestrated_domain_bundle(args):
    cli_argument_dict = domain_orchestration.create_argument_dict(args)
    odb = domain_orchestration.OrchestratedDomainBundle(cli_argument_dict).as_json()
    _store_odb(odb, args.output)


def serialize_odb(odb):
    zipped_odb = zlib.compress(json.dumps(odb))
    return base64.b64encode(zipped_odb).decode()


def _store_odb(odb, output_dir):
    print(f"storing orchestrated domain bundle in file '{output_dir}'")
    file = open(output_dir, 'w')
    file.write(serialize_odb(odb))
    file.close()
