...
class OnionQueue:
    ...
    def __threading_target(self, session: OnionSession):
        ...
        try:
            ...
            while self.__is_running:  # Only check self.__is_running to see if the loop should be running.
                ...
                url = task_data["url"]
                args = task_data["res_args"]
                kwargs = task_data["res_kwargs"]
    ...
    def add_task(
        ...
        :param method: Either "GET" or "POST" to indicate which to use during the request. Default is "GET".
        :param res_args: A list of arguments to pass to the session's "get" or "post" method.
        :param res_kwargs: A dictionary of keyword arguments to pass to the session's "get" or "post" method.
        ...
        Note about the args/kwargs arguments:
            "res_args" and "res_kwargs" are used during the request "GET" or "POST". For example, if you
            ...
            Here's some example code to further describe the differences:
                with session.get(url, *res_args, **res_kwargs) as res:
        ...
        assert isinstance(res_args, list) or isinstance(res_args, tuple)
        assert isinstance(res_kwargs, dict)
    ...
    def is_running(self) -> bool:
        if self.__is_running:
            ...
            for key in self.__threads_running:
