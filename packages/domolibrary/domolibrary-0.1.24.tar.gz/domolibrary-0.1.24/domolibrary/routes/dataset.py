...
class UploadDataError(de.DomoError):
    ...
    def __init__(self, stage_num: int, dataset_id: str, status, message , domo_instance: str):
        ...
        super().__init__(entity_id = dataset_id , message = message, status = status, domo_instance = domo_instance)
...
async def upload_dataset_stage_1(
    ...
    if not res.is_success:
        raise UploadDataError(
            stage_num=1, dataset_id=dataset_id, domo_instance=auth.domo_instance, status = res.status, message = status.message
        )
...
async def upload_dataset_stage_2_file(
    ...
    if not res.is_success:
        raise UploadDataError(
            stage_num=2, dataset_id=dataset_id, domo_instance=auth.domo_instance, status = res.status, message = res.message
        )
...
async def upload_dataset_stage_3(
    ...
    if not res.is_success:
        raise UploadDataError(
            stage_num=3, dataset_id=dataset_id, domo_instance=auth.domo_instance, status=res.status, message=res.message
        )
