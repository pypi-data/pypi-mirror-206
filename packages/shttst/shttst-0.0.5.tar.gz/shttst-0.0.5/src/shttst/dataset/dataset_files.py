import os
import shutil
from shttst.files import destroy_file, get_files, move_to_parent
...
def check_dataset(metadata_path: str, delete_missing = False):
    ...
    # lowered = [m.name.lower() for m in transcriptions]
    # transcriptions = [z for (i,z) in enumerate(transcriptions) if lowered.index(z.name.lower()) == i]
