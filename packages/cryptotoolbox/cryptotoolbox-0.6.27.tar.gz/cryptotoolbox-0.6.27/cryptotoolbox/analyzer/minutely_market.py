...
class MarketAnalyzer:
    ...
    parser_metrics = {
        'sharpe': {'f': metrics.sharpe, 'p': {'period': 252*24*60}},
        'calmar': {'f': metrics.calmar, 'p': {'period': 252*24*60}},
        'annual_return': {'f': metrics.annual_return, 'p': {'period': 252*24*60}},
        'annual_volatility': {'f': metrics.annual_volatility, 'p': {'period': 252*24*60}},
        'mdd': {'f': metrics.mdd, 'p': {}},
    }
    metrics = []
    m = 0
    ...
    def _set_hours_per_year(self):
        ...
        self.n_hours_per_year = 252*24*60
        # t = self.df.shape[0]
    ...
    def get_kpi(self, drop_duplicate=True):
        ...
        if drop_duplicate:
            ...
            #_, indices = np.unique(results, axis=1, return_index=True)
    ...
    def get_corr(self, window):
        ...
        correlation_df = roll_corr(df, window=window).loc[self.idx, :].dropna()
