...
class Group:
    ...
    def __init__(self, name, columns=None, wide=True, style='section2',
      ...
      cellspacing=1, cellgap='0.6em', label=None, translated=None):
        ...
        # p_css, if specified, is a CSS class that will be applied to the whole
        # group.
        self.css = css
    ...
    def setColumns(self, columns):
        '''Sets these p_columns instead of the current ones'''
        self.columns = columns
        self._setColumns(self.style)
...
class UiGroup:
    ...
    pxHelp = Px(lambda c: '<abbr title="%s"><img class="iconS" src="%s"/>' \
                    '</abbr>' % (c._('help', field=c.field), c.svg('help')))

    # Standard CSS class to use, per group style and layout. If a style is not
    # in this dict, it means that no deffault CSS class must be applied to its
    # main tag.
    byStyle = {'grid': {'edit': 'gridG gridE', None: 'gridG gridV'}}
    ...
    pxGrid = Px('''
     ...
     css='''
      .gridG>tbody>tr { vertical-align:top }
      .gridG>tbody>tr:nth-child(odd) { background-color:|oddColor| }
      .gridG>tbody>tr:nth-child(even) { background-color:|evenColor| }
    ...
    pxViewSearches = Px('''
     <x var="collapse=field.getCollapseInfo(field.labelId, req)">
      ...
      <!-- Group content -->
      <div id=":collapse.id" class="portletGC" style=":collapse.style">
    ...
    def getCss(self, layout):
        '''Returns the CSS class(es) to apply to p_self's main tag'''
        r = []
        # Get a potential CSS class depending on p_self's style and p_layout
        css = self.byStyle.get(self.style)
        if css:
            css = css.get(layout) or css.get(None)
            if css:
                r.append(css)
