...
class Browser(GroupsBase):
    ...
    def replyThread(self):
        ...
        Ursache noch unbekannt! Vorerst protokollieren wir das und führen das
        Template thread-reply.pt in solchen Fällen nicht mehr aus.
        """
        context = self.context
        request = context.REQUEST
        handle_falsy_tid = 0
        tid = request.get('tid', None)
        if not tid:
            if tid in ('', None):
    ...
    def edit_thread(self):
        ...
        url = ('%(desktop_path)s/@@groupboard/replyThread?tid=%(tid)s'
               % locals())
        return context.REQUEST.RESPONSE.redirect(url)
