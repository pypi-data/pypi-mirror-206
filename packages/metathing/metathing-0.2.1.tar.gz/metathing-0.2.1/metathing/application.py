...
class Application():
    ...
    @staticmethod
    def _print_frame(content):
        frame_width = len(max(content, key=len)) + 4
        frame = '+' + '-' * (frame_width) + '+'
        logger.info(frame)
        for line in content:
            spaces_before = int((frame_width - len(line)) / 2)
            spaces_after = frame_width - len(line) - spaces_before
            logger.info('|' + ' ' * spaces_before + line + ' ' * spaces_after + '|')
        logger.info(frame)
