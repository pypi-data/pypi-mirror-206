...
if run_as_module:
    ...
    file = f"aurora{sys.argv[6]}.log" if len(sys.argv) > 6 else "aurora.log"

    if os.path.exists(file):
        os.remove(file)

    HOSTED_ON = where_hosted()
    LOGS = getLogger("Aurora Logs")
    ...
    LOGS.info(f"Auroralibs Version - {__Ayra__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Aurora Version - {ayra_version} [{HOSTED_ON}]")
