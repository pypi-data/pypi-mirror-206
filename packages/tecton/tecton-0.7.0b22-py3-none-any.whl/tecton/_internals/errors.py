...
def FV_NEEDS_TO_BE_BATCH_TRIGGER_MANUAL(fv_name):
    return TectonValidationError(f"batch_trigger of Feature View {fv_name} needs to be BatchTriggerType.MANUAL")
...
def FV_INVALID_MOCK_SOURCES(mock_sources_keys: List[str], fv_params: List[str]):
    return TectonValidationError(
        f"Mock sources' kwargs {mock_sources_keys} do not match the Feature View's input parameters {fv_params}"
    )


def FV_INVALID_MOCK_INPUTS(mock_inputs: List[str], inputs: List[str]):
    return TectonValidationError(f"Mock input kwargs {mock_inputs} do not match FeatureView's inputs {inputs}")
