...
def get_artefacts_path_from_registry_name(registry_name: str, list_registries: List[dict]):
    for registry in list_registries:
        if registry["registry_name"] == registry_name:
            return registry["artefacts_path"]
...
def get_model_artefact(registry_name: str, artefact_path: str, registries_inputs: List[dict], client):
    try:
        artefacts_path = get_artefacts_path_from_registry_name(registry_name, registries_inputs)
        object_storage_client = ObjectStorageClient({}, client)
        return object_storage_client.read_from_bucket_to_file("job-lib", artefacts_path + "/" + artefact_path)
    except Exception as ex:
        raise Exception("Not able to read the artefact {artefact_path} of registry {registry_name} / " + str(ex))


def save_model_artefact(registry_name: str, artefact_path: str, data, registries_inputs: List[dict], client):
    try:
        artefacts_path = get_artefacts_path_from_registry_name(registry_name, registries_inputs)
        object_storage_client = ObjectStorageClient({}, client)
        object_storage_client.write_file_in_bucket("job-lib", artefacts_path + "/" + artefact_path, data)
    except Exception as ex:
