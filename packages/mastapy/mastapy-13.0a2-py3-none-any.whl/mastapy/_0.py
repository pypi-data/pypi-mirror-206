...
class APIBase(_7515.MarshalByRefObjectPermanent):
    """APIBase
    ...
    class _Cast_APIBase:
        ...
        def __init__(self, parent: 'APIBase'):
            self._parent = parent

        @property
        def api_base(self) -> 'APIBase':
            return self._parent
    ...
    def get_property(self, name: 'str') -> 'APIBase.T_get_property':
        ...
        return constructor.new_from_mastapy_type(APIBase.T_get_property)(method_result) if method_result is not None else None
    ...
    def cast(self, type_):
        ...
        """
        if not self.is_of_type(type_):
            raise CastException('Could not cast {} to type {}. Is it a mastapy type?'.format(type(self), type_)) from None
    ...
    @property
    def cast_to(self) -> 'APIBase._Cast_APIBase':
