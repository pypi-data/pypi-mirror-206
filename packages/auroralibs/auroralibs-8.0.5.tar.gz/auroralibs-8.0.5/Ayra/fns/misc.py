...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Aurora All Cmds", content=[txt])
    await eor(event, f"All Aurora Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in ayra_bot._cache:
        return ayra_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            ayra_bot._cache.update({"insta_creds": cl})
