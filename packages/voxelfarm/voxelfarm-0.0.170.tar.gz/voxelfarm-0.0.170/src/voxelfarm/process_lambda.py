...
class process_lambda_host:
    ...
    def create_report(self, folder, name, report_lambda, region, lod, inputs, fields = None, update_type = None):
        ...
        if update_type:
            fields['callback_update_type'] = self.entity_id + "/" + update_type
    ...
    def set_lambda_definition(self, on_init, on_stage_done):
        ...
        if update_type:
            if on_stage_done:
                ...
                result = on_stage_done(self.vf_api, self.lambda_framework, update_type)
                if result:
            ...
            if on_init:
                result = on_init(self.vf_api, self.lambda_framework)
                if result:
                    if 'success' in result and result['success']:
                        if 'complete' in result and result['complete']:
