...
class SIGTT(Functional):
    """Method for SIGTT, or Spherical Integral Geometric Tensor Tomography.
    This ``Method`` is an appropriate choice for SAXS tensor tomography, as it relies
    ...
    def __init__(self,
        ...
        self._coefficients = np.zeros((*self._data_container.geometry.volume_shape, len(self._basis_set)))
        self._integration_samples = integration_samples
        # Check if full circle appears covered in data or not.
        delta = np.abs(self._data_container.geometry.detector_angles[0] -
                       self._data_container.geometry.detector_angles[-1] % (2 * np.pi))
    ...
    def get_residuals(self,
        ...
        residuals = projection - self._data_container.stack.data
        if get_gradient:
            ...
            if get_weights:
                gradient = self._projector.adjoint(
                    self._basis_set.gradient(residuals * self._data_container.weights))
            else:
                gradient = self._projector.adjoint(
                    self._basis_set.gradient(residuals))
        else:
            gradient = None

        if get_weights:
            weights = self._data_container.weights
        else:
            weights = None
    ...
    def _get_probed_coordinates(self) -> NDArray:
        ...
        n_seg = len(self._data_container.geometry.detector_angles)
        ...
        if not self._full_circle_covered:
            ...
            shift = 0
        det_bin_middles_extended = np.copy(self._data_container.geometry.detector_angles)
    ...
    def coefficients(self, val: NDArray) -> None:
        self._coefficients = val.reshape(self._coefficients.shape)
