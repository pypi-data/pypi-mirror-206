...
def check_file(directory, *, fmt=None, do_iter=True) -> (List, Dict):
    # error checking
...
def dir_info(
    ...
    if fmt == IMG_FORMAT + LABEL_FORMAT + VIDEO_FORMAT:
        image_list, image_dict = check_file(directory, fmt=IMG_FORMAT, do_iter=do_iter)
        label_list, label_dict = check_file(directory, fmt=LABEL_FORMAT, do_iter=do_iter)
        video_list, video_dict = check_file(directory, fmt=VIDEO_FORMAT, do_iter=do_iter)
    else:
        f_list, f_dict = check_file(directory, fmt=fmt, do_iter=do_iter)
...
def run_dir_info(args: DictConfig):
    ...
    with CONSOLE.status("[bold green]Working on Directory INFO...") as status:
        dir_info(
            directory=args.dir,
            fmt=args.fmt,
            do_iter=not args.non_recursive,
            title='DIR INFO',
