...
from pycollimator.model_builder.id import Id


def _is_literal(x):
    return isinstance(x, (numbers.Number, np.generic, np.array))
...
class ModelBuilder:
    ...
    @Recorder.record_call
    def add_diagram(self, node, submodel):
        self.submodels[node] = submodel
    ...
    def add_link(self, src, dst):
        if isinstance(src, Node):
            ...
            raise TypeError(f"Expected src is OPort for add_link; got {src}")

        if isinstance(dst, Node):
            dst = dst.input_port()
        elif not isinstance(dst, IPort):
            raise TypeError(f"Expected dst is IPort for add_link; got {dst}")

        if dst in self.links:
            raise TypeError(f"Link {dst} is already connected to {self.links[dst]}")
        self.links[dst] = Link(src=src, dst=dst)
...
def _get_model_from_input(i):
    if isinstance(i, (Node, OPort)):
        return i.model
    else:
        return None
...
class Node:
    def __init__(self, model, input_names=None, output_names=None, name=None, **params):
        self.name = name
        self.pos = None
        self.align = None
        self.model = None
    ...
    def input_port(self, i=None):
        ...
        if i is None:
            if len(self.input_names) == 1:
                ...
                if i is None:
                    raise ValueError(f"Expected a single input port for {self.typename}")
        elif type(i) is int:
    ...
    def output_port(self, i=None):
        ...
        if i is None:
            if len(self.output_names) == 1:
                ...
                if i is None:
                    raise ValueError(f"Expected a single output port for {self.typename}")
        elif type(i) is int:
...
@Id.with_id
class IPort(namedtuple("IPort", ["node", "name"])):
    """
...
@Id.with_id
class OPort(namedtuple("OPort", ["node", "name"])):
    """
    ...
    def __mul__(self, other):
        if isinstance(other, (numbers.Number, np.generic, np.array)):
            import core

            return core.Gain(in_0=self, gain=other)
        else:
            import core

            return core.Product(in_0=self, in_1=other)
...
@Id.with_id
class Link(namedtuple("Link", ["src", "dst"])):
    pass
