...
class Catalog(PersistentMapping):
    ...
    @classmethod
    def populate(class_, root, handler, populate, forceLog=False):
    ...
    def getSorted(self, r, sortBy, sortOrder, maxResults=None):
        ...
        # Get sorted results
        res = Sorter(sortIndex, r, reverse=sortOrder=='desc').run()
        # Truncate the result if required
        if res and maxResults is not None:
            res = res[:maxResults]
        return res
