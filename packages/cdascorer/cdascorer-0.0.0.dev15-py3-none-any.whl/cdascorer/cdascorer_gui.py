...
Number of columns:
        ...
        '''
        # Adding an entry box to give the number of CDAs on the leaf so that the images can be combined and randomised.
        self.count_info = tk.Label(self.left_frame, text="""
Number of CDAs per leaf:
        """, justify=tk.CENTER, font=("Arial", _scale_val(30, self.scale)), height=1)
        self.count_info.place(relx=0.5, rely=0.9), anchor=tk.S)


        self.count_input = tk.Entry(self.left_frame, font=("Arial", _scale_val(30, self.scale)))
        self.count_input.place(relx=0.5, rely=1.0, anchor=tk.S)
        '''
    ...
    def _get_entry_values(self):
        ...
        #self.num_spots = self.col_input.get()
        ...
        elif self.num_spots.isdigit() == FALSE:
