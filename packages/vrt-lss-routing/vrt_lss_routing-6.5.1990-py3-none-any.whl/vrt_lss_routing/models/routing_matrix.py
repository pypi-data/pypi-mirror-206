...
class RoutingMatrix(BaseModel):
    ...
    """
    waypoints: conlist(Waypoint, max_items=15001, min_items=2) = Field(..., description="Array of geographical points with distances and times calculated between them.")
    distances: conlist(conlist(conint(strict=True, le=10000000, ge=-1), max_items=15001, min_items=2), max_items=15001, min_items=2) = Field(..., description="Routes length between two neighbors locations, in meters.         The values in the array are ordered according to the elements order in the `waypoints` parameter. Each matrix row is an array of distances from the desired point to every other point. ")
    durations: conlist(conlist(conint(strict=True, le=10000000, ge=-1), max_items=15001, min_items=2), max_items=15001, min_items=2) = Field(..., description="Array of route durations between two neighbors locations, in seconds. The values in the array are ordered according to the elements order in the `waypoints` parameter. Each matrix row is an array of distances from the desired point to every other point. ")
