...
class tilt_series_alignment():
    ...
    def filter_adjust(self):
        ...
        h_ratio = pyw.FloatText(value=0.7, description="height ratio for X-correlation", style=st)
        w_ratio = pyw.FloatText(value=0.7, description="width ratio for X-correlation", style=st)
    ...
    def interact_widget(self, snum, xp, yp, w, h, lc, hc, 
        ...
        iyl, ixl, xcorr_l = template_match(uint_f1, uint_f0, hr=hrr, wr=wrr, verbose=False)
        min_v1, max_v1, min_l1, max_l1 = cv2.minMaxLoc(xcorr_l)

        iyr, ixr, xcorr_r = template_match(uint_f1, uint_f2, hr=hrr, wr=wrr, verbose=False)
        min_v2, max_v2, min_l2, max_l2 = cv2.minMaxLoc(xcorr_r)
        ...
        self.ax7.imshow(xcorr_l, cmap="gray")
        self.ax8.imshow(xcorr_r, cmap="gray")
    ...
    def apply_alignment(self):
        ...
        print(sch_a)
        hrr = widget_values["hrr"]
        print(hrr)
        wrr = widget_values["wrr"]
        print(wrr)
