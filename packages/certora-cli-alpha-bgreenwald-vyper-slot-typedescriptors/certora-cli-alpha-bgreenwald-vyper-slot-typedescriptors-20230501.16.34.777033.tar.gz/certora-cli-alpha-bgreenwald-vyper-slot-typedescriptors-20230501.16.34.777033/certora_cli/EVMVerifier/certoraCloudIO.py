...
class CloudVerification:
    ...
    def __set_url(self, url_attr: str, index: str, user_id: int, anonymousKey: str,
        ...
        if self.context.domain == "":
            ...
            url = f"{self.context.domain}/{index}/{user_id}/{self.runName}{resource_req}?anonymousKey={anonymousKey}"
    ...
    def cli_verify_and_report(self, cl_args: str, send_only: bool = False) -> bool:
        ...
        job_list = JobList()
        job_list.add_job(
            self.runName, self.reportUrl, self.msg, self.context.domain, str(self.userId), self.anonymousKey)
    ...
    def verification_request(self, auth_data: Dict[str, Any]) -> Optional[Response]:
        verify_url = self.context.domain + "/cli/verify"
        response = None
