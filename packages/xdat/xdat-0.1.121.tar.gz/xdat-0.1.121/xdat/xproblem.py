...
def train_cv(df, target_col, clf, n_splits=12, stratify_on=None, group_on=None, ordered_split=False, del_cols=tuple(), feature_cols=tuple(), uid_col=None, sample_weight_col=None, eval_size=0.0, framework='auto', fit_params=None, target_manipulation=None, with_tqdm=True, as_numpy=False, n_jobs=1):
    # fix feature names...
    ...
    def calc_fold(fold_num, split_data: CVSplit):
        ...
        if hasattr(clf_fold, "predict_proba"):
            prob_1 = clf_fold.predict_proba(X_train)
            if prob_1.shape[1] == 1:
                prob_1 = prob_1[:, 0]
            else:
                prob_1 = prob_1[:, 1]
...
class ModelEnsemble:
    ...
    def predict_confidence(self, X, n_sim=1000):
