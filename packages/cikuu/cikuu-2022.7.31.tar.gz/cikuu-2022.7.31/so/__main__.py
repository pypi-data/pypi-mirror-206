...
def add(infile, idxname="testdoc", taglist:str=None):
	...
	requests.es.index(index=idxname, body={"did": did, "doc":text,  "filename": infile, 'type':'doc', 'tags':[] if taglist is None else taglist.strip().split(',') if isinstance(taglist, str) else taglist }, id = did)
