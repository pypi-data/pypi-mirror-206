    ...
    def handle(self, *args, **options):
        ...
        if log_sock_file.exists():
            raise Exception(
                ...
                "It's probable that a worker process is already running. Try: 'ps awx | grep linod' OR 'sudo supervisorctl status | grep worker'\n"
                "Or the last instance of the worker process did not finish properly. In that case remove log_sock file and run this command again.")
