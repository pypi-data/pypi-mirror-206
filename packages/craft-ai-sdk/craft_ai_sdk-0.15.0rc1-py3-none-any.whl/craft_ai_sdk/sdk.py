...
class CraftAiSdk:
    ...
    _version = "0.15.0rc1"  # Would be better to share it somewhere
    ...
    @log_func_result("Pipeline execution startup")
    def run_pipeline(self, pipeline_name, inputs=None):
        """Run a pipeline.
...
value is not a string or bytes"
                ...
                data["_non_file_inputs"][input_name] = input_value
        data["_non_file_inputs"] = json.dumps(data["_non_file_inputs"])
        # Execute pipeline
        url = f"{self.base_environment_api_url}/pipelines/{pipeline_name}/run"
...
/outputs?wait_for_results=true"
        ...
        for output in response:
            if output["type"] == "parameter":
                parsed_response[output["name"]] = output["value"]
            elif output["type"] == "artifact":
                parsed_response[
                    output["name"]
                ] = self._retrieve_single_pipeline_execution_output(
                    pipeline_name, execution_id, output["name"]
                )
