...
class Stubber:
	...
	def get_obj_attributes(L,item_instance):
		...
		A=i([A for A in A if not A[0].startswith(O)],key=lambda x:x[4]);C.collect();return A,J
	...
	def create_module_stub(I,module_name,file_name=F):
		...
		if D is F:K=A.replace(L,O)+'.py';D=I.path+G+K
		else:K=D.split(G)[-1]
	...
	def write_object_stub(L,fp,object_expr,obj_name,indent,in_class=0):
		...
		for (F,K,G,S,f) in R:
			...
			if G=="<class 'type'>"and M(D)<=v*4:
				...
				A='\n{}class {}({}):\n'.format(D,F,U)
				if V:A+=D+'    ...\n';H.write(A);return
				H.write(A);L.write_object_stub(H,S,'{0}.{1}'.format(obj_name,F),D+'    ',Q+1);A=D+'    def __init__(self, *argv, **kwargs) -> None:\n';A+=D+'        ...\n\n';H.write(A)
...
def _info():
	h='ev3-pybricks';g='pycom';f='pycopy';c='GENERIC';a='arch';Z='cpu';Y='ver';V='with';G='mpy';E='build';A=b({N:sys.implementation.name,D:B,E:B,Y:B,l:'stm32'if sys.platform.startswith('pyb')else sys.platform,K:c,Z:B,G:B,a:B})
	try:A[D]=L.join([str(A)for A in sys.implementation.version])
	except I:pass
	try:W=sys.implementation._machine if'_machine'in R(sys.implementation)else os.uname().machine;A[K]=W.strip();A[Z]=W.split(V)[1].strip();A[G]=sys.implementation._mpy if'_mpy'in R(sys.implementation)else sys.implementation.mpy if G in R(sys.implementation)else B
	except (I,S):pass
	C.collect()
	...
	for (j,k,n) in [(f,f,'const'),(g,g,'FAT'),(h,'pybricks.hubs','EV3Brick')]:
		try:o=__import__(k,F,F,n);A[N]=j;del o;break
		except (Q,T):pass
