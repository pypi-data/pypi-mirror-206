...
import requests
import math
from .utilities import get_bus_energy_rate, get_current_tou, f_bus, get_buffer
...
class Ebus:
    def __init__(self, env, bussim, name, bus_type = 'std', gen = 'gen_1', capacity = 500):
        ...
        self.name = name
        self.bussim = bussim
        self.energy_rates = self.bussim.bus_energy_rates
...
def dispatch(env, bussim, block, block_waittime):
    ...
    energy_rates, block_tracker, depot, bus_gen, bus_batt_min, bus_kwh_capacity = bussim.bus_energy_rates, bussim.block_tracker, bussim.depot, bussim.bus_gen, bussim.bus_batt_min, bussim.bus_kwh_capacity
    ...
    if block_energy_need > bus_kwh_capacity[bus_gen][block['type']]: # don't even try to run bus if there are no buses with big enough battery
