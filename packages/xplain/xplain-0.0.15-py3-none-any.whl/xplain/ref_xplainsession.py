...
class Ref_xplainsession:
    ...
    def build_predictive_model(self, model_name, model_config,
        ...
        self.run(query)
        result = {}
        df_independent_var = self.load_file_as_df(output_prefix + model_name +
                                                  "-IndependentVariables.csv")
        df_learning_log = self.load_file_as_df(output_prefix + model_name +
                                               "-Learning-Log.txt")

        df_observation = self.load_file_as_df(output_prefix + model_name +
                                              "-Observations.csv")

        df_predicted_probabilities = self.load_file_as_df(output_prefix +
                                                          model_name +
                                                          "-PredictedProbabilities.csv")
        df_roc = self.load_file_as_df(output_prefix + model_name + "-ROC.csv")
    ...
    def read_file(self, ownership, file_type, file_path, file_extension):
        payload = json.dumps({
            'file': {
                'ownership': ownership,
                'file_type': file_type,
