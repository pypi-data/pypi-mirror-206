...
class AsyncFaunaModel(JSONModel):
    ...
    async def find_all(cls, limit:int, offset:int)->List[AsyncFaunaModel]:
        ...
        try:
            ...
            refs = (await _q(q.paginate(q.match(q.index(cls.__name__.lower())), after=offset), size=limit))[
                "data"
            ]
            ...
            for field in self.__fields__.values():
                ...
                if field.field_info.extra.get("unique"):
                    ...
                    if isinstance(instance, self.__class__):
                        ...
                        raise ValueError(f"Field {field.name} must be unique")
    ...
    async def update(cls, ref:str, **kwargs)-> AsyncFaunaModel:
        try:
            ...
            if isinstance(instance, cls):
                instance_updated = await cls.q()(q.update(q.ref(q.collection(cls.__name__.lower()), ref), {"data": kwargs}))
                return cls(
                    **{
