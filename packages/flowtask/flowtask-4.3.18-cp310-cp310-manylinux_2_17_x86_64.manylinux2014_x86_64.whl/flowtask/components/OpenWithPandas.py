    ...
    async def run(self) -> Any:
        ...
        if len(result) == 1:
            ...
            if self.no_multi is True: # get only one element
                ...
                try:
                    df = pandas.concat(result, sort=False, axis=0, ignore_index=True).reindex(result[0].index)
                    print('CONCAT ', df)
                except Exception as err:
                    print('ERR ', err)
                    raise ComponentError(
        ...
        if self._colinfo:
            # fix the datatype for every column in dataframe
            for column, dtype in self._colinfo.items():
                ...
                try:
                    if dtype == 'timestamp without time zone' or dtype == 'timestamp with time zone' or dtype == 'date':
                        ...
                        if not ctype in ('Int8', 'Int32', 'Int64'):
                            df[column] = pandas.to_numeric(
        ...
        if self._debug is True:
            ...
            #     self._logger.debug(
            #         f"Open Chunk File with Pandas {self.fileIterator}"
            #     )
            # else:
            self._logger.debug(
