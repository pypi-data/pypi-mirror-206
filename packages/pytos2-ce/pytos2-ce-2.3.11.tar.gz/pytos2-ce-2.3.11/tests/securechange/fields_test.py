...
from tests.securechange.conftest import mock_devices
...
class TestField:
    ...
    def test_read_only(self):
        field = TextArea.kwargify(
            json.load(open(f"tests/securechange/json/field/read_only-field.json"))
        )
        ...
        def test_devices(devices):
            for d in devices:
                responses.add(
                    ...
                    json=json.load(open(f"tests/securetrack/json/policies/20.json")),
                )
        ...
        def add_targets(devices):
            ...
            for d in devices:
                responses.add(
                    responses.GET,
                    f"https://198.18.0.1/securetrack/api/devices/{d.id}/policies",
                    json=json.load(open(f"tests/securetrack/json/policies/157.json")),
                )
                device = st.get_device(d.id)
                assert isinstance(device, Device)
