    ...
    def decor(dec):
        async def wrapp(ay):
            ...
            try:
                ...
                ftext = "**Aurora Client Error:** `Forward this to` @kynansupport\n\n"
                ftext += "**Aurora Version:** `" + str(pyver)
                ftext += "`\n**Userbot Version:** `" + str(ayra_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START AURORA CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Aurora Client Error:** `Forward this to` @kynansupport\n\n",
                        )
                else:
