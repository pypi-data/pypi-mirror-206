...
class StorageAnnotation(Annotation):
    def __init__(self, number_of_bytes: int, slot: Optional[int] = None, offset: Optional[int] = None) -> None:
...
class Type(ABC):
    def __init__(self, name: str, type_string: str, annotation: Optional[Annotation] = None):
        """
...
class PrimitiveType(Type):
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {"primitiveName": self.name,
                "type": "Primitive",
                "annotation": self.annotation.as_dict() if self.annotation is not None else None
                }
