...
class Resource(API, Operations):
    ...
    def simulate_callback(self, response={}):
        if self.get_bank_id() in [super().EQUITY]:
            if self.get_operation() not in [super().BANK_TO_MPESA]:
    ...
    def standardize_callback(self, callback_data={}, forward=True):
        ...
        try:
            ...
            if callback_data.get("bank_id", -1) == super().COOP:
                ...
                if callback_data.get("type", -1) in [super().IFT, super().BANK_TO_MPESA, super().PESALINK_TO_BANK, super().PESALINK_TO_MOBILE]:
                    ...
                    if 'Destinations' in callback_data.keys():
                        ...
                        if isinstance(callback_data.get("Destinations", []), list):
                            ...
                            if callback.type_code == super().BANK_TO_MPESA:
                                response["account_destination"] = callback_data.get(
                ...
                if callback_data.get("type", -1) == super().BANK_TO_MPESA:
        ...
        try:
            ...
            if ipn_data.get("bank_id", -1) == super().EQUITY:
                ...
                response.update({
                    ...
                    "amount": f'{ipn_data.get("trans_ammount", "-1")}',
    ...
    def register_callback(self):
        ...
        if self.get_bank_id() in [super().COOP, super().EQUITY]:
            if super().EQUITY == self.get_bank_id() and self.get_operation() not in [super().BANK_TO_MPESA, super().PDF_TO_JSON]:
                async_request = False
        ...
        if save_callback:
            ...
            if reference != '' and callback != '':
                callback_created=Callbacks.objects.create(reference=reference, tenant_id=request.get("__tenant", None), business_id=request.get("business_organization_unit_id", None),
                ...
                callback_created.async_request=async_request
                callback_created.save()
