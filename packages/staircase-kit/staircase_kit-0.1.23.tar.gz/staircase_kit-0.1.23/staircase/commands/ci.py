...
async def command(
    *,
    ...
    if not ci_env:
        ...
        selected_env = prompt(env_names, header="Select ci-env, default = marketplace.staircaseapi.com...")
        if selected_env:
    ...
    if "clone" in actions:
        ...
        bundle_url = await clone(
            ci_env, repo=repo, branch=branch, github_token=user_config.github_token
        )
        if verbose:
