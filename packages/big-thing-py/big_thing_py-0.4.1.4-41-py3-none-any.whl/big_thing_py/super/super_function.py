...
class MXSuperFunction(MXFunction):
    ...
    def __init__(self, func: Callable,
        ...
        self._is_scanned: bool = True

        # scheduling에 대한 결과를 super thing이 넣어주기 위해 존재함
        #     super_request_key: str = make_super_request_key(requester_middleware, scenario)
        #     super_request_key: {requester_middleware}@{scenario}, value: MXSuperScheduleRequest
        ...
        #     super_request_key: str = make_super_request_key(requester_middleware, scenario)
        #     super_request_key: {requester_middleware}@{scenario}, value: MXSuperExecuteRequest
        self._mapping_table: Dict[str, MXSuperExecuteRequest] = {}

        # super service의 동작을 하는데 있어 필요한 subscribe, unsubscribe동작을 수행하기 위해 필요
    ...
    def _add_sub_service_request(self, sub_service_request: MXSubServiceRequest):
        '''
            super service에 명세된 req() 라인 수 만큼 존재한다. 따라서 중복된 sub_service_type이 존재할 수 없다.
        '''
    ...
    def _request_subschedule(self, subschedule_request: MXSubScheduleRequest) -> None:
        ...
        MXLOG_DEBUG(
            f'[{subschedule_request._action_type.value.upper()} {subschedule_msg._status.value.upper()} START] {subschedule_msg.sub_service_name}|{subschedule_msg.target_thing_name}|{subschedule_msg.target_middleware_name}', 'cyan')
    ...
    def _select_target_sub_service_by_range_type(self, super_schedule_request: MXSuperScheduleRequest, single_select_func: Callable) -> MXSubScheduleRequest:
        for sub_service_request in self._sub_service_request_list:
            ...
            sub_service_request_key = make_sub_service_request_key(
                sub_service_request._sub_service_type.get_name(), sub_service_request._sub_service_request_order)
    ...
    def _subschedule_parallel(self, super_schedule_request: MXSuperScheduleRequest, target_schedule_status: MXScheduleStatus, result_schedule_status: MXScheduleStatus) -> List[MXSubScheduleRequest]:
        ...
        for sub_service_request in super_schedule_request._sub_service_request_table.values():
            ...
            for candidate_subschedule_request in candidate_subschedule_request_list:
                ...
                MXLOG_DEBUG(f'[{MXActionType.SUB_SCHEDULE.value.upper()} {candidate_subschedule_request._status.value.upper()} END] \
                                {candidate_subschedule_request.trigger_msg.sub_service_name}|{candidate_subschedule_request.trigger_msg.target_thing_name}|\
                                {candidate_subschedule_request.trigger_msg.target_middleware_name}, duration: {candidate_subschedule_request.duration():.4f} Sec', 'cyan')

                # 굳이 구독 해제를 할 필요가 없다. 재호출될 가능성이 높음.
                # subschedule_result_msg: MXSubScheduleResultMessage = candidate_subscehdule_request._result_msg
        ...
        if super_schedule_request._status == MXScheduleStatus.CHECK:
            ...
            MXLOG_DEBUG(f'[{MXActionType.SUPER_SCHEDULE.value.upper()} {MXScheduleStatus.CHECK.value.upper()} END] \
                            {super_schedule_request.trigger_msg.super_service_name}|{super_schedule_request.trigger_msg.super_thing_name}|\
                            {super_schedule_request.trigger_msg.super_middleware_name}, duration: {super_schedule_request.confirm_duration():.4f} Sec', 'green')
            ...
            MXLOG_DEBUG(f'[{MXActionType.SUPER_SCHEDULE.value.upper()} {MXScheduleStatus.CONFIRM.value.upper()} END] \
                            {super_schedule_request.trigger_msg.super_service_name}|{super_schedule_request.trigger_msg.super_thing_name}|\
                            {super_schedule_request.trigger_msg.super_middleware_name}, duration: {super_schedule_request.confirm_duration():.4f} Sec', 'green')
    ...
    def _super_schedule_wrapper(self, super_schedule_request: MXSuperScheduleRequest, hierarchical_service_table: Dict[str, List[MXService]], timeout: float = 10):
        '''
        ...
        super_request_key = make_super_request_key(requester_middleware_name, scenario)

        MXLOG_DEBUG(
            f'[{MXActionType.SUPER_SCHEDULE.value.upper()} START] {self._name} by scenario {scenario}.', 'green')
        ...
        try:
            ...
            super_schedule_request.generate_subschedule_request(self._sub_service_request_list, hierarchical_service_table['functions'])
            # self._check_confirm_parallel(super_schedule_request, timeout=timeout)
            ...
            MXLOG_DEBUG(f'[SCHEDULE TIMEOUT] {super_schedule_request._action_type.value} of {self._name} \
            ...
            print_error(e)
            MXLOG_DEBUG(f'[{super_schedule_request._action_type.value.upper()} FAILED] {super_schedule_request._action_type.value} of {self._name} \
            ...
            super_execute_request = MXSuperExecuteRequest(super_schedule_request=super_schedule_request)
            ...
            MXLOG_DEBUG(
                f'[{MXActionType.SUPER_SCHEDULE.value.upper()} END] {self._name} by scenario {scenario}. duration: {super_schedule_request.duration():.4f} Sec', 'green')

    def _super_execute_wrapper(self, super_execute_request: MXSuperExecuteRequest):
        '''
            super service의 하위 함수에 대한 정보를 추출한다.
            service_list 구조는
        ...
        MXLOG_DEBUG(
            f'[{MXActionType.SUPER_EXECUTE.value.upper()} START] {self._name} by scenario {scenario}.', 'green')

        try:
            super_execute_request._running = True
            self._running = True
            error = MXErrorCode.NO_ERROR
            ...
            if self._timeout:
                self._return_value = func_timeout(
                    self._timeout, self._func, args=(*tuple(arguments), ))
            else:
            ...
            MXLOG_DEBUG(f'[FUNC TIMEOUT] {super_execute_request._action_type.value} of {self._name} \
                    by scenario {requester_middleware_name}|{scenario} timeout...', 'red')

            # Timeout이 발생했다고 해서 mapping_table에서 삭제하지는 않는다.
            # if super_execute_request._action_type == MXActionType.SUPER_EXECUTE:
            ...
            print_error(e)
            MXLOG_DEBUG(f'[{MXActionType.SUB_EXECUTE.value.upper()} FAILED] {super_execute_request._action_type.value} of {self._name} \
                by scenario {requester_middleware_name}|{scenario} failed...', 'red')
            ...
            super_execute_request._current_sub_service_request_cnt = 0
            ...
            super_execute_request._running = False
            self._running = False
            self._running_scenario_list.remove(super_execute_msg.scenario)
            MXLOG_DEBUG(
                f'[{MXActionType.SUPER_EXECUTE.value.upper()} END] {self._name} by scenario {scenario}. duration: {super_execute_request.duration():.4f} Sec', 'green')

    def start_super_schedule_thread(self, super_schedule_msg: MXSuperScheduleMessage, hierarchical_service_table: Dict[str, List[MXService]], timeout: float = 10) -> MXThread:
        super_request_key = make_super_request_key(super_schedule_msg.requester_middleware_name, super_schedule_msg.scenario)
        if super_request_key in self._temporary_scheduling_table:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super request with key: {super_request_key} is currently in the scheduling phase!', 'red')
            return False
        ...
        super_request_key = make_super_request_key(super_schedule_msg.requester_middleware_name, super_schedule_msg.scenario)
        self._temporary_scheduling_table[super_request_key] = super_schedule_request
    ...
    def start_super_execute_thread(self, super_execute_msg: MXSuperExecuteMessage, super_request_key_table: Dict[str, List[str]]) -> MXThread:
        super_request_key = make_super_request_key(super_execute_msg.requester_middleware_name, super_execute_msg.scenario)
        if super_request_key in self._temporary_scheduling_table:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super request with key: {super_request_key} is currently in the scheduling phase!', 'red')
            return False

        super_execute_request = self._mapping_table[super_request_key]
        if super_execute_request._running:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Execute of Super request key: {super_request_key} is already executed!', 'red')
            return False

        super_request_key_list = [super_request_key] * len(super_execute_request._sub_service_request_table)
        if self.get_name() in super_request_key_table:
            super_request_key_table[self.get_name()].extend(super_request_key_list)
        else:
            super_request_key_table[self.get_name()] = super_request_key_list

        super_execute_request.trigger_msg = super_execute_msg
        super_execute_request.result_msg = MXSuperExecuteResultMessage(super_service_name=self._name,
                                                                       super_thing_name=self._thing_name,
                                                                       super_middleware_name=self._middleware_name,
                                                                       requester_middleware_name=super_execute_msg.requester_middleware_name,
                                                                       scenario=super_execute_msg.scenario,
        ...
        super_execute_thread = MXThread(
            ...
            name=f'{self._func.__name__}_{super_execute_request._action_type.value}_thread',
            daemon=True,
            args=(super_execute_request, ))
    ...
    def add_sub_service_request_info(self, sub_service_name: str, arg_list: Union[Tuple[MXArgument], Tuple], tag_list: List[MXTag], return_type: MXType, range_type: MXRangeType):
        mx_arg_list = [arg if isinstance(arg, MXArgument) else MXArgument(name=f'DUMMY_ARG_{arg}',
                                                                          bound=(-sys.maxsize - 1, sys.maxsize),
                                                                          type=MXType.get(type(arg))) for arg in arg_list]
        ...
        MXLOG_DEBUG(f'Extract sub_service_request end. \
            super service: {self._name}, \
            sub_service: {sub_service_name}, \
            arg_list: [{", ".join([mx_arg._name for mx_arg in mx_arg_list])}] \
            tag_list: [{", ".join([str(tag) for tag in tag_list])}]', 'green')
        return True
    ...
    def put_subschedule_result(self, super_service_request_key: str, sub_service_request_key: str, subschedule_result_msg: MXSubScheduleResultMessage) -> bool:
        '''
            super thing에 의해 호출되어 super thing이 받은 subschedule결과를 super service에게 전달한다.
        '''
        if super_service_request_key not in self._temporary_scheduling_table:
            MXLOG_DEBUG(
                f'Could not find key {super_service_request_key} in temporary_scheduling_table... This key is not mine!', 'yellow')
            return False
    ...
    def put_sub_service_execute_result(self, super_request_key: str, sub_service_request_key: str, sub_service_execute_result_msg: MXSubExecuteResultMessage) -> bool:
        '''
            super thing에 의해 호출되어 super thing이 받은 sub_service_execute결과를 super service에게 전달한다.
        '''
        if super_request_key not in self._mapping_table:
            MXLOG_DEBUG(
                f'Could not find key {super_request_key} in mapping_table... This key is not mine!', 'yellow')
            return False

        super_execute_request = self._mapping_table[super_request_key]
        result = super_execute_request.put_sub_service_execute_result_msg(sub_service_request_key, sub_service_execute_result_msg)
        return result
    ...
    def set_first_execute(self, first_execute: bool) -> None:
        self._is_scanned = first_execute
