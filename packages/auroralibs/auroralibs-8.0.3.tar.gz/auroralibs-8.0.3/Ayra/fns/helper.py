...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "© Kynan"
            await eod(ok, f"✓ `Aurora - Installed`: `{plug}` ✓\n\n{output}")
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Aurora {ayra_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Aurora {ayra_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
