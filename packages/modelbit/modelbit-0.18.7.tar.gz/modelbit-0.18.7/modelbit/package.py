...
def add_module(m: ModuleType, api: Optional[MbApi] = None) -> Optional[PackageInfo]:
  ...
  builder = PackageBuilder(api)
  if "." in m.__package__:
    baseModuleName = m.__package__.split(".")[0]
    baseModule = sys.modules[baseModuleName]
    if baseModule is None:
      raise UserFacingException(f"Cannot find module '{baseModuleName}' to upload")
    return add_module(baseModule, api)
...
class QuietBuilder(build.ProjectBuilder):
  ...
  def __init__(self, *args, **kwargs):
    try:
      ...
      kwargs["runner"] = pyproject_hooks.quiet_subprocess_runner
    except:
      pass
