...
def genphen(y_G0, G1, covDat, options, nInd, K1=None, fracCausal=None, randseed=None):
    ...
    Notes: uses np.random.X so that the seed that was set can be used
    """

    np.random.seed(int(randseed % 2147483647))  # old maxint
    ...
    assert not (
        G1 is not None and K1 is not None
    ), "need to provide only either G1 or K1"
