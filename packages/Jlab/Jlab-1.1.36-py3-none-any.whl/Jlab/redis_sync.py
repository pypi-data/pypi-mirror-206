...
class RedisSync:
    ...
    def get_files(self):
        if not os.path.exists("finlab_database.csv"):
            ...
            download_file(repo=repo, github_file_path="finlab_database.csv")
    ...
    def sync_github_to_redis(self, force_download=False):
        if self.check_update("price:收盤價") or force_download:
