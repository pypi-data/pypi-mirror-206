...
from ..objective_functions.base_objective_function import ObjectiveFunction
...
class LBFGS(Optimizer):
    ...
    available for usage with an ``ObjectiveFunction``.
    ...
    Notes
    -----
    Valid entries in :attr:`options` are
        x0
            Initial guess for solution vector. Must be the same size as
            :attr:`functional.coefficients`. Defaults to :attr:`objective_function.initial_values`.
        bounds
    ...
    def __init__(self,
                 objective_function: ObjectiveFunction,
                 **kwargs: Dict[str, Any]):
        super().__init__(objective_function, **kwargs)
    ...
    def optimize(self) -> Dict:
        ...
        -------
            ...
            the shape of the gradient from :attr:`objective_function`.
        """
        lbfgs_kwargs = dict(x0=self._objective_function.initial_values,
                            bounds=None)
        ...
        def objective_function_wrapper(coefficients):
            d = self._objective_function.get_residual_norm(coefficients, get_gradient=True)
            # Store gradient shape to reshape flattened output.
            if self._output_shape is None:
                self._output_shape = d['gradient'].shape
            return d['residual_norm'], d['gradient'].ravel()

        result = minimize(fun=objective_function_wrapper, **lbfgs_kwargs,
    ...
    def __hash__(self) -> int:
        to_hash = [self._options, hash(self._objective_function)]
