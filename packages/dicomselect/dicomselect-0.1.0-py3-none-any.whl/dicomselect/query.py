...
class Query:
    ...
    def count(self) -> int:
        if not self.is_base:
            return self._factory.temp_tables[self._name]
        return self._factory.count
    ...
    def where(self, column: str, operator: str, values: Union[List[str], str], groupby_any: str = None,
              groupby_all: str = None, invert: bool = False) -> 'Query':
        """
        ...
        *values : str
            ...
            where appropriate
        groupby_any : str, optional
            If provided, the rows are grouped by the given column before the query is executed. The query is performed on
            each group, and if any in a group passes the query, those groups are returned.
        groupby_all : str, optional
            If provided, the rows are grouped by the given column before the query is executed. The query is performed on
            each group, and if all in a group passes the query, those groups are returned.
        invert: bool, optional
            Invert the query, by prefixing the query with a NOT.
        """
        loc = locals()
        self._factory.check_if_exists('column', self.columns, loc['column'], loc['groupby_any'], loc['groupby_all'])
        ...
        condition = f'{invert} ({values})'
        if groupby_any and groupby_all:
            raise ValueError('only group_by_any or group_by_all may be defined, not both')
        if groupby_any:
            raise NotImplementedError()
            sql_where = f'GROUP BY {groupby_any} HAVING COUNT(CASE WHEN {condition} THEN 1 END) > 0;'
        elif groupby_all:
            raise NotImplementedError()
            sql_where = f'WHERE {condition} GROUP BY {groupby_all} HAVING COUNT(*) = SUM(CASE WHEN {condition} THEN 1 ELSE 0 END)'
        else:
            sql_where = f'WHERE {condition}'

        return self._factory.create_query_from_sql(sql_where, self._name if not self.is_base else '')
    ...
    def as_paths(self) -> List[Path]:
        paths = self._factory.execute(f'SELECT path FROM data INNER JOIN {self._name} ON data.id = {self._name}.id').fetchall()
        return [Path(p[0]) for p in paths]
