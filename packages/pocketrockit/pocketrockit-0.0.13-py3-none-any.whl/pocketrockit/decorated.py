...
from pocketrockit.engine import EMPTY_COMMAND, STEP_SIZE, Env, Stage, run
from pocketrockit.misc import error
...
def track(track_definition_fn) -> None:
    ...
    logger().info(f"define track '%s'", track_definition_fn.__name__)
    stage.new_track = []
...
def resolve(note_str: str, scale: Sequence[int]) -> int:
    ...
    shift = sum(12 if e == "+" else -12 if e == "-" else 0 for e in modifier) if modifier else 0
    tone = (
        scale[
            int(degree) - 1
            if degree is not None
            else rom_degree
            and ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii"].index(rom_degree.lower())
        ]
        if degree or rom_degree
        else NOTES[note or num_note]
    )
...
def midi_commands(string: str, channel: int, force_note: None | str, scale, velocity: int):
    ...
    try:
        ...
        raise RuntimeError(f"could not turn '{string}' into note ({exc})")
...
# elements: Sequence[Union[str,"Block"]] = field(default_factory=list)
    ...
    for i, elem in enumerate(nested_pattern):
        if isinstance(elem, str):
            ...
            for j, sub_elem in enumerate(elem):
                result.append((i * step_size + j * sub_step_size, sub_elem))
    # print(result)
    return result
...
def pattern_list_from(
    *pattern_definition: str | Sequence[str],
    ...
    def block(pattern: str) -> Sequence[str]:
        # print(list(parenthetic_contents(pattern)))
        # print(list(parenthetic_contents(" a  b")))
        # print(list(parenthetic_contents("ab(cd)ef(gh)if(kl(mn))op")))
        return [
            quantized_pattern(parenthetic_contents(line), STEP_SIZE)
    ...
    *pattern: str | Sequence[str],
    ...
    while True:
        if reset_tick is not None:
            logger().debug(f"reset to {reset_tick}")
            yield EMPTY_COMMAND
        ...
        for loop_nr, block_nr, block_key, scale, line_nr, (next_relative_tick, next_element) in (
            ...
            # print(f"loop:{loop_nr} block: {block_nr} key:{block_key}"
            #      f" line:{line_nr} next_tick:{next_tick} => {next_element}")
