...
class UserRole:
	...
	def set_role(self, id: int, name: str, access_type: int):
		self.id, self.name, self.access_type = (id, name, access_type)
		return self
...
class UserSubscriptions:
	...
	def add(self, key, value):
		if key not in self._subscriptions.keys():
			self._subscriptions[key] = value

	def has_access(self, url) -> bool:
		pass
...
class SessionData:
	...
	def __init__(self, id: id = None, email: str = None, username: str = None, **kwargs):
		...
		self.has_management_access: bool = False
		...
		self.subscriptions: UserSubscriptions = self.__parsed_token_data(kwargs.get('subscriptions', {}), UserSubscriptions)
	...
	def add_subscription(self, key, value):
		self.subscriptions.add(key, value)
