...
class MaterializedFeatureView(FeatureView):
    ...
    def run(
        ...
        **mock_sources: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if (
            aggregation_level == run_api.AGGREGATION_LEVEL_FULL
            and self.feature_start_time is not None
            and pendulum.instance(start_time) < self.feature_start_time
        ):
            logger.warn(
                f"run() start_time ({start_time}) is less than the feature view feature_start_time ({self.feature_start_time}). Fully aggregated values before feature_start_time will be filtered from the final result."
            )
    ...
    def test_run(
        ...
        **mock_sources: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if (
            ...
            logger.warn(
                f"test_run() start_time ({start_time}) is less than the feature view feature_start_time ({self.feature_start_time}). Fully aggregated values before feature_start_time will be filtered from the final result."
            )
    ...
    def _create_unvalidated_feature_definition(
        ...
        if set(input_name_to_ds_ids.keys()) != set(mock_sources.keys()):
            raise errors.FV_INVALID_MOCK_SOURCES(list(mock_sources.keys()), list(input_name_to_ds_ids.keys()))
