...
from keycloak import KeycloakOpenID as KeycloakOpenIDDefault, urls_patterns
from keycloak import KeycloakAdmin as KeycloakAdminDefault
import json
...
from django_keycloak.groups_permissions import GroupsPermissions


class KeycloakAdmin(KeycloakAdminDefault):
    ...
    def __init__(
        ...
        KeycloakAdminDefault.__init__(self, server_url,
                                      username,
                                      password,
                                      token,
                                      totp,
...
def get_default_keycloak_open_id():
    ...
    try:
        ...
        client_id_valids = config.get('CLIENT_ID_VALIDS', [])
        return KeycloakOpenID(server_url=server_url,
                              client_id=client_id,
                              realm_name=realm,
                              client_secret_key=client_secret_key,
                              client_id_valids=client_id_valids)
...
class KeycloakOpenID(KeycloakOpenIDDefault):
    ...
    def __init__(
        ...
        KeycloakOpenIDDefault.__init__(self, server_url,
                                       realm_name,
                                       client_id,
                                       client_secret_key,
                                       verify,
                                       ...
                                       proxies,
                                       timeout)
        self.client_id_valids = client_id_valids
...
-----END PUBLIC KEY-----"""
    ...
    def get_roles_permissions_user(self, token, public_key_validate='inter', method_token_info="introspect", **kwargs):
        ...
        # if token_info['azp'] not in self.client_id_valids:
        #     raise KeycloakInvalidTokenError("Invalid Client.")

        if method_token_info == "introspect" and not token_info["active"]:
            raise KeycloakInvalidTokenError("Token expired or invalid.")

        # user_resources = token_info["resource_access"].get(token_info['azp'])  # type: ignore
