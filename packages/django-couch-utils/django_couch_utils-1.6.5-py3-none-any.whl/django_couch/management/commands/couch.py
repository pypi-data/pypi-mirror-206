...
if getattr(settings, 'COUCHDB_WRAPPER', None) == 'couchdbcurl':
    ...
    from couchdbrq import Server, Document, ResourceNotFound
else:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('command', help = u'Restore/dump'),
        parser.add_argument('--force', '-f', action = 'store_true', dest = 'real_run', default = False, help = u'Real run (change target data)'),
        parser.add_argument('--server', '-s', action = 'store', dest = 'server', default = 'http://localhost:5984', help = u'Server address'),
        parser.add_argument('--database', '-d', action = 'store', dest = 'database', help = u'Database name'),
        parser.add_argument('--path', '-p', action='store', dest='path', help=u'Views root directory, if not defined in settings'),
        parser.add_argument('--name', '-n', action = 'store', dest = 'dbkey', help = u'Database key (from settings.py). Will override --server and --database arguments', default = None),
        parser.add_argument('--delete', '-r', action = 'store_true', dest = 'delete', help = u'Delete exceed items from database during restore', default = False),
    ...
    def execute(self, *args, **options):
        ...
        if options.get('path'):
            path = options['path']
            if self.verbose:
                print("Using provided views path")
        elif settings.COUCHDB[options['dbkey']].get('path'):
            path = settings.COUCHDB[options['dbkey']]['path']
            if self.verbose:
                print("Using views path from database definition")
        else:
            path = join(settings.COUCHDB_VIEWS_DIR, options['dbkey'])
            if self.verbose:
                print("Using calculated views path")




        if command == 'backup':
            print("Backing up remote to", path)
            self.backup(db, options.get('path'))
        elif command == 'restore':
            print("Restoring local path to remote server", path)
            self.restore(db, path, options)
                ...
                for f in os.listdir(join(path, '_macros')):
                    ...
                    if ext in ['.js', '.py']:
                        if not ext in macros:
                            macros[ext] = {}
                        macros[ext][name] = open(join(path, '_macros', f), 'r').read()
            ...
            for key in os.listdir(join(path, item)):
                if key == 'views':
                    for view_name in os.listdir(join(path, item, key)):
                        ...
                        if os.path.exists(map_filename):
                            map_func = open(map_filename, 'r').read() #.decode('utf-8')

                        if os.path.exists(reduce_filename):
                            reduce_func = open(reduce_filename, 'r').read() #.decode('utf-8')
                ...
                if key in ['filters', 'shows']:
                    for func_filename in os.listdir(join(path, item, key)):
                        ...
                        filter_func = open(join(path, item, key, func_filename), 'r').read()
