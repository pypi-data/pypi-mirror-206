...
class Gene_Ontology_enrichment:
    ...
    @staticmethod
    def check_bg(query_gene_set: set, bg_gene_set):
        match_set = set()
        miss_set = set()
        for gene in query_gene_set:
            if gene in bg_gene_set:
                match_set.add(gene)
            else:
                miss_set.add(gene)
        print(f'{len(match_set):,} genes in bg, including: {match_set}.')

        print(f'{len(miss_set):,} genes not in bg, including: {miss_set}.')

    def ontology_enrichment(self, query_gene_set: list or set, id_type: str,
        ...
        # Background gene set check.
        #self.check_bg(query_gene_set, bg_data.gene_set)
