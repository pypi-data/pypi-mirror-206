...
def distance_between_blocks_distribution_2(df_blocks):
    ...
    return pd.DataFrame(ds, columns=['', 'Adjacency lengths, bp, log scale'])
...
def lengths_between(state):
    ...
    sns.histplot(hue='', x='Adjacency lengths, bp, log scale', data=ds, log_scale=(True, False), bins=25, element="step")
