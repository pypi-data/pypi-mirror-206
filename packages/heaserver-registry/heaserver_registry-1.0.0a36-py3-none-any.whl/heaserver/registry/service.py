...
def main() -> None:
    ...
    async def register_myself(app: web.Application):
        coll = app[appproperty.HEA_DB].get_collection(MONGODB_COMPONENT_COLLECTION)
        await coll.find_one_and_update({'name': 'heaserver-registry'},
                                       {'$set': {'external_base_url': app[appproperty.HEA_COMPONENT]}})


    start(db=mongo.MongoManager,
          wstl_builder_factory=builder_factory(__package__), config=config,
          cleanup_ctx=register_myself)
