...
class MqttClient(BaseMqttClient):
    ...
    def connect(self, limit: int):
        ...
        def on_disconnect(client, user_data, rc):
            self.client.connected_flag = False
            if rc != 0:                
                logger.warn(f"Unexpected disconnection: (rc = {rc}), continuing to try to connect in the background.:")
        ...
        if self.username != "" and self.password != "":
            ...
            #logger.info(f"mqtt_client connected to {self.server}:{self.port}")
        try:
            self.client.connect(
                host=self.server, port=int(self.port), keepalive=60)
        except Exception as e:
            logger.error(
                f"Failed to connect to {self.server}:{self.port} :{str(e)},continuing to try to connect in the background.:")
            # return 0
