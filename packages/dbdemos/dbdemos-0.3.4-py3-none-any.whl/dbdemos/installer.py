        ...
        for pipeline in demo_conf.pipelines:
            ...
            if existing_pipeline == None:
                ...
                if 'error_code' in p:
                    ...
                    continue
            permissions = self.db.put(f"2.0/preview/permissions/pipelines/{id}", {
                "access_control_list": [{"user_name": self.db.conf.username, "permission_level": "IS_OWNER"},
                                        {"group_name": "account users", "permission_level": "CAN_MANAGE"}]
            })
