...
class TableBody(LooqObject):
    ...
    def to_json_structure(self, visitor):
        ...
        if self.collapsable and self.row_hierarchy:
            body_content = self._collapse_table_by_rows(body_content)

        if self.collapsable and self.col_hierarchy:
            body_content = self._collapse_table_by_columns(body_content)
            for i in range(len(body_content)):
                body_content[i].update({"key": self._generate_row_key(i)})
