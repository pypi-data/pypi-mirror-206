...
def test_chunk_compressed_chunk_shape_encoder_bps_encoder_matches(tmp_datasets_dir):
    ...
    """
    ds = deeplake.dataset(tmp_datasets_dir / LZ4_CHUNK_COMPRESSON_SHAPES_ENCODER, overwrite=True)
    with ds:
        ds.create_tensor(
            "tokens",
            "text",
...
def test_samples_compressed_chunk_shape_encoder_bps_encoder_matches(tmp_datasets_dir):
    ...
    assert len(cpp_ds) == 2
    assert cpp_ds.tensors[0][0].shape == [390]
    assert cpp_ds.tensors[0][1].shape == [374]
...
def test_coco_bmasks_equality():
    ...
    for i in range(100):
        ...
        assert hub_ds.images[i].numpy().shape == tuple(cpp_ds.tensors[0][i].shape)
...
def test_if_dataset_exists():
    ...
    with pytest.raises(api.api.dataset_not_found) as ds_not_found:
