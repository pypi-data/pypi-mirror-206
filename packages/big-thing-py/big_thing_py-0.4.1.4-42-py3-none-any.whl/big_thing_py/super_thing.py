...
class MXSuperThing(MXBigThing):
    ...
    # Super Service Execution 요청이 들어왔을때 mapping_table에 있는 super_requestf를 찾기 위한 super_request_key 리스트
    # Super Service는 자신의 이름으로 super_request_key를 찾을 수 있다.
    # {
    #     'super_service_name1': ['super_request_key1', 'super_request_key2', ...],
    #     'super_service_name2': ['super_request_key3', 'super_request_key4', ...],
    #      ...
    # }
    _SUPER_REQ_KEY_TABLE: Dict[str, List[str]] = dict()
    ...
    def __init__(self, name: str = DEFAULT_NAME, service_list: List[MXService] = [], alive_cycle: float = 60, is_super: bool = True, is_parallel: bool = True,
        ...
        self._SUPER_REQ_KEY_TABLE = dict()
    ...
    def _handle_MS_SCHEDULE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        schedule_thread = target_super_service.start_super_schedule_thread(super_schedule_msg, self._global_service_table, timeout=10)
    ...
    def _handle_MS_EXECUTE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        super_execute_thread = target_super_service.start_super_execute_thread(super_execute_msg, self._SUPER_REQ_KEY_TABLE)
    ...
    def _handle_MS_RESULT_SCHEDULE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        super_request_key = make_super_request_key(subschedule_result_msg.requester_middleware_name, subschedule_result_msg.scenario)
        subrequest_key = make_sub_service_request_key(subschedule_result_msg.sub_service_name, subschedule_result_msg.sub_service_request_order)
        ...
        result = target_super_service.put_subschedule_result(super_request_key, subrequest_key, subschedule_result_msg)
        return result
    ...
    def _handle_MS_RESULT_EXECUTE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        subrequest_key = make_sub_service_request_key(subexecute_result_msg.sub_service_name, subexecute_result_msg.sub_service_request_order)

        target_super_service = self._get_function(subexecute_result_msg.super_service_name)
        if not target_super_service:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {subexecute_result_msg.super_service_name} does not exist...', 'yellow')
            return False

        super_request_key = make_super_request_key(subexecute_result_msg.requester_middleware_name, subexecute_result_msg.scenario)
        result = target_super_service.put_sub_service_execute_result(super_request_key, subrequest_key, subexecute_result_msg)
        return result
    ...
    def _send_SM_EXECUTE(self, subexecute_msg: MXSubExecuteMessage) -> None:
        subexecute_mqtt_msg = subexecute_msg.mqtt_message()
        self._publish_queue.put(subexecute_mqtt_msg)
    ...
    def _request_subexecute(self, subexecute_request: MXSubExecuteRequest) -> None:
        if not isinstance(subexecute_request, MXSubExecuteRequest):
            raise MXTypeError(f'[{get_current_function_name()}] Invalid type of subexecute_request: {type(subexecute_request)}')

        subexecute_msg = subexecute_request.trigger_msg
        self._send_SM_EXECUTE(subexecute_msg)
        MXLOG_DEBUG(f'[{subexecute_request._action_type.value.upper()} START] {subexecute_msg.sub_service_name}|{subexecute_msg.target_thing_name}|{subexecute_msg.target_middleware_name}', 'cyan')

    # 하나의 reqline에 있는 subexecute_request들을 병렬로 실행한다.
    def _subexecute_parallel(self, subexecute_request_list: List[MXSubExecuteRequest], arg_list: Union[int, float, str, bool]) -> List[Union[int, float, str, bool]]:
        result_list = []
        for subexecute_request in subexecute_request_list:
            subexecute_request.timer_start()
            subexecute_request.trigger_msg.arguments = arg_list
            self._request_subexecute(subexecute_request)

        for subexecute_request in subexecute_request_list:
            subexecute_request.result_msg = subexecute_request.get_result_msg()
            subexecute_request.timer_end()

            subexecute_msg: MXSubExecuteMessage = subexecute_request.trigger_msg
            subexecute_result_msg: MXSubExecuteResultMessage = subexecute_request.result_msg
            result_list.append(dict(scenario=subexecute_result_msg.scenario,
                                    return_type=subexecute_result_msg.return_type,
                                    return_value=subexecute_result_msg.return_value,
                                    error=subexecute_result_msg.error))

            MXLOG_DEBUG(
                f'[{MXActionType.SUB_EXECUTE.value.upper()} END] {subexecute_msg.sub_service_name}|{subexecute_msg.target_thing_name} duration: {subexecute_request.duration():.4f} Sec', 'cyan')
    ...
    def req(self, sub_service_name: str, tag_list: List[str], arg_list: Union[Tuple[MXArgument], Tuple] = [],
        ...
        if target_super_service.get_is_scanned():
            ...
            super_request_key = self._SUPER_REQ_KEY_TABLE[super_service_name].pop(0)
            super_execute_request = target_super_service._mapping_table[super_request_key]
            subrequest_key = make_sub_service_request_key(
                sub_service_name, super_execute_request._current_sub_service_request_cnt)
            subexecute_request_list = super_execute_request._sub_service_request_table[subrequest_key]._target_request_list
            result_list = self._subexecute_parallel(subexecute_request_list, arg_list)
            super_execute_request._current_sub_service_request_cnt += 1
            return result_list
    ...
    def _extract_sub_service_request_info(self) -> None:
        for function in self._function_list:
            if self._is_super and function.get_is_scanned():
