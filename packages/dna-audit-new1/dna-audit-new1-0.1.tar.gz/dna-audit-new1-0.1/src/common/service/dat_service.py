...
def check_config_exists():
    ...
    """
    config_path = os.path.join(os.path.dirname(__file__), constants.Paths.CONFIG_FILE_PATH)
    try:
...
def get_or_create_config():
    ...
    try:
        ...
        config_changes={}
        if config_exists:
            with open(os.path.join(os.path.dirname(__file__), constants.Paths.CONFIG_FILE_PATH), encoding='utf-8') as f:
                config_data = json.load(f)
            if config_data['region_name'] == os.environ.get('AWS_DEFAULT_REGION'):
        ...
        config_data = helper.create_config_file(output,config_changes)
        helper.store_config_file(config_data)
    except Exception as ex:
