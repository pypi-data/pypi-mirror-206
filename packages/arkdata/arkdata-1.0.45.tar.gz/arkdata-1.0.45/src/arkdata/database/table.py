...
from .table_types import BulkInsert, BulkUpdate, BulkDelete
...
class Table:
    ...
    @classmethod
    def find_by(cls, **kwargs) -> Table | None:
        if len(kwargs) == 0:
            return
    ...
    @classmethod
    def find_all_by(cls, **kwargs) -> list[Table]:
        if len(kwargs) == 0:
            return []
        args = [cls.__dict__[param] == arg for param, arg in kwargs.items()]
        with sqlalchemy.app_context():
    ...
    @classmethod
    def bulk_insert(cls, values: list) -> BulkInsert:
        with sqlalchemy.app_context():
            inserted_records = []
            failed = []
    ...
    @classmethod
    def bulk_update(cls, values: list) -> BulkUpdate:
        with sqlalchemy.app_context():
            updated_ids = []
            failed = []
            for kwargs in values:
    ...
    @classmethod
    def bulk_delete(cls, items: list[Table]) -> BulkDelete:
        with sqlalchemy.app_context():
            deleted = []
            failed = []
            for item in items:
