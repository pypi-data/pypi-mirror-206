...
def command_injection_scanner(url, secure=True, tor=False, crawl=0, my_file=" ", parse=" ", delay=1):
    clear()
    ...
    for mal in url_mal_payloads:
        ...
        try:
            if tor:
                start = time.time()
                my_request = web_session.get(new_url, verify=False, headers={"User-Agent": return_user_agent()}, proxies=tor_proxy, timeout=(60,120)).text.lower()
                end = time.time()

            else:
                start = time.time()
                my_request = web_session.get(new_url, verify=False, headers={"User-Agent": return_user_agent()}, timeout=(5,30)).text.lower()
                end = time.time()

            alert = False
            if mal.startswith("echo") and "thesilent" in my_request:
    ...
    for mal in other_mal_payloads:
        ...
        try:
            alert = False
            if mal.startswith("echo") and "thesilent" in my_request:
                print(RED + f"True forms: {my_url} {mal}:{mal}")
                my_list.append(f"forms: {my_url} {mal}:{mal}")
                alert = True
                break
            ...
            if not alert:
                print(GREEN + f"False forms: {my_url} {mal}:{mal}")
