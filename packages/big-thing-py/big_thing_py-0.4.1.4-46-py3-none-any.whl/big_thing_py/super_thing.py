...
class MXSuperThing(MXBigThing):
    ...
    def req(self, sub_service_name: str, tag_list: List[str], arg_list: Union[Tuple[MXArgument], Tuple] = [],
        ...
        if not target_super_service.get_is_scanned():
            ...
            request_key = self._SUPER_SERVICE_REQUEST_KEY_TABLE[super_service_name][0]
            super_service_request_key = request_key['super_service_request_key']
            sub_service_request_key_list: List[str] = request_key['sub_request_key_list']
            sub_service_request_key = sub_service_request_key_list.pop(0)
            if len(sub_service_request_key_list) == 0:
                self._SUPER_SERVICE_REQUEST_KEY_TABLE[super_service_name].pop(0)

            super_service_execute_request = target_super_service._mapping_table[super_service_request_key]

            # for debug
            for k, v in super_service_execute_request._sub_service_request_table.items():
                MXLOG_DEBUG(f'k: {k}, v: {v}')
            MXLOG_DEBUG(f'sub_service_request_key: {sub_service_request_key}')

            try:
                subexecute_request_list = super_service_execute_request._sub_service_request_table[sub_service_request_key]._target_request_list
            except KeyError as e:
                MXLOG_DEBUG(f'KeyError: {sub_service_request_key}')
                MXLOG_DEBUG(f'super_service_name: {super_service_name}')
                MXLOG_DEBUG(f'sub_service_name: {sub_service_name}')
                raise e
