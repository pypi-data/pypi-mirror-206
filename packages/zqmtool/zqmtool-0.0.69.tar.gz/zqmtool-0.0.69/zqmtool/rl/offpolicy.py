...
TRAIN_SUCCESS_RATE_THRESHOLD = 0.95
...
def offpolicy_trainer(
    ...
    return gather_info(
        start_time, train_collector, test_collector, 0, 0
    )


def run_offpolicy_trainer(trainer, tester, config):
    offpolicy_trainer(
        tester.policy, tester.collector["train"], tester.collector["test"], config.epoch, config.step_per_epoch,
        config.step_per_collect,
        config.test_num, config.batch_size, update_per_step=1 / config.step_per_collect,
        train_fn=trainer.train_fn,
        test_fn=tester.test_fn,
        # stop_fn=lambda training_rew: training_rew > TRAIN_SUCCESS_RATE_THRESHOLD,
