import logging
from typing import Dict

import numpy as np
from numpy.typing import NDArray

from .base_objective_function import ObjectiveFunction
from ...methods.functionals.base_functional import Functional
from ...core.hashing import list_to_hash

logger = logging.getLogger(__name__)


class SquaredLoss(ObjectiveFunction):

    r"""Class object for obtaining the squared loss function and
    gradient from a given :attr:`functional`.

    Notes
    -----
    This loss function can be written as :math:`L(r(x, d)) = 0.5 r(x, d)^2`, where :math:`r` is the
    residual, a function of :math:`x`, the optimization coefficients, and :math:`d`, the data.
    The gradient with respect to :math:`x` is then :math:`\frac{\partial r}{\partial x}`.
    The partial derivative of :math:`r` with respect to :math:`x` is the responsibility
    of the :attr:`functional` to compute.

    Generally speaking, the squared loss function is easy to compute and has a well-behaved
    gradient, but it is not robust against outliers in the data. Using weights to normalize
    residuals by the variance can mitigate this somewhat.

    Parameters
    ----------
    functional : :class:`Functional`
        The functional instance from which the residuals, weights and gradient terms
        are obtained.
    use_weights : bool
        Whether to use weighting in the computation of the residual norm and gradient.
        Default is ``False``.
    objective_function_multiplier : float
        Multiplier for objective function. Can be useful if objective function is
        is less than 1, in which case some optimizers change their behaviour.
    """

    def __init__(self,
                 functional: Functional,
                 use_weights: bool = False,
                 objective_function_multiplier: float = 1):
        super().__init__(functional, use_weights, objective_function_multiplier)

    def get_residual_norm(self,
                          coefficients: NDArray = None,
                          get_gradient: bool = False) -> Dict:
        """ Gets the residual norm, and if needed,
        the gradient, using the attached :attr:`functional`.

        Parameters
        ----------
        coefficients
            A set of new coefficients. If given, the :attr:`functional`
            will be updated with new coefficients. Defaults to ``None``,
            i.e., the calculation with use the current coefficients.
        get_gradient
            Whether to return the gradient. Default is ``False``.

        Returns
        -------
             A ``dict`` with two entries, ``residual_norm`` and ``gradient``.
             If ``get_gradient`` is false, its value will be ``None``.
        """
        if coefficients is not None:
            self._functional.coefficients = coefficients

        functional_output = self._functional.get_residuals(
            get_gradient=get_gradient, get_weights=self._use_weights)
        residuals = functional_output['residuals']
        if self.use_weights:
            # weights (1/variance) need to be applied since they depend on the loss function
            residual_norm = np.einsum(
                'ijkh, ijkh, ijkh -> ...', residuals, residuals, functional_output['weights'])
        else:
            residual_norm = np.einsum(
                'ijkh, ijkh -> ...', residuals, residuals)

        residual_norm *= 0.5 * self.objective_function_multiplier

        if get_gradient:
            # weights already applied in previous step
            gradient = self.objective_function_multiplier * functional_output['gradient']
        else:
            gradient = None

        if residual_norm < 1:
            logger.warning(f'The objective function value ({residual_norm}) is < 1.'
                           ' Note that some optimizers change their convergence criteria for'
                           ' objective functions < 1!')

        return dict(residual_norm=residual_norm, gradient=gradient)

    def __hash__(self) -> int:
        to_hash = [hash(self._functional), self.use_weights, self._objective_function_multiplier]
        return int(list_to_hash(to_hash), 16)

    @property
    def _function_as_str(self) -> str:
        """ Should return a string representation of the associated loss function. """
        return 'L(r) = r ** 2'

    @property
    def _function_as_tex(self) -> str:
        """ Should return a string representation of the associated loss function
        in MathJax-renderable TeX."""
        return r'$L(r) = r^2$'
