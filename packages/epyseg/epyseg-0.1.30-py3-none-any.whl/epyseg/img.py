...
def get_voxel_conversion_factor(orig, return_111_if_none=True):
    ...
    try:
        if orig.metadata['vz'] is None or orig.metadata['vy'] is None or orig.metadata['vx'] is None:
            ...
            voxel_conversion_factor = (orig.metadata['vz'], orig.metadata['vy'], orig.metadata['vx'])
    ...
    # just a hack in case voxel size is not None --> assume isotropic
...
def _rotate_along_Z_axis(img):
    """
    just to create a set where z and y are flipped --> so that spots can be detected in the perpendicular direction better
    :param img:
    :return:
    """
...
def _recover_orig_after_rotation_along_z_axis(img):
    """
    the opposite of _rotate_along_Z_axis to recover an original file
    :param img:
    :return:
    """
...
def dilate_3D_spots_a_bit(single_channel_3D_mask,iterations=1):
    '''
    Small class to dilate a bit blobs/3D masks (do not go more than 2 iterations otherwise there will be big diamond artifacts)
    :param single_channel_3D_mask:
    :return:
    '''
...
def elastic_deform(image, displacement=None, axis=None, order=0, zoom=None, rotation=None, mode='reflect',
    ...
    if displacement is None:
        np.random.seed(random.randint(0,
                                      10000000))  # force numpy rand seed based on random seed so that if several instanaces are run and no parameter is passed the radom numpy deformation would still be the same
...
def get_ImageJ_ROIs(filename):
    from roifile import ImagejRoi
    rois = ImagejRoi.fromfile(filename)  # causes a bug but ROIs seem ok ???
    return rois
...
def get_threshold_value_corresponding_to_percentile(cum_hist, bins, percentile_values_to_recover):
    ...
    for val in percentile_values_to_recover:
        idx1 = np.argmax(cum_hist >= val)
        out.append(bins[idx1])
...
def get_histogram_density_and_cum_histo_density(single_channel_img):
    '''
    get the histo and bins using numpy
    :param single_channel_img:
    :return:
    '''
...
def auto_scale(img, individual_channels=True, min_px_count_in_percent=0.005):
    ...
    # TODO make sure there is no better way to obtain the histo in order to do the processing be much faster
    if len(img.shape) > 2 and individual_channels:
        ...
        # not very smart --> rather rely on histo generated by numpy to be more efficient
        # rng = mx - mn
        # px_count = img.size
        #
        ...
        # we compute the histo using numpy --> very efficient
        cum_hist, hist, bins = get_histogram_density_and_cum_histo_density(img)
        # we use the histo to get the thresholds
...
def fill_holes(img, fill_hole_below_this_size):
    ...
    if len(img.shape) == 3:
        ...
        mask = img > 0
        mask = remove_small_holes(mask, area_threshold=fill_hole_below_this_size, connectivity=1, out=mask) # small change because in place was removed from scipy
        img[mask != 0] = img.max()
    return img
...
def to_stack_channel(*images):
    # create a multichannel image from single channel ones --> should be easy
    return np.stack(tuple(images), axis=-1)

def to_stack(images):
    # this stuff is to create Z stacks
...
def create_random_liner_gradient(img, pos_h=None, off_centered=True, mock=False):
    '''

    :param img:
    :param pos_h:
    :param off_centered:
    :param mock: if mock is True --> returns the original image but calls random the same nb of time so that augmentation remains in sync
    :return:
    '''
...
def apply_2D_gradient(img, gradient2D):
    ...
    if gradient2D is None:
        # no gradient to apply --> return the original image
...
def pad_border_xy(img, dim_x=-2, dim_y=-3, size=1, mode='symmetric', **kwargs):
    # added kwargs to make it compatible to data augmenter of epyseg, by default it will ignore unknown parameters --> maybe should have a bool to set to ignore or warn on unknown kwargs
    if size <= 0:
...
class Img(np.ndarray):  # subclass ndarray
    ...
    def reassemble_tiles(tiles, crop_parameters, three_d=False):
        '''Reassembles tiles to a single image
