...
class MessageHandlerCallback(Protocol):
    def __call__(self, message_type: str, payload: Dict[str, object]) -> bool:
        pass
...
class MessageHandler(object):
    ...
    def deserialize_message(self, message: Message) -> Dict[str, object]:
        return self._schema.loads(message.value()['payload'])

    def handle(self, message_type: str, payload: Dict[str, object]) -> bool:
...
class IncomingMessageRouter(object):
    ...
    def register_handler(self, topic: str, schema: Schema, handler: MessageHandlerCallback) -> None:
        self._handlers[topic] = MessageHandler(schema, handler)
