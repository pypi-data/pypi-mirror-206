...
def sqlacodegen(url, schemas=None, table_names=None, file=None):
                    ...
                    for foreign_key in t.foreign_keys:
                        if hasattr(foreign_key, "column"):
                            ...
                            if child_field not in db_table_info_map[t.schema][child_table]:
                                foreign_key_params = {
                                    "column": fr'{parent_db}.{parent_table}.{parent_field}'
                                }
    ...
    for db, db_indo in db_table_info_map.items():
        for table, table_info in db_indo.items():
            ...
            class_class_name_str = f"""\n\nclass {to_upper_camel_case(table)}(Base):"""
            ...
            if relationship:
                for child_table, relationship_info in relationship.items():
                    ...
                    table_info["columns"].append(
                        {f'{child_table}': f"relationship('{to_upper_camel_case(child_table)}',"
                                           f" backref='{child_table}')",
                         "name": f'{child_table}', "is_relation_field": True})
