...
@LastEditTime: 2023-01-04 17:46:27
...
class IpBaseModel():
    ...
    def get_ip_info_list(self, app_id, act_id, page_size, page_index, is_del=0,is_release=-1, field="*", condition="", params=[], order_by="sort_index desc", is_cache=True, page_count_mode="total"):
        """
        :description: 获取IP列表
        :param app_id：应用标识
        :param act_id：活动标识
        ...
        if is_cache == True:
            page_list = IpInfoModel(context=self.context).get_cache_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=condition_where.to_string(), group_by="", order_by=order_by, params=params_list, dependency_key=DependencyKey.ip_info_list(act_id), cache_expire=600, page_count_mode=page_count_mode)
        else:
