...
def get_db_cache(cache_dir: Text) -> sqlite3.Connection:
    ...
    cursor = conn.cursor()
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS report_hash (
...
class URLCache:
    ...
    def __init__(
        ...
        requests_common_kwargs: Optional[Dict] = None,
    ) -> None:
    ...
    def query_cache(self, url: Text) -> CacheResponse:
        ...
        res = cursor.execute(
            "SELECT * FROM report_hash WHERE url = ?", [url.strip()]
        ).fetchall()
        ...
        return CacheResponse(
            res[0][1], res[0][2], datetime.datetime.utcfromtimestamp(res[0][3])
        )

    def update_cache(
        self, url: Text, status_code: Optional[int], report_hash: Optional[Text]
    ) -> None:
        ...
        if res:
            ...
            cursor.execute(
                "INSERT INTO report_hash VALUES (?,?,?,?)",
                [url, status_code, report_hash, time.time()],
            )
    ...
    def query_download_update(self, url: Text) -> Optional[Text]:
        ...
        if cache.report_hash and cache.added > now - datetime.timedelta(days=7):
            info(
                "URL in cache (with hash found): {}, {}, {}, {}".format(
                    url, cache.report_hash, cache.status_code, time.time()
                )
            )
        elif (
            (not cache.report_hash)
            and (cache.status_code == 404)
            and (cache.added > now - datetime.timedelta(days=7))
        ):
            info(
                "URL in cache (last status was 404): {}, {}, {}, {}".format(
                    url, cache.report_hash, cache.status_code, time.time()
                )
            )
        elif (not cache.report_hash) and (
            cache.added > now - datetime.timedelta(days=2)
        ):
            # status_code != 404 - attempt new download
            info(
                "URL in cache (Unknown status): {}, {}, {}, {}".format(
                    url, cache.report_hash, cache.status_code, time.time()
                )
            )
        else:
