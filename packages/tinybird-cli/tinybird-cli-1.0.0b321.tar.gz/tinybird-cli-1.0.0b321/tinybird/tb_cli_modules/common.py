...
from tinybird.tb_cli_modules.exceptions import CLIAuthException, CLIException, CLIWorkspaceException
...
async def print_branch_regression_tests_summary(client, job_id, host, response=None):
    ...
    for step in response['progress']:
        ...
        if run.get('metrics_summary') and run.get('metrics_timing'):
            column_names = [f"{run['pipe_name']}({run['test_type']})", 'Main', 'Branch', 'Delta']
    ...
    if failed:
        raise click.ClickException("Check failures for more information. If the result is expected disable asserts or increase thresholds")
