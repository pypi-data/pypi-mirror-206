...
class MXSuperThing(MXBigThing):
    ...
    def _request_subexecute(self, subexecute_request: MXSubExecuteRequest) -> None:
        ...
        MXLOG_DEBUG(f'[SUB_EXECUTE START] {subexecute_msg.sub_service_name}|{subexecute_msg.target_thing_name}|{subexecute_msg.target_middleware_name}', 'cyan')

    # 하나의 sub_service_request에 있는 subexecute_request들을 병렬로 실행한다.
    def _subexecute_parallel(self, subexecute_request_list: List[MXSubExecuteRequest], arg_list: List[Union[int, float, str, bool]]) -> List[Union[int, float, str, bool]]:
        result_list = []
        for subexecute_request in subexecute_request_list:
            subexecute_request.timer_start()
            subexecute_request.trigger_msg.arguments = tuple(arg_list)
        ...
        for subexecute_request in subexecute_request_list:
            ...
            MXLOG_DEBUG(
                f'[SUB_EXECUTE END] {subexecute_msg.sub_service_name}|{subexecute_msg.target_thing_name} duration: {subexecute_request.duration():.4f} Sec', 'cyan')
