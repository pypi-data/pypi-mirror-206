...
def translate_w_msk(mask, im):
    # Calculate the center of mass of the mask
    com = np.round(np.array([np.sum(np.arange(mask.shape[0]) * np.sum(mask, axis=1)),
                             np.sum(np.arange(mask.shape[1]) * np.sum(mask, axis=0))]) / np.sum(mask))
    # Calculate the translation needed to center the mask
    translate = np.array([mask.shape[0] // 2, mask.shape[1] // 2]) - com

    # Translate the mask
    translated_im = np.roll(np.roll(im, int(translate[0]), axis=0), int(translate[1]), axis=1)
    return translated_im

def get_foreground_mask(img, margin=0, convex=False, blur=False, translate=False):
    ...
    if translate:
        im_msk = translate_w_msk(bi_mask, im_msk)
