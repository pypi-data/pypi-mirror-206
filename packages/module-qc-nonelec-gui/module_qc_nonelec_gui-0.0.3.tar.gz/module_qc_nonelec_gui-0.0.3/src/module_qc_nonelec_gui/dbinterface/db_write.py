...
def init(db_cfg):
    ...
    url = "mongodb://{}:{}".format(db_cfg["hostIp"], db_cfg["hostPort"])
    logger.info(
        "[Connection Test] DB Server: {}/{}".format(
            url, db_cfg.get("dbName", "localdb")
        )
    )
    ...
    if db_tls or db_ssl:
        url += "/?ssl=true&ssl_ca_certs={}&ssl_certfile={}&ssl_match_hostname=false".format(
