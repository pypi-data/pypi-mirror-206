...
class MXSuperScheduleRequest(MXRequest):
    def __init__(self, trigger_msg: MXSuperScheduleMessage = None,
        ...
        # sub_request_key = make_sub_request_key(subschedule_result_msg.sub_service_name, subschedule_result_msg.subrequest_order)
        # sub_request_key: {sub_service_name}@{reqline_order}
    ...
    def put_sub_service_schedule_result_msg(self, subrequest_key: str, result_msg: MXSubScheduleResultMessage, timeout: float = None) -> bool:
        if subrequest_key not in self._sub_service_request_table:
            MXLOG_DEBUG(f'Could not find key {subrequest_key} in sub_service_request_table... This key is not mine!', 'yellow')
            return False

        target_sub_service_request = self._sub_service_request_table[subrequest_key]
        for candidate_request in target_sub_service_request._candidate_request_list:
            ...
            raise Exception(f'No sub_service {subrequest_key} found in _sub_service_request_table')

    def generate_subschedule_request(self, sub_service_request_list: List['MXSubServiceRequest'], hierarchical_function_service_table: List[MXFunction]):
        for sub_service_request in sub_service_request_list:
            ...
            # TODO: 해당 로직 부분 분리하기
            # 미들웨어 마다 reqline가 실행하고자 하는 sub_service이 존재하는 지 체크한다.
            ...
            for middleware_name in candidate_middleware_name_list:
                subschedule_msg = MXSubScheduleMessage(sub_service_name=target_sub_service.get_name(),
                                                       ...
                                                       subrequest_order=target_sub_service_request._sub_service_request_order,
                ...
                subschedule_request = MXSubScheduleRequest(trigger_msg=subschedule_msg,
                                                           sub_service=target_sub_service)
            ...
            subreqeust_key = make_sub_service_request_key(
                target_sub_service.get_name(), target_sub_service_request._sub_service_request_order)
            self._sub_service_request_table[subreqeust_key] = target_sub_service_request
            ...
            if len(target_sub_service_request._candidate_request_list) == 0:
                ...
                raise Exception(
                    f'No candidate sub_service found in key:{subreqeust_key} {self.trigger_msg.super_service_name} super service')
...
class MXSuperExecuteRequest(MXRequest):
    def __init__(self, trigger_msg: MXSuperExecuteMessage = None, super_schedule_request: MXSuperScheduleRequest = None) -> None:
        ...
        self._current_sub_service_request_cnt: int = 0
        self._running: bool = False
        ...
        if super_schedule_request:
            self.generate_subexecute_request_list(super_schedule_request=super_schedule_request)
    ...
    def put_sub_service_execute_result_msg(self, subrequest_key: str, result_msg: MXSubExecuteResultMessage, timeout: float = None) -> bool:
        if subrequest_key not in self._sub_service_request_table:
            MXLOG_DEBUG(f'Could not find key {subrequest_key} in sub_service_request_table... This key is not mine!', 'yellow')
            return False
        ...
        for target_request in target_sub_service_request._target_request_list:
            thing_check = (target_request.trigger_msg.target_thing_name == result_msg.target_thing_name)
            middleware_check = (target_request.trigger_msg.target_middleware_name == result_msg.target_middleware_name)
    ...
    def get_subexecute_result_msg_list(self, subrequest_key: str, timeout: float = None) -> List[MXSubExecuteResultMessage]:
        result_msg_list = []
        target_sub_service_request = self._sub_service_request_table[subrequest_key]
        for target_request in target_sub_service_request._target_request_list:
    ...
    def generate_subexecute_request_list(self, super_schedule_request: 'MXSuperScheduleRequest' = None) -> 'MXSuperExecuteRequest':
        # schedule 단계에서 MXSuperExecuteRequest를 생성한다.
        for subrequest_key, sub_service_request in super_schedule_request._sub_service_request_table.items():
            self._sub_service_request_table[subrequest_key] = MXSubServiceRequest(sub_service_type=sub_service_request._sub_service_type,
                                                                                  sub_service_request_order=sub_service_request._sub_service_request_order,
            ...
            self._sub_service_request_table[subrequest_key]._target_request_list = target_request_list
        return self


class MXSubScheduleRequest(MXRequest):
    def __init__(self, trigger_msg: MXSubScheduleMessage = None,
                 result_mqtt_msg: MXSubScheduleResultMessage = None, sub_service: MXFunction = None) -> None:
...
class MXSubExecuteRequest(MXRequest):
    def __init__(self, trigger_msg: MXSubExecuteMessage = None,
                 result_mqtt_msg: MXSubExecuteResultMessage = None, sub_service: MXFunction = None) -> None:
        super().__init__(trigger_msg, result_mqtt_msg)
...
class MXSubServiceRequest():
    ...
    def __eq__(self, o: 'MXSubServiceRequest') -> bool:
        ...
        subrequest_order_check = (o._sub_service_request_order == self._sub_service_request_order)

        return instance_check and sub_service_type_check and candidate_request_list_check and target_request_list_check and subrequest_order_check
...
def to_subexecute_request(subschedule_request: MXSubScheduleRequest) -> MXSubExecuteRequest:
    subschedule_msg: MXSubScheduleMessage = subschedule_request.trigger_msg
    subexecute_msg = MXSubExecuteMessage(subschedule_msg.sub_service_name,
                                         'SUPER',
                                         subschedule_msg.target_middleware_name,
                                         subschedule_msg.requester_middleware_name,
                                         subschedule_msg.super_thing_name,
                                         subschedule_msg.super_service_name,
                                         subschedule_msg.subrequest_order,
                                         subschedule_msg.scenario,
                                         request_ID=subschedule_msg.request_ID)
    subexecute_request = MXSubExecuteRequest(
        trigger_msg=subexecute_msg, sub_service=subschedule_request._sub_service)
    return subexecute_request


def to_subschedule_request(subexecute_request: MXSubExecuteRequest) -> MXSubScheduleRequest:
    subexecute_msg: MXSubExecuteMessage = subexecute_request.trigger_msg
    subschedule_msg = MXSubScheduleMessage(sub_service_name=subexecute_msg.sub_service_name,
                                           target_thing_name=subexecute_msg.target_thing_name,
                                           target_middleware_name=subexecute_msg.target_middleware_name,
                                           requester_middleware_name=subexecute_msg.requester_middleware_name,
                                           super_thing_name=subexecute_msg.super_thing_name,
                                           super_service_name=subexecute_msg.super_service_name,
                                           subrequest_order=subexecute_msg.subrequest_order,
                                           scenario=subexecute_msg.scenario,
                                           request_ID=subexecute_msg.request_ID)
    subschedule_request = MXSubScheduleRequest(
        trigger_msg=subschedule_msg, sub_service=subexecute_request.sub_service)
