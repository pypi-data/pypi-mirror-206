...
def validated_choice(choices, 
    ...
    *Dependencies*
    ...
    if long_choices == False:
        ...
        list_x2 = []
        for i in list_x:
            list_x2.append(Coordinates.to_pygame(global_,
                                                 x=i))
            
        coordinates = {
        'x':list_x2,
        'y':number*[Coordinates.to_pygame(global_,
                                          y=y)],
        'width_raw':(20-boxes_space*number)/number,
        'height_raw':distance_y,
        'width':Coordinates.to_pygame(global_, 
                                      distance_x=(20-boxes_space*number)/number),
        'height':Coordinates.to_pygame(global_, 
                                       distance_y=distance_y)
        }
    
        if number == 2:
            coordinates = {
        'x':[Coordinates.to_pygame(global_, 
                                   x=-5.1),
             Coordinates.to_pygame(global_, 
                                   x=0.6)],
        'y':number*[Coordinates.to_pygame(global_, 
                                          y=y)],
        'width_raw':4.4,
        ...
        list_y2 = []
        for i in list_y:
            list_y2.append(Coordinates.to_pygame(global_, 
                                                 y=i))
            
        coordinates = {
        'x':number*[Coordinates.to_pygame(global_, 
                                          x=x-width_raw/2)],
        'y':list_y2,
        'width_raw':width_raw,
        'height_raw':distance_y,
        'width':Coordinates.to_pygame(global_, 
                                      distance_x=width_raw),
        'height':Coordinates.to_pygame(global_, 
                                       distance_y=distance_y)
        }
    ...
    width_confirm = 4
    coordinates_validate = {'x': Coordinates.to_pygame(global_, 
                                                       x=width_confirm/2),
                            'y': Coordinates.to_pygame(global_, 
                                                       y=-5),
                            'width': Coordinates.to_pygame(global_, 
                                                           distance_x=width_confirm),
        ...
        for i in range(number):
            ...
            if write_choices is True:
                if overwrite_choices_display is None:
                    write(choices[i], global_,
                          x = Coordinates.from_pygame(global_, 
                                                      coordinates['x'][i]) + coordinates['width_raw']/2,
                          y = Coordinates.from_pygame(global_, 
                                                      y=coordinates['y'][i]) + coordinates['height_raw']/2,
                          color=choices_color,
                          size=choices_size)
                    
                if isinstance(overwrite_choices_display, list):
                    write(overwrite_choices_display[i], global_, 
                          x = Coordinates.from_pygame(global_, 
                                                      coordinates['x'][i]) + coordinates['width_raw']/2,
                          y = Coordinates.from_pygame(global_, 
                                                      y=coordinates['y'][i]) + coordinates['height_raw']/2,
        ...
        if title != None:
            if title_position == 'left':
                write(title, global_, 
                      x = title_x,
                      y = Coordinates.from_pygame(global_, 
                                                  y=coordinates['y'][0]) + coordinates['height_raw']/2)
            if title_position == 'top':
                write(title, global_, 
                      y = Coordinates.from_pygame(global_, 
                                                  y = coordinates['y'][0]) + title_space)
...
def choice(choices=["Yes","No"], 
    ...
    *Dependencies*
    ...
    if number == 2:
        coordinates = {
    'x':[Coordinates.to_pygame(x=-5.1),Coordinates.to_pygame(global_, 
                                                             x=0.6)],
    'y':number*[Coordinates.to_pygame(global_, 
                                      y=y)],
    'width_raw':4.4,
