...
class MtrxData(object):
        ...
        if id == 'DPXE':
            ...
            if s1:
                ...
                if s2 and s1.group(2) == 'STS_LOCATION':
                    ...
                    v = s1.group(3)
                key = id[::-1] + '::{0}.{1}'.format(s1.group(1), s1.group(2))
                if key in ['MARK::MTRX.FOREIGN_PARAM_ADDITION',
                           'MARK::MTRX.FOREIGN_PARAM_VALUE']:
                    (unit, data_type, value, prop) = v.split(';')
                    if data_type == 'Boolean':
                        ...
                        data = value
                    key1 = 'EEPA::-Foreign'
                    pseudo_key = '{0}.{1}'.format(key1, prop)
                    self.param[pseudo_key] = [data, unit]
                if key == 'MARK::MTRX.FOREIGN_PARAM_REMOVAL':
                    key1 = 'EEPA::-Foreign'
                    pseudo_key = '{0}.{1}'.format(key1, v)
                    if pseudo_key in self.param:
                        self.param.pop(pseudo_key)
                if (key in self.param and
                    key in ['MARK::MTRX.FOREIGN_PARAM_ADDITION',
                            'MARK::MTRX.FOREIGN_PARAM_VALUE',
                            'MARK::MTRX.FOREIGN_PARAM_REMOVAL']):
                    n = len([k for k in self.param if k.startswith(key)])
                    key = key + '_{0}'.format(n)
                self.param[key] = v
        else:
            self.param[id[::-1]] = ''
        return dp + dl
    ...
    def open(self, result_data_file):
        ...
        try:
            ...
            f.close()
            self.raw_param = b''
            chain_link = lambda i: '{0:s}_{1:04d}.mtrx'. \
                                   format(result_file_chain, i)
            link_nr = 1
            while os.path.exists(chain_link(link_nr)):
                with open(chain_link(link_nr), 'rb') as f:
                    if link_nr == 1:
                        self.raw_param = f.read()
                    else:
                        self.raw_param += f.read()[len(self._file_id):]
                link_nr += 1
        except:
            self.session = ''
            self.cycle = ''
