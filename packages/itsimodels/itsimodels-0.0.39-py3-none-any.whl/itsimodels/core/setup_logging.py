...
# Keeping this check for scenarios when itsicli is not found. Currently, itsicli is shipped in ITSI along with itsimodels 
try:
    from itsicli.setup_logging import extra
except:
    extra = {'cp_id': 'unknown'}
...
def setup_logging():
    ...
    if not _logger.hasHandlers():
        ...
        _logger.debug('itsimodels version={} logger configured. LOGGING_FILE_NAME={}'.format(__version__,
                                                                                  LOGGING_FILE_NAME))
    return _logger
