...
def get_logger(name: str = None) -> logging.Logger:
    parent_logger = logging.getLogger("chatcell")

    if name:
        # Append the name if given but allow explicit full names e.g. "chatcell.test"
        # should not become "chatcell.chatcell.test"
...
def setup_logging():
    ...
    if not any(isinstance(h, RichHandler) for h in logger.handlers):
        handler = RichHandler(
            ...
            markup=False,
            console=Console(width=settings.log_console_width),
        )
