...
class Stubber:
	...
	def report(A,filename='modules.json'):
		J('Created stubs for {} modules on board {}\nPath: {}'.format(M(A._report),A._fwid,A.path));F=v.format(A.path,filename);C.collect()
		try:
			with W(F,'w')as B:
...
def _info():
	i='ev3-pybricks';h='pycopy';e='GENERIC';c='arch';a='cpu';Z='ver';V='with';G='mpy';E='build';A=d({N:sys.implementation.name,D:B,E:B,Z:B,m:'stm32'if sys.platform.startswith(w)else sys.platform,K:e,a:B,G:B,c:B})
	...
	for (k,l,n) in [(h,h,'const'),(b,b,'FAT'),(i,'pybricks.hubs','EV3Brick')]:
		try:o=__import__(l,F,F,n);A[N]=k;del o;break
		except (Q,T):pass
