...
import re, json, pandas, pickle, os
...
class Runtime:
  ...
  def _makeAndUploadDataFiles(self, pyState: RuntimePythonProps):
    ...
    if pyState.extraSourceFiles:
      for nName, nSource in pyState.extraSourceFiles.items():
        dataFiles[nName] = nSource
...
def add_objects(deployment: str, values: Dict[str, Any]):
  ...
  """
  dataFiles: Dict[str, str] = {
      f"data/{name}.pkl": uploadRuntimeObject(val, pickle.dumps(val), name) for [name, val] in values.items()
  }
  return _changeFilesAndDeploy(deployment, dataFiles)
...
def add_files(deployment: str,
  ...
  for [localFilepath, modelbitFilepath] in files.items():
    ...
    with open(localFilepath, "rb") as f:
      data = f.read()
      dataFiles[modelbitFilepath] = uploadRuntimeObject(None, data, localFilepath) if shouldUploadFile(
          localFilepath, data) else data.decode("utf8")
  return _changeFilesAndDeploy(deployment, dataFiles)
