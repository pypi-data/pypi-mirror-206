...
def campoSinFiltro(c):
    filtros = ["__icontains", "__contains", "__startswith", "__istartswith", "__endswith", "__iendswith", "__range"]
    for f in filtros:
        if c.endswith(f):
            return False
    return True
        ...
        for c in campos:
            if campoSinFiltro(c):
                if cri.startswith("%") and cri.endswith("%"):
                    ...
                    if isPostgres:
                        filtros |= formarCondicion(c + "__search", cri.replace("%", ""))
                    c = c + "__icontains"
            filtros |= formarCondicion(c, cri.replace("%", ""))
    else:
