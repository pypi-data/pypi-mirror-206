...
class Label2Num:
    def __init__(self, mecab):
        ...
        self.model = LSTM_Model(num_classes, src_vocab_size, embed_size, hidden_size, num_layers, dropout, num_filters=32,
                           kernel_size=5)
        # self.model.load_state_dict(torch.load("KorDigits/results/save_model/CNN_LSTM_cls_refine_10.pt"))
        model_path = pkg_resources.resource_filename("KorDigits", "results/save_model/CNN_LSTM_cls_refine_10.pt")
    ...
    def _preprocess(self, sentence):
        ...
        if re.search(number_pattern, sentence):
            ...
            num_pattern = r"<N>.+?</N>"
            past_pos, past_word = "", ""
            for w, p in self.mecab.pos(sentence):
                if past_pos != "NNBC" and p == 'SN' and past_pos != 'SN':
                    sentence = sentence.replace(past_word + w, past_word + "<N>" + w, 1).replace(
                        past_word + " " + w, past_word + " <N>" + w, 1)
                elif "<N>" not in sentence and p == 'SN':
                    sentence = sentence.replace(w, "<N>" + w, 1)
                elif past_pos == 'NNBC' and p == 'SN':
                    sentence = sentence.replace(past_word + "</N>" + w, past_word + w, 1).replace(
                        past_word + "</N> " + w, past_word + " " + w, 1)
                if past_pos == 'SN' and p == 'NNBC':
                    sentence = sentence.replace(past_word + w, past_word + w + "</N>", 1)
                if p == 'SC':
                    continue
                past_pos, past_word = p, w
            num_list = re.findall(num_pattern, sentence)
            num_list = [x.replace("<N>", "").replace("</N>", "") for x in num_list]
            non_split_num = [x.replace(" ", "") for x in num_list]
            is_num = True
            sentences = sentence.replace("<N>", "").replace("</N>", "")
            # 결과를 저장할 리스트 초기화
            result = []
            for i in range(len(num_list)):
                sentence = sentences.replace(num_list[i], "<N>" + non_split_num[i] + "</N>")
                # 태그 안 다른 숫자 N으로 치환
                for j in range(i + 1, len(num_list)):
                    sentence = sentence.replace(num_list[j], "N")
                sentences = sentences.replace(num_list[i], "N")
                # 입력 문장을 어절 단위로 분리
                words = sentence.split()
                for j in range(len(words)):
                    if re.search(number_regex, words[j]):
                        # 현재 어절에 숫자가 있는 경우
                        start = max(j - 2, 0)
                        end = min(j + 3, len(words))
                        current = words[j][:].replace(non_split_num[i], num_list[i])
                        words[j] = current
    ...
    def generate(self, sentence):
        ...
        if is_num:
            for i in range(len(sentence_list)):
                with torch.no_grad():
                    tokenized_sentence = self.tokenizer.encode(sentence_list[i], add_special_tokens=True,
                                                               max_length=128,
                ...
                match = re.search(r"<N>(.*?)<\/N>", sentence_list[i])
                tagging = ""
                if match:
    ...
    def _label_0(self, tagging):
        ...
        if '시간' in output:
            ...
            if re.search(hour_regex, output):
                hour = re.search(hour_regex, output).group()[:-2]
            ...
            hour_regex = r"(\d+)시"
            if re.search(hour_regex, output):
                hour = re.search(hour_regex, output).group()[:-1]
                # 12시 이하
                if int(hour) <= 12:
                    hour_string = self.number_to_kor(hour)
                # 12시 이상
        ...
        if re.search(minute_regex, output):
            minute = re.search(minute_regex, output).group()[:-1]
            # '분' 십의 자리 숫자
            if minute == '0':
                minute_string = "영"
            else:
        ...
        if re.search(second_regex, output):
            second = re.search(second_regex, output).group()[:-1]
            if second == '00':
                second_string = "영"
            else:
        ...
        if month:
            for m in month:
                ...
                try:
                    ...
                    num_words = self.number_to_han(m)
                    output = output.replace(m, num_words, 1)
        if day:
    ...
    def _label_3(self, tagging):
        ...
        # if wave_match:
        #     wave_list = re.findall(r"(\d+)~(\d+)(\D+)", wave_match.group())
        #     if wave_list:
        #         start, end, char = wave_list[0]
    ...
    def _label_5(self, tagging):
        ...
        output = output.replace(".", '점', 1).replace("~", "에서 ").replace("+", "플러스").replace("-", " ").replace(":",
                                                                                                               "대")
        return output
...
if __name__ == '__main__':
    ...
    label2num = Label2Num()
    print(label2num.generate("20일에 축구로 2:0."))
