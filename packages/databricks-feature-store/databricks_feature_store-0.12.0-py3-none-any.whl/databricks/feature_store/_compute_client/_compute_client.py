...
class ComputeClient:
    ...
    def _create_table(
        ...
        *,
        ...
        # UC tables does not support tagging at the moment
        if is_uc_table and tags:
            raise uc_utils.tagging_for_uc_tables_error(name)
    ...
    def register_table(
        ...
        *,
        ...
        # UC tables does not support tagging at the moment
        if is_uc_table and tags:
            raise uc_utils.tagging_for_uc_tables_error(delta_table)
    ...
    def get_table(self, name: str, req_context: RequestContext) -> FeatureTable:
        ...
        if uc_utils.is_uc_entity(name):
            # tags for UC tables are always empty since we currently don't support setting tags for UC tables.
            tag_entities_dict = {}
            # get description from Delta table
            feature_table.description = self._spark_client.get_table_comment(name)
        else:
    ...
    def drop_table(self, name: str) -> None:
        ...
        if not feature_table_exists and not delta_table_exist:
            if not uc_utils.is_uc_entity(name):
                raise ValueError(f"Feature table '{name}' does not exist.")
            else:
                # For UC tables drop_table() should still work when the table is dropped from data explorer
                # or through DROP TABLE ... statement.
                # Feature catalog will return RESOURCE_DOES_NOT_EXIST in these cases due to UC ACL validation,
                # delete_feature_table() call should be proceeded regardless of such error.
                try:
                    self._catalog_client.delete_feature_table(name, req_context)
                except Exception as e:
                ...
                _logger.warning(
                    ...
                    "You must delete the online stores of the table (if any) separately with your cloud provider."
                )
                return
    ...
    def set_feature_table_tag(self, *, table_name: str, key: str, value: str) -> None:
        if uc_utils.is_uc_entity(table_name):
            raise uc_utils.tagging_for_uc_tables_error(table_name)
