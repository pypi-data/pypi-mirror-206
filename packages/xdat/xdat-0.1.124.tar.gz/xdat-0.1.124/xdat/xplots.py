...
def plot_confusion_matrix(y_true, y_pred, y_score=None, labels=None, title='', as_xpptx=None):
...
def plot_multi(df, kind=None, plot_func=None, x=None, y=None, var_name='variable', value_name='value', plot_on=None, group_on=None, color_on=None, cmap=None, label_on=None, style_on=None, size_on=None, figsize=(10,6), alpha=1.0, hdi_probs=(0.1, 0.25, 0.5, 0.8, 0.999), kde_cov=0.25, kde_percentile=None, hist_calc='perc', hist_type='step', hist_bins=10, hist_range=None, color_dict=None, title='', x_axis_type=None, y_axis_type=None, x_axis_fmt=None, y_axis_fmt=None, legend_loc='best', xlim=None, ylim=None, save_to=None, clear_folder=False, add_date=True, add_counts=True, plot_decorate_func=None, drop_na=True, as_xpptx=None, desc='', show=True, **kwargs):
    ...
    if kind in ['scatter']:
        label_counts = df.value_counts(label_on).reset_index()
    else:
        label_counts = df.drop_duplicates(group_on).value_counts(label_on).reset_index()
    ...
    for df_p, keys, plot_title in xpd.x_iter_groups(df, plot_on):
        ...
        for df_g, _, group_title in xpd.x_iter_groups(df_p, group_on):
            ...
            if label_on:
                ...
                if add_counts:
                    count = xpd.x_filter_by_keys(label_counts, sa_label)['count'].iloc[0]
                    label = f"{label} ({count})"
