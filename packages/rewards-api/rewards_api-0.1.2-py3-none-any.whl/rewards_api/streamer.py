...
class RewardsStreamer:
    ...
    def train(self):
        ...
        while True:
            ...
            text = f"Score: {score}  Record: {record} Trials: {agent.n_games}"
            font = cv2.FONT_HERSHEY_SIMPLEX
            font_scale = 0.7
            thickness = 2
            color = (255, 255, 255)
            text_size, _ = cv2.getTextSize(text, font, font_scale, thickness)
            text_position = (img.shape[1] - text_size[0] - 10, img.shape[0] - text_size[1] - 10)
            cv2.putText(
                img, text, 
                text_position, font, 
    ...
    def evaluate(self, mode = "training", track_num : int  = 1):
        ...
        while True:
            ...
            text = f"Score: {score}  Record: {record} Trials: {num_trials}"
            font = cv2.FONT_HERSHEY_SIMPLEX
            font_scale = 0.7
            thickness = 2
            color = (255, 255, 255)
