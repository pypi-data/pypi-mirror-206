    ...
    def run(self, result=None):
        with self._caplog.at_level(logging.DEBUG), mock_s3(), mock_organizations(), mock_sts(), closing(MockS3ManagerWithMockMongo()) as db, ExitStack() as es, freeze_time("2022-05-17"):
            ...
            versions = s3.list_object_versions(
                Bucket='arp-scale-2-cloud-bucket-with-tags11', Prefix=key1)
            self.__version1 = next(v['VersionId'] for v in sorted(
                versions['DeleteMarkers'], key=lambda x: x['LastModified'], reverse=True) if v['Key'] == key1)
