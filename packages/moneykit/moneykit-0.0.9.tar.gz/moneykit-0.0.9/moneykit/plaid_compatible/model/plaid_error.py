...
class PlaidError(ModelNormal):
    ...
    def openapi_types():
        ...
        return {
            ...
            'causes': ([bool, date, datetime, dict, float, int, list, str, none_type], none_type,),  # noqa: E501
    ...
    def __init__(self, error_type, error_code, error_message, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            causes ([bool, date, datetime, dict, float, int, list, str, none_type], none_type): In the Assets product, a request can pertain to more than one Item. If an error is returned for such a request, `causes` will return an array of errors containing a breakdown of these errors on the individual Item level, if any can be identified.  `causes` will only be provided for the `error_type` `ASSET_REPORT_ERROR`. `causes` will also not be populated inside an error nested within a `warning` object.. [optional]  # noqa: E501
