...
def parse_inline_table(src, pos, parse_float):
    # type(str, int, Callable[[str], type(any)])) -> tuple[Pos, dict]:
    pos += 1
...
def parse_basic_str_escape(src, pos, multiline= False):
    #type(str, int, bool) -> tuple[int, str]:
