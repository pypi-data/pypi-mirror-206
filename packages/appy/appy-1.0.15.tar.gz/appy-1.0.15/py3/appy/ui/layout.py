...
class Layouts(dict):
    ...
    types = (
      ...
      'sub'    , # the series of buttons rendered besides the object title on a
                 # list of objects ;
      'buttons', # the series of buttons rendered on a view/edit page ;
      'pre'    , # The zone in the page preamble, on object/view or edit ;
    ...
    restrictedTypesB = asDict(('sub', 'pre', 'query'))

    # Use this lambda as attribute field.renderable for a field that must be
    # renderable on layout "buttons" in addition to standard layouts.
    onButtons = lambda layout: layout not in Layouts.restrictedTypesB
