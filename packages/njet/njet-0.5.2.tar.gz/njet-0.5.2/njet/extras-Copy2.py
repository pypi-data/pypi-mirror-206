...
def _jbuild(jets):
    ...
    '''
    # This code attempts to work with 'jets' as iterable (in contrast to what has been done earlier)
    new_array0 = []
    new_array = []
    pos = 0
    all_keys = {} # will store the keys of a specific order (order mapped to set of keys)
    for j in jets:
        j0 = j.array(0)
        new_array0.append(j0)
        
        jmax_order_p1 = j.order + 1
        while len(new_array) < jmax_order_p1: # Create sufficient space to store data up to the order given by the jets
            new_array.append({}) # The dictionary will carry the liepoly terms
        
        for k in range(1, jmax_order_p1):
            jkt = getattr(j.array(k), 'terms', {})
            if len(jkt) == 0:
                continue
            
            keys_k = []
            terms_k = new_array[k] # terms_k is a dict
            for key, value in jkt.items():
                # We shall model the final array as dictionary at this stage: No keys means zeros, as we do not yet know the maximal length.
                if key not in terms_k.keys():
                    terms_k[key] = {pos: value}
                else:
                    terms_k[key][pos] = value
                keys_k.append(key)
            ...
            if k not in all_keys.keys():
                ...
                all_keys[k] = all_keys[k].union(keys_k)
                
        pos += 1
        
    del jets
    # now build the values to be used for the resulting jet:
    mk = all_keys.keys()
    result = jet(0, n=len(new_array) - 1)
    def result_array(k):
        if k == 0:
            return np.array(new_array0)
        elif k in mk:
            return jetpoly(terms={key: np.array([new_array[k].get(key, {}).get(pos, 0) for pos in range(pos)]) for key in all_keys[k]})
        else:
            return np.zeros(np.array(new_array0).shape)
    ...
    def cycle(self, *args, outf='default', **kwargs):
        ...
        **kwargs
        ...
        if outf == 'default':
            ...
            del cycling_data
            del compose_result
            gc.collect() # cleanup to reduce memory build-ups
            return result
        else:
            ...
            for l in range(len(new_functions)):
                ...
                evdat = []
                for icmp in range(n_args):
                    ff = (cycling_data[k][icmp] for k in range(len(ordering)) if ordering[k] == l)
                    evdat.append(_jbuild(ff))
                new_functions[l]._evaluation = evdat
            del cycling_data
            gc.collect() # cleanup to reduce memory build-ups
