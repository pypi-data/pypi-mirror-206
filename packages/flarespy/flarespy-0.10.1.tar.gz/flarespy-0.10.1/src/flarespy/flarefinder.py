...
class FlareLightCurve(lk.LightCurve):
    ...
    def _query_gaia_dr3_id(self):
        ...
        with warnings.catch_warnings():
            ...
            try:
                simbad_result = CUSTOM_SIMBAD.query_object(self.label)[0]
                id_list = simbad_result["IDS"].split("|")
                for i in id_list:
                    if "Gaia DR3" in i:
            ...
            if gaia_dr3_source_id == "":
                mast_result = Catalogs.query_criteria(catalog="TIC", ID=self.ticid)[0]
                gaia2_source_id = mast_result["GAIA"]
                if gaia2_source_id != "":
                    query = """
                    ...
                    ra = mast_result["ra"] * u.deg
                    dec = mast_result["dec"] * u.deg
                    pm_ra = mast_result["pmRA"] * u.mas / u.yr
                    pm_dec = mast_result["pmDEC"] * u.mas / u.yr
    ...
    def _query_gaia_dr3_params(self):
        ...
        if gaia_dr3_id != "":
            ...
            self.stellar_parameters["Gmag"] = np.round(result["Gmag"], 4)
            self.stellar_parameters["BP-RP"] = np.round(result["BP-RP"], 4)
