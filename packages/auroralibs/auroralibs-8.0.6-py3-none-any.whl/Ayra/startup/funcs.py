...
async def ajg():
    ...
    try:
        ...
        await ayra_bot.join_chat("@Kamarsnow")
...
async def autobot():
    ...
    if who.username:
        ...
        username = "aurora_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "aurora_" + (str(who.id))[6:] + str(ran) + "_bot"
        await ayra_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await ayra_bot.get_messages(bf, limit=1))[0].text
...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ayra_bot(
                CreateChannelRequest(
                    title="Aurora-Userbot Logs",
                    about="Jangan Keluar Dari Grup Log Ini !/nPowered By Aurora-Userbot",
                    megagroup=True,
                ),
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://graph.org/file/5e87b24315262edfb40db.jpg", "logo.jpg"
        )
...
async def customize():
    ...
    try:
        ...
        await ayra_bot.send_message(
            "botfather",
            f"✨ Powerful Aurora Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ © Aurora-Userbot ✨",
        )
        await asyncio.sleep(2)
        ...
        try:
            async for x in ayra_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
