...
class AbstractDataStorage(ABC):
    ...
    def add_node_type_where(self, query, type):  # pylint: disable=redefined-builtin
        if type in {"f", "file", "files"}:
            ...
            return query.where(self.nodes.c.dir_type.in_([DirType.DIR, DirType.ROOT]))
        return query
    ...
    def expand_path(self, path: str) -> List[Node]:
        ...
        if path.endswith("/"):
            res = [node for node in res if node.is_dir]
        return res
    ...
    def size(
        ...
        if isinstance(node, dict):
            is_dir = node.get("is_dir", node["dir_type"] in (DirType.DIR, DirType.ROOT))
            node_size = node["size"]
    ...
    def _find_query(
        ...
        if node.path_str:
            ...
            conds.append(n.c.path_str.op(self.glob_op)(self.compile_glob(sub_glob)))
