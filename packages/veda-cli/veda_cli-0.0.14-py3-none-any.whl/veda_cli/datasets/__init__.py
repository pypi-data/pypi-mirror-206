...
def download_dataset(dataset_name, destination_path):
    db_conn = get_db()
    datasets = db_conn.getBy({"type":"Dataset",  "name": dataset_name})
    if len(datasets) > 0:
        ds = datasets[0]
        for f in ds['files']:
            copy(ds["storageId"] + "/" + ds["base_path"] + "/" + f, "local-agent" + "/" + destination_path + "/" + f)
...
def copy(source, destination):
  ...
  source_storage_id, source_secret_id = ops.fetch_storage_and_secret_ids(source.split("/")[0])
  dest_storage_id, dest_secret_id = ops.fetch_storage_and_secret_ids(destination.split("/")[0])

  ## TODO : Check agent availability and deploy cloud agents if required

  file_list = []
  source_metadata = ops.get_resource_metadata(source)
  endpoint_paths = []
  total_volume = 0
  ...
  if (source_metadata.WhichOneof('metadata') == 'directory') :
    ...
    ops.flatten_directories(source_metadata.directory, "", file_list)
