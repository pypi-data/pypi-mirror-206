...
from ..services.mapping_process import MappingProcess
...
class DataMigration(APIView):
    ...
    def list_col(self):
        ...
        data, err_txt = DataConsolidation.list_col(localhost, port, username, password, database, table_name)
        if not err_txt:
            return util_response(data=data)
        return util_response(err=47767, msg=err_txt)

    def mapping_rocessing(self):
        data, err_txt = MappingProcess.handle().delay()
        if not err_txt:
            return util_response(data=data)
        return util_response(err=47767, msg=err_txt)
