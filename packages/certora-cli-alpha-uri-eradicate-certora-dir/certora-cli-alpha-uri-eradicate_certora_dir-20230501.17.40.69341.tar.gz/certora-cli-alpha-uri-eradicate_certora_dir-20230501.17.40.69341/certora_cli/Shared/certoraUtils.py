...
def find_jar(jar_name: str) -> Path:
    """
    Searches for a jar file with the given name and returns the absolute path to it, which might or might not exist

    :param jar_name: A string that specifies the name of the jar file to be searched for.

    :returns: A `Path` object that represents the absolute path to the jar file. If the jar file is found in the `bin`
              directory (for dev users), the function returns the absolute path to that jar file. Otherwise, it returns
              the absolute path to the jar file located in the `certora_jars` directory (for customers).
    """

    jar_path_prefix = Path(__file__).parents[2]
    if jar_path_prefix.name != "EVMVerifier":
        jar_path_prefix = Path(__file__).parents[1]
        if jar_path_prefix.name != "repo":  # CI
            raise RuntimeError(f"Ran script from an unexpected location, cannot locate jar files: {__file__}")
    jar_path_suffix = CERTORA_JARS / jar_name

    jar_in_bin_path = jar_path_prefix / "bin" / jar_path_suffix
    if jar_in_bin_path.is_file():
        resource_path = jar_in_bin_path
    else:
        resource_path = jar_path_prefix / jar_path_suffix
