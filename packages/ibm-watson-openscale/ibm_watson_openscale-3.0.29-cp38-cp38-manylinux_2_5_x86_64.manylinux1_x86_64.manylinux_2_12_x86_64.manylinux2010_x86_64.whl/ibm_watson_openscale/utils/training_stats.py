...
class TrainingStats():
    ...
    def __validate_training_data_info(self):
        ...
        if self.probability is None:
            self.probability = self.training_data_info.get("probability")

        if self.problem_type != "regression" and not self.probability:
            raise Exception("'probability_column' cannot be empty for classification models")
        self.class_probabilities = self.training_data_info.get("class_probabilities")

        if self.problem_type != "regression" and self.class_probabilities:
            probability = [self.probability] if isinstance(self.probability, str) else self.probability
            if not (set(self.class_probabilities) <= set(probability)):
                raise Exception("'class_probabilities' and 'probability_column' must have same values.")
    ...
    def __generate_output_data_schema(self, input_data_schema):
        if not self.prediction or not self.probability:
            return None
        ...
        if len(probability) > 1:
            # always do this in case we have an array of more than 1 value
            add_as_array = False
        elif not self.class_probabilities:
            # probability array is of size 1 and class_probabilities is not defined
            # in such a case, this must be added as array of double
            add_as_array = True
