...
def create_config_file(resource_names,config_changes):
    """
...
def store_config_file(config_data):
    """
...
def generate_audit_report(service: dict, timestamp: str) -> None:
    ...
    try:
        for service_name, service_id in service.items():
            env = Environment(
                loader=FileSystemLoader(pkg_resources.resource_filename('dna_audit_tool.common', 'templates')),
                autoescape=select_autoescape(['html', 'xml'])
            )
            print(pkg_resources.resource_filename('dna_audit_tool.common', 'templates'))
            template = env.get_template('audit_report.html')
            print(template,"template is here")
            html = template.render(check_results=service_id, service=service_name)

            service_name = service_name.lower()
            file_name = f"reports/{timestamp}/aws_services/{service_name}.html"
            os.makedirs(f"reports/{timestamp}/aws_services", exist_ok=True)
        ...
        index_file_name = f"reports/{timestamp}/index.html"
