...
def single_snp_scale(
    ...
    min_work_count=1,
    gtg_runner=None,
    gtg_min_work_count=None,
    svd_runner=None,
    postsvd_runner=None,
    postsvd_min_work_count=None,
    test_snps_runner=None,
    test_snps_min_work_count=None,
    count_A1=False,
    ...
    * 4: TestSNPs - For each test SNP, read its data, regress out covariates, use the appropriate U and compute a Pvalue.
    ...
    with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
        ...
        chrom_list, pheno1, RxY, test_snps1, X, Xdagger, G0 = preload(
            covar, G0, pheno, test_snps, count_A1=count_A1, multi_pheno_is_ok=True
        )
        cache_dict = _cache_dict_fixup(cache, chrom_list)

        G0_memmap_lambda, ss_per_snp = get_G0_memmap(
            G0, cache_dict[0], X, Xdagger, memory_factor
        )

        gtg_npz_lambda = get_gtg(
            cache_dict[0],
            G0.iid_count,
            G0.sid,
            G0_memmap_lambda,
            memory_factor,
            gtg_runner,
            min_work_count=gtg_min_work_count,
        )
        ...
        frame = do_test_snps(
            ...
            X,
            Xdagger,
            test_snps=test_snps1,
            runner=test_snps_runner,
            output_file_name=output_file_name,
            min_work_count=test_snps_min_work_count,
        )
    ...
    if not os.path.exists(merge_fn):
        snpreader = _MergeIIDs(
            [
                ...
                for file_index in range(file_index_end)
            ]
        ).read()
...
def read_bed_cache(data_folder, file_index_end):
    ...
    if os.path.exists(snps_iid_list_fn):
        ...
        iid_list = [
            bed.iid for bed in bed_list(data_folder, snp0, file_index_end, None)
        ]
...
def _create_dataframe(row_count):
    ...
    # Not currently calculated: dataframe['EffectSize'] = dataframe['EffectSize'].astype(float)
...
def preload(covar, G0, pheno, test_snps, count_A1, multi_pheno_is_ok=False):
    ...
    if pheno.sid_count == 1:
        ...
        assert not np.isnan(
            ...
            "When multiple phenotypes are given, they must not have any missing values."
        )
    ...
    G0, test_snps, pheno, covar = pstutil.intersect_apply(
        [G0, test_snps, pheno, covar]
    )  #!!!is this time well spent?

    X = np.c_[
        covar.read(view_ok=True, order="A").val, np.ones((covar.iid_count, 1))
...
def get_clear_local(file_cache):
    ...
    def clear_local_lambda():
        for local_root in set(
            storage.local_lambda()[1] for storage in file_cache.values()
        ):
...
def get_G0_memmap(G0, file_cache_parent, X, Xdagger, memory_factor):
    ...
    assert file_cache.file_exists(fn_G0_memmap) == file_cache.file_exists(
        fn_ss
    ), "expect '{0}' and '{1}' to either both exist or neither".format(
        fn_G0_memmap, fn_ss
    )

    work_count2 = -(
        G0.iid_count // -int(G0.sid_count * memory_factor)
    ...
    with _file_transfer_reporter(
        "G0_data.memmap upload", size=0, updater=None
    ) as updater2:
        with file_cache.open_write(
            fn_G0_memmap, size=8 * G0.iid_count * G0.sid_count, updater=updater2
        ) as G0_memmap_storage_file_name:
            G0_data_memmap = SnpMemMap.empty(
                iid=G0.iid,
                sid=G0.sid,
                filename=G0_memmap_storage_file_name,
                pos=G0.pos,
                order="F",
            )  #!!!is this the best order? dtype default ok?
            ...
            for work_index in range(work_count2):
                if work_count2 > 1:
                    logging.info(
                        "get_G0_data_memmap: Working on part {0} of {1}".format(
                            work_index, work_count2
                        )
                    )
                ...
                ss_per_snp[start:stop] = np.einsum(
                    "i...,i...", G0_data_piece.val, G0_data_piece.val
                )  # Find the (G0_data.val**2).sum(0) without memory allocation
            ...
            logging.info(
                "About to SnpMemMap.close G0_data_memmap in '{0}'".format(
                    G0_memmap_storage_file_name
                )
            )
            ...
            t3 = time.time()
            logging.info(
                "done with SnpMemMap.close G0_data_memmap in {0}".format(
                    format_delta(t3 - t2)
                )
            )
...
def get_gtg(
    ...
    work_count = -(
        G0_iid_count // -int(G0_sid_count * memory_factor)
    )  # Find the work count based on batch size (rounding up)
    work_count = max(
        min_work_count, work_count
    )  # Always divide the work into at least min_work_count parts
    work_count = min(
        work_count, len(G0_sid)
    )  # Don't divide the work more than the number of test SNPs

    t0_gtg = time.time()
    name = "{0}.get_gtg".format(
        os.path.basename(common_cache.name)
    )  #!!! little bug: if tree_cache.name ends with "/" or "\" doesn't work
    mmultfile_ata(
        G0_memmap_lambda,
        writer_closure,
        G0_sid,
...
def get_U(chrom, chrom_cache):
    ...
    assert chrom_cache.file_exists(fn_U) == chrom_cache.file_exists(
        fn_UUYetc
    ), "expect '{0}' and '{1}' to either both exist or neither".format(fn_U, fn_UUYetc)
...
def apply_h2(
    h2, S, UYUY, UUYUUYsum0, N, k
):  #!!!make more matrix like? (or multiproc???)
    result_list = []
    for pheno_index in range(len(UUYUUYsum0)):
        result = apply_h2_inner(
            h2[pheno_index], S, UYUY[:, [pheno_index]], UUYUUYsum0[[pheno_index]], N, k
        )
...
def apply_h2_inner(h2, S, UYUY, UUYUUYsum0, N, k):
    Sd = h2 * S + (1.0 - h2)  # 25K
    denom = 1.0 - h2
    YKY = (UYUY / Sd.reshape(-1, 1)).sum(
        0
    ) + UUYUUYsum0 / denom  # Can be done in blocks
...
def find_h2(k, N, UUYUUYsum0, UYUY, S, chrom):
    h2_list = []
    for pheno_index in range(len(UUYUUYsum0)):
        h2_inner = find_h2_inner(
            k, N, UUYUUYsum0[[pheno_index]], UYUY[:, [pheno_index]], S, chrom
        )
...
def find_h2_inner(k, N, UUYUUYsum0, UYUY, S, chrom):
    ...
    # 25K x about 30
    # =============================================================
    _ = minimize1D(f=f, nGrid=10, minval=0, maxval=0.99999)
    h2 = resmin[0]["h2"]
...
def svd(
    ...
    needed_chrom_list = [
        ...
        if not common_cache.file_exists("done{0}.txt".format(int(chrom)))
    ]
    ...
    def mapper_closure(chrom):
        ...
        logging.info(
            "About to svd on square {0}. Expected time ({2} procs)={1}".format(
                ata.iid_count,
                format_delta((ata.iid_count * 0.000707) ** 3 * 20.0 / num_threads),
                num_threads,
            )
        )
        t0 = time.time()
        ...
        logging.info(
            "Actual time for svd on square={0}".format(format_delta(time.time() - t0))
        )
...
def postsvd_piece(
    ...
    with open(G0_memmap.filename, "rb") as fp:
        ...
        for start_idx, stop_idx in idx_array:
            start_idx, stop_idx = int(start_idx), int(
                stop_idx
            )  # We convert from numpy.int32 to python ints so that their product will be a python long instead of a (overflowed) numpy.int32
            fp.seek(
                G0_memmap.offset + start_idx * G0_memmap.iid_count * 8
            )  # Skip sids that are not of interest
            for sid_index in range(so_far_idx, so_far_idx + stop_idx - start_idx):
                if log_frequency > 0 and sid_index % log_frequency == 0:
                    logging.info(
                        "on sid_index {0} of {1}".format(sid_index, SVinv3b.shape[0])
                    )
                row = np.fromfile(
                    fp, dtype=np.float64, count=G0_memmap.iid_count
    ...
    logging.info(
        "post svd piece: clocktime {0}".format(format_delta(time.time() - t0_piece))
    )
...
def postsvd(
    ...
    needed_chrom_list = [
        chrom
        for chrom in chrom_list
        if not common_cache.file_exists("chrom{0}/done.txt".format(int(chrom)))
    ]
    if not needed_chrom_list:
        return
    ...
    def mapper_closure(chrom):
        ...
        def mapper_closure_inner(work_index):
            ...
            idx_intrangeset = IntRangeSet(
                i for i, keep in enumerate(idx) if keep
            )  # IntRangeSet('285:1000')
            ...
            if work_count > 1:
                logging.info(
                    "post svd, chrom {0}: Working on part {1} of {2}".format(
            ...
            fn_U_done = "chrom{0}/U_pieces/done{1}_{2}.txt".format(
                chrom, work_index, work_count
            )
            fn_U_piece = "chrom{0}/U_pieces/{1}_{2}.memmap".format(
                chrom, work_index, work_count
            )
            chrom_cache = cache_dict[chrom].join(sub_dir)
            if chrom_cache.file_exists(fn_U_done):
                logging.info(
                    "Piece '{0}' already exists, so skipping work".format(fn_U_piece)
                )
            else:
            ...
            return (
                ...
                start_iid_index,
                stop_iid_index,
                len(sid) if work_index == 0 else None,
                S if work_index == 0 else None,
            )

        def reducer_closure_inner(fn_U_piece_sequence):
            if clear_local_lambda is not None:
                logging.info(
                    "To save space, removing all cached files from local drive"
                )
                clear_local_lambda()

            t0_start = time.time()
            logging.info(
                "Starting postsvd reduce for chrom '{0}' with file downloads".format(
                    chrom
                )
            )

            fn_U_piece_list = list(
                fn_U_piece_sequence
            )  # make it a list instead of generator, so can run through it twice
            _, _, _, sid_count, S = fn_U_piece_list[
                0
            ]  # Get sid_count and S from the first work item. We pass these this way to avoid bring in files to the local machine (if run with "map_reduceX") or to the reduce
            ...
            logging.info(
                "File downloads took {0}. Next putting U together".format(
            ...
            with chrom_storage.open_write(fn_U) as handle_fn_U_file_name:
                ...
                U_snp_mem_map = SnpMemMap.empty(
                    iid=G0_iid,
                    sid=sid,
                    filename=handle_fn_U_file_name,
                    dtype=np.float64,
                    order="F",
                )
                fp_list = (
                ...
                for piece_index, (
                    ...
                    _,
                    _,
                ) in enumerate(fn_U_piece_list):
                    with chrom_storage.open_read(
                        fn_U_piece
                    ) as handle_local:  # The local file is used after this is closed. Not nice, but OK in this case.
                        logging.info(
                            "About to open piece {0} of {1}".format(
                                piece_index, len(fn_U_piece_list)
                            )
                        )
                ...
                with open(U_snp_mem_map.filename, "r+b") as fp_U:
                    ...
                    with log_in_place("Creating U file", logging.INFO) as log_writer:
                        for sid_index in range(sid_count):
                            if (
                                sid_index % 100 == 0
                            ):  #!!!use something like log_freq instead of '100'
                                log_writer(
                                    "On sid {0} of {1}".format(sid_index, sid_count)
                                )
                            prev_stop = 0
                            for piece_index, (
                                fn_U_piece,
                                ...
                                buf2 = np.fromfile(
                                    fp,
                                    dtype=np.float64,
                                    count=stop_iid_index - start_iid_index,
                                )
                                buf2.tofile(fp_U)
                ...
                t0_U = time.time()
                logging.info(
                    "Putting U together took {0}. Next creating UY & UUY files".format(
                        format_delta(t0_U - t0_download)
                    )
                )
                ...
                # =============================================================
                UY = U_snp_mem_map.val.T.dot(
                    RxY
                )  # Note: This could be pushed into the 'map' step, with each step returning a UYi that would all be summed together.
                # =============================================================
                # 1M x 25K x 2 -> 1M x 2
                # =============================================================
                UUY = RxY - U_snp_mem_map.val.dot(
                    UY
                )  # This is can't be pushed into the 'map' step because it depends on all of UY
            ...
            t0_etc = time.time()
            logging.info(
                "Creating related files took {0}. Next uploading files.".format(
                    format_delta(t0_etc - t0_U)
                )
            )
            ...
            logging.info(
                "Postsvd Reduce Summary: Down {0}, Calc {1}, Up {2}, Total {3}".format(
                    format_delta(t0_download - t0_start),
...
def get_U_h2(
    ...
    # 1M x 25K x 25K => 1M x 25K
    # =============================================================
    S, U_memmap, UY, UUY = get_U(
        chrom, chrom_cache
    )  #!!!y: at the end, U is multiplied twice with yish things
    ...
    k = S.shape[0]
    h2, logdetK, YKY, Sd, denom = get_h2(
        k, N, UUYUUYsum0, UYUY, S, chrom_cache, chrom
    )  #!!!y: h2 depends on y
...
def do_test_snps(
    ...
    def mapper_closure2(chrom):
        ...
        logging.info(
            "test_snps_chrom({1}) has sid_count of {0}".format(
                test_snps_chrom.sid_count, chrom
            )
        )

        work_count = int(
            -(-test_snps_chrom.iid_count // (G0_sid_count * memory_factor))
        )  # Do blocks based on the size of G0
        work_count = max(
            min_work_count, work_count
        )  # Always divide the work into at least min_work_count parts
        work_count = min(
            work_count, test_snps_chrom.sid_count
        )  # Don't divide the work more than the number of test SNPs
        ...
        def mapper_closure(work_index):
            ...
            cache_file = "chrom{0}/result.{1}of{2}.tsv".format(
                chrom, work_index, work_count
            )
            ...
            def debatch_closure(work_index):
                ...
                logging.debug(
                    "chrom={0},work_index={1},start={2},test_snps_chrom.sid_count={3}".format(
                        chrom, work_index, start, test_snps_chrom.sid_count
                    )
                )
                return start

            h2, U_memmap, Sd, denom, UY, UUY, YKY, N, logdetK = get_U_h2(
                chrom,
                gtg_npz_lambda,
                memory_factor,
                chrom_storage,
            ...
            if work_count > 1:
                logging.info(
                    "single_low_snp: Working on snp block {0} of {1}".format(
                        work_index, work_count
                    )
                )
            start = debatch_closure(work_index)
            logging.debug(
                "A:chrom={0},work_index={1},start={2}".format(chrom, work_index, start)
            )
            stop = debatch_closure(work_index + 1)
            logging.debug(
                "A:chrom={0},work_index+1={1},end={2}".format(
                    chrom, work_index + 1, stop
                )
            )
            t0_gen = time.time()
            snps_read = test_snps_chrom[:, start:stop].read().standardize()
            logging.info(
                "test snp reader {0} of {1}, clocktime {2}".format(
                    work_index, work_count, format_delta(time.time() - t0_gen)
                )
            )
            logging.debug(
                "Xdagger {0}x{1}. snp_read {2}x{3}".format(
                    Xdagger.shape[0],
                    Xdagger.shape[1],
                    snps_read.iid_count,
                    snps_read.sid_count,
            ...
            logging.debug(
                "U_data {0}x{1}. Rxsnps {2}x{3}. time={4}".format(
                    U_memmap.iid_count,
                    U_memmap.sid_count,
                    Rxsnps.shape[0],
                    Rxsnps.shape[1],
            ...
            Usnps, UUsnps = mmultfile_b_less_aatb(
                U_memmap, Rxsnps, log_frequency=log_frequency
            )
            logging.debug(
                "U_data {0}x{1}. Usnps {2}x{3}. time={4}".format(
                    U_memmap.iid_count,
                    U_memmap.sid_count,
                    Usnps.shape[0],
                    Usnps.shape[1],
            ...
            for pheno_index in range(
                len(h2)
            ):  #!!!make more matrix like? (or multiprocess???)
                # ==============================================================
                # 25K x 4M -> 4M
                # ==============================================================
                logging.debug(
                    "time={0}".format(datetime.now().strftime("%Y-%m-%d %H:%M"))
                )
                logging.debug(
                    "snpsKsnps = ((Usnps / Sd.reshape(-1,1) * Usnps).sum(0) + (UUsnps * UUsnps).sum(0) / denom).reshape(-1,1) #Can be done in blocks"
                )
                snpsKsnps = (
                    (Usnps / Sd[:, [pheno_index]] * Usnps).sum(0)
                    + UUSnpsUUSnps_sum0 / denom[pheno_index]
                ).reshape(
                    -1, 1
                ...
                UAS = Usnps / np.lib.stride_tricks.as_strided(
                    Sd[:, [pheno_index]],
                    (Sd.shape[0], Usnps.shape[1]),
                    (Sd.itemsize, 0),
                )
                # ==============================================================
                ...
                snpsKY = (
                    UAS.T.dot(UY[:, [pheno_index]])
                    + UUsnps.T.dot(UUY[:, [pheno_index]]) / denom[pheno_index]
                )
                # ==============================================================
                # 4M
                # ==============================================================
                ...
                dataframe_inner = compute_stats(
                    ...
                    snps_read.pos,
                    h2[pheno_index],
                    covar_bias_count=Xdagger.shape[0],
                    pheno_or_none=pheno_or_none,
                )
        ...
        frame = map_reduce(
            range(work_count),
            mapper=mapper_closure,
            name="{0}.test_snps_{1}".format(
                os.path.basename(cache_dict[0].name), chrom
            ),
    ...
    frame = map_reduceX(
        ...
        name=lambda index: name_prefix
        + str(int(chrom_list[index])),  # Create a job name based on the chrom number
        runner=runner,
    )
...
def clear_cache_dict(start_stage, cache_dict, chrom_num_list):
    ...
    with log_in_place(
        "clearing cache dict to '{0}'".format(start_stage), logging.INFO
    ) as log_writer:
...
def _clear_cache_dict_internal(start_stage, cache_dict, chrom_num_list, log_writer):
    if start_stage == "all":
        _clear_cache_dict_internal(
            "gtg", cache_dict, chrom_num_list, log_writer
        )  # Recursion
        ...
        _clear_cache_dict_internal(
            "svd", cache_dict, chrom_num_list, log_writer
        )  # Recursion
        ...
        _clear_cache_dict_internal(
            "postsvd", cache_dict, chrom_num_list, log_writer
        )  # Recursion
        ...
        _clear_cache_dict_internal(
            "testsnps", cache_dict, chrom_num_list, log_writer
        )  # Recursion
...
def _cache_dict_fixup(cache_dict, chrom_list):
    ...
    if isinstance(cache_dict, Mapping):
        return {
            k: FileCache._fixup(v, default_subfolder="single_snp_scale")
...
def map_reduceX(input_seq, mapper=_identity, reducer=list, runner=None, name=None):
    ...
    if hasattr(runner, "run_list"):
        ...
        result_list = [
            runner.run(distributable) for distributable in distributable_list
        ]
