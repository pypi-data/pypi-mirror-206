...
EXPORT const K k_has_pykx_flag(K x) {
    return k(0, "\"insights.lib.pykx\" in \" \" vs .z.l 4", NULL);
}
...
K lic_check() {
    K k = (K)0;
    if (pykx_flag != 1) {
        // This is ugly but it only does one check if you are licensed
        if (pykx_flag == -1){
            pykx_flag = (int)(k_has_pykx_flag((K) 0)->g);
            if (pykx_flag != 1) {
                k = ks("License does not support use of pykx");
                k->t = -128;
                return k;
            }
        } else {
...
static P k_to_py_cast(K x, K typenum) {
    PyGILState_STATE gstate;
    gstate = PyGILState_Ensure();
    if (x->t == 112) {
        return get_py_ptr(x);
...
EXPORT K k_to_py_foreign(K x, K typenum) {
    K k;
    PyGILState_STATE gstate;
    gstate = PyGILState_Ensure();
    P p = k_to_py_cast(x, typenum);
    if ((k = k_py_error())) {
        PyGILState_Release(gstate);
        return k;
    }
