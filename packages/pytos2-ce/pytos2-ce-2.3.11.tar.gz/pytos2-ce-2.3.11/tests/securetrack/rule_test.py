...
from pytos2.securetrack.rule import SecurityRule, Track, PolicyXsiType
...
class TestRule:
    ...
    def test_attributes(self, rules_20):
        ...
        assert type(rule.track) == Track
        assert rule.track.level == Track.Level.LOG
        ...
        assert type(rule.src_networks) == list
        assert type(rule.dest_networks) == list
