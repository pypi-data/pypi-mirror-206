...
<xsl:template match="text:a">
    <ref>
        <xsl:attribute name="target">
            <xsl:choose>
                <!-- référence interne -->
                <xsl:when test="starts-with(@xlink:href,'#') and contains(@xlink:href,':')">
...
<xsl:template match="text:bookmark-start">
    <anchor xml:id="{replace(@text:name,':','_')}"/>
</xsl:template>
    
<xsl:template match="text:bookmark-ref">
<!--    <anchor xml:id="{@text:ref-name}"/>-->
    <ref target="{concat('#',@text:ref-name)}"><xsl:apply-templates/></ref>
</xsl:template>
