...
from tinybird.client import (MAX_GET_LENGTH, AuthException,
...
class TestCLIPipeCopy(TestCLI):
    @tornado.testing.gen_test
    async def test_pipe_set_copy__should_set_a_node_to_copy_data(self) -> None:
        with User.transaction(self.WORKSPACE_ID) as w:
            w.feature_flags[FeatureFlagWorkspaces.EXPORT.value] = True
        await self._auth()
        ...
        # Unpublish endpoint to force the copy later
        ...
        output = await self._tb(["pipe", "set_copy", pipe_name, "--target-datasource", target_ds_name])
        expected = [
            FeedbackManager.success_node_copy(pipe=pipe_name, host=self.host)
        ]
        self._assert_feedback_output(expected, output)
    ...
    async def test_pipe_copy__should_run_a_copy_job(self) -> None:
        ...
        output = await self._tb(["pipe", "copy", pipe_name, "--yes", "--wait"])
...
def mock_bq_cdk_conn():
    ...
    with patch(
        "tinybird.views.api_datasources_import.get_connector", new_callable=AsyncMock
    ) as mock_get_conn:
        with patch(
...
IMPORT_SCHEDULE '@once'
            ...
            with patch("tinybird.ingest.external_datasources.connector.CDKConnector.create_stage", return_value={'stage': 'sf_stage', 'gcp_account': 'gcp_account'}):
                with patch("tinybird.views.api_datasources_import.grant_bucket_write_permissions_to_account", return_value=None):
...
IMPORT_EXTERNAL_DATA_SOURCE 'TINYBIRD.SAMPLES.ORDERS_1M'
IMPORT_STRATEGY 'replace'
IMPORT_QUERY 'SELECT O_ORDERKEY, O_CUSTKEY FROM TINYBIRD.SAMPLES.ORDERS_1M'
IMPORT_SCHEDULE '@once'
            """)
...
IMPORT_SCHEDULE 0 0 1 1 *
        ...
        needed_lines = {
            ...
            "IMPORT_EXTERNAL_DATA_SOURCE 'TINYBIRD.SAMPLES.ORDERS_1M'": False,
...
class TestCLIBranch(TestCLI):
    ...
    async def test_create_branch_disabled_branching(self) -> None:
        ...
        output = await self._tb(["branch", "create", workspace_branch_to_create], assert_exit_code=1)

        self._assert_feedback_output([
            "** This command depends on the experimental branching feature currently in beta."
        ], output)
    ...
    async def test_create_branch_nok_from_other_branch(self) -> None:
        ...
        output = await self._tb(["branch", "create", workspace_another_branch_to_create], assert_exit_code=1)
...
SQL >
    ...
    async def test_regression_tests_branch_vs_main_all_pipes_coverage(self) -> None:
        ...
        for args in [["branch", "regression-tests", "coverage", "--assert-time-increase-percentage", "-1", "--wait"], ["branch", "regression-tests", "--wait"]]:
    ...
    async def test_regression_tests_branch_vs_main_all_pipes_last(self) -> None:
        ...
        # run regression-tests from branch to main
        output = await self._tb(["branch", "regression-tests", "last", "--assert-time-increase-percentage", "-1", "--wait"])
    ...
    async def test_regression_tests_branch_vs_main_pipe_argument_coverage(self) -> None:
        ...
        # run regression-tests from branch to main
        output = await self._tb(["branch", "regression-tests", "coverage", "test_pipe_a_good", "--assert-time-increase-percentage", "-1", "--wait"])
    ...
    async def test_regression_tests_branch_vs_main_pipe_argument_manual(self) -> None:
        ...
        # run regression-tests from branch to main
        output = await self._tb(["branch", "regression-tests", "manual", "test_pipe_a_good", "--param", "1", "--assert-time-increase-percentage", "-1", "--wait"])
...
SQL >
    ...
    LIMIT 1000
        ...
        output = await self._tb(["branch", "create", "--last-partition"], input=f"{workspace_branch_to_create}\n")
