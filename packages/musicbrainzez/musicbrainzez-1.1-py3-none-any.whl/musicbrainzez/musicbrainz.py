...
from musicbrainzez import mbsubmission
from musicbrainzez import util
from musicbrainzez import compat
from musicbrainzez._version import _version
...
oauth_auth_path = "/oauth2/authorize"
...
def set_useragent(app, version, contact=None):
    ...
    if contact is not None:
        _useragent = "%s/%s python-musicbrainzez/%s ( %s )" % (app, version, _version, contact)
    else:
        _useragent = "%s/%s python-musicbrainzez/%s" % (app, version, _version)
    _client = "%s-%s" % (app, version)
    _log.debug("set user-agent to %s" % _useragent)


def set_hostname(new_hostname, use_https=True):
...
def oauth_new_handler(client_id, scope=[]):
    return OAuth2Session(client_id,
                         redirect_uri=oauth_redirect_uri,
                         scope=scope)

def oauth_get_authorization_url(handler):
    ...
    url = compat.urlunparse((
            ...
            '',
            ''))
    return handler.create_authorization_url(url)

def oauth_get_authorization(handler, client_secret, code):
    ...
    try:
        url = compat.urlunparse((
                ...
                '',
                ''))
        authorization = handler.fetch_token(url,
...
def oauth_refresh_token(handler, client_secret, refresh_token):
    ...
    try:
        url = compat.urlunparse((
                ...
                '',
                ''))
        authorization = handler.refresh_token(url,
...
def oauth_revoke_token(handler, client_id, client_secret, token):
    ...
    try:
        url = compat.urlunparse((
                ...
                '',
                ''))
        token = handler.revoke_token(url,
                                     token=token,
                                     client_id=client_id,
                                     client_secret=client_secret,
...
def _mb_request(path, method='GET', auth_required=AUTH_NO,
    ...
    if auth_required == AUTH_YES:
        ...
        if not (user or client_id):
            raise UsageError("authorization required; "
                             "provide a user/pass or an oauth token")
        add_auth = True
    ...
    if body:
        req.add_header('Content-Type', 'application/{}; charset=UTF-8'.format(api))
...
def _do_mb_query(entity, id, includes=[], params={}, api="json",
                 user=None, password=None, client_id=None, access_token=None):
	"""Make a single GET call to the MusicBrainz API. `entity` is a
	...
	path = '%s/%s' % (entity, id)
	return _mb_request(path, 'GET', auth_required, args=args, api=api,
                           user=user, password=password, client_id=client_id, access_token=access_token)

def _do_mb_search(entity, query='', fields={},
                  limit=None, offset=None, strict=False, api="json"):
	...
	return _do_mb_query(entity, '', [], params, api=api)

def _do_mb_delete(path, user=None, password=None, client_id=None, access_token=None):
	"""Send a DELETE request for the specified object.
	"""
	return _mb_request(path, 'DELETE', AUTH_YES, True,
	                   user=user, password=password, client_id=client_id, access_token=access_token)

def _do_mb_put(path, user=None, password=None, client_id=None, access_token=None):
	"""Send a PUT request for the specified object.
	"""
	return _mb_request(path, 'PUT', AUTH_YES, True,
	                   user=user, password=password, client_id=client_id, access_token=access_token)

def _do_mb_post(path, body, user=None, password=None, client_id=None, access_token=None):
...
@_docstring_get("area")
def get_area_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    ...
    return _do_mb_query("area", id, includes, params, api=api)

@_docstring_get("artist")
def get_artist_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    """Get the artist with the MusicBrainz `id` as a dict with an 'artist' key.

    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("artist", includes,
                                           release_status, release_type)
    return _do_mb_query("artist", id, includes, params, api=api)

@_docstring_get("instrument")
def get_instrument_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("instrument", includes,
                                           release_status, release_type)
    return _do_mb_query("instrument", id, includes, params, api=api)

@_docstring_get("label")
def get_label_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    """Get the label with the MusicBrainz `id` as a dict with a 'label' key.

    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("label", includes,
                                           release_status, release_type)
    return _do_mb_query("label", id, includes, params, api=api)

@_docstring_get("place")
def get_place_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    """Get the place with the MusicBrainz `id` as a dict with an 'place' key.

    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("place", includes,
                                           release_status, release_type)
    return _do_mb_query("place", id, includes, params, api=api)

@_docstring_get("event")
def get_event_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("event", includes,
                                           release_status, release_type)
    return _do_mb_query("event", id, includes, params, api=api)

@_docstring_get("recording")
def get_recording_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("recording", includes,
                                           release_status, release_type)
    return _do_mb_query("recording", id, includes, params, api=api)

@_docstring_get("release")
def get_release_by_id(id, includes=[], release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("release", includes,
                                           release_status, release_type)
    return _do_mb_query("release", id, includes, params, api=api)

@_docstring_get("release-group")
def get_release_group_by_id(id, includes=[],
                            release_status=[], release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("release-group", includes,
                                           release_status, release_type)
    return _do_mb_query("release-group", id, includes, params, api=api)

@_docstring_get("series")
def get_series_by_id(id, includes=[], api="json"):
    """Get the series with the MusicBrainz `id` as a dict with a 'series' key.

    *Available includes*: {includes}"""
    return _do_mb_query("series", id, includes, api=api)

@_docstring_get("work")
def get_work_by_id(id, includes=[], api="json"):
    """Get the work with the MusicBrainz `id` as a dict with a 'work' key.

    *Available includes*: {includes}"""
    return _do_mb_query("work", id, includes, api=api)

@_docstring_get("url")
def get_url_by_id(id, includes=[], api="json"):
    """Get the url with the MusicBrainz `id` as a dict with a 'url' key.

    *Available includes*: {includes}"""
    return _do_mb_query("url", id, includes, api=api)
...
def get_releases_by_discid(id, includes=[], toc=None, cdstubs=True, media_format=None, api="json"):
    ...
    *Available includes*: {includes}"""
    ...
    return _do_mb_query("discid", id, includes, params, api=api)


@_docstring_get("recording")
def get_recordings_by_isrc(isrc, includes=[], release_status=[],
                           release_type=[], api="json"):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("isrc", includes,
                                           release_status, release_type)
    return _do_mb_query("isrc", isrc, includes, params, api=api)

@_docstring_get("work")
def get_works_by_iswc(iswc, includes=[], api="json"):
    """Search for works with an :musicbrainz:`ISWC`.
    The result is a dict with a`work-list`.

    *Available includes*: {includes}"""
    return _do_mb_query("iswc", iswc, includes, api=api)


def _browse_impl(entity, includes, limit, offset, params,
                 release_status=[], release_type=[], api="json"):
    ...
    p.update(filterp)
    return _do_mb_query(entity, "", includes, p, api=api)
...
def browse_artists(recording=None, release=None, release_group=None,
                   work=None, includes=[], limit=None, offset=None, api="json"):
    ...
    *Available includes*: {includes}"""
    params = {"recording": recording,
              ...
              "work": work}
    return _browse_impl("artist", includes, limit, offset, params, api=api)

@_docstring_browse("event")
def browse_events(area=None, artist=None, place=None,
                  includes=[], limit=None, offset=None, api="json"):
    ...
    *Available includes*: {includes}"""
    params = {"area": area,
              "artist": artist,
              "place": place}
    return _browse_impl("event", includes, limit, offset, params, api=api)

@_docstring_browse("label")
def browse_labels(release=None, includes=[], limit=None, offset=None, api="json"):
    """Get all labels linked to a relase. You need to give a MusicBrainz ID.

    *Available includes*: {includes}"""
    params = {"release": release}
    return _browse_impl("label", includes, limit, offset, params, api=api)

@_docstring_browse("place")
def browse_places(area=None, includes=[], limit=None, offset=None, api="json"):
    """Get all places linked to an area. You need to give a MusicBrainz ID.

    *Available includes*: {includes}"""
    params = {"area": area}
    return _browse_impl("place", includes, limit, offset, params, api=api)

@_docstring_browse("recording")
def browse_recordings(artist=None, release=None, includes=[],
                      limit=None, offset=None, api="json"):
    ...
    *Available includes*: {includes}"""
    params = {"artist": artist,
              "release": release}
    return _browse_impl("recording", includes, limit, offset, params, api=api)
...
def browse_urls(resource=None, includes=[], limit=None, offset=None, api="json"):
    """Get urls by actual URL string.
    You need to give a URL string as 'resource'

    *Available includes*: {includes}"""
    params = {"resource": resource}
    return _browse_impl("url", includes, limit, offset, params, api=api)

@_docstring_browse("work")
def browse_works(artist=None, includes=[], limit=None, offset=None, api="json"):
    """Get all works linked to an artist

    *Available includes*: {includes}"""
    params = {"artist": artist}
    return _browse_impl("work", includes, limit, offset, params, api=api)

# Collections
def get_collections(api="json"):
    ...
    return _do_mb_query("collection", '', api=api)

def _do_collection_query(collection, collection_type, limit, offset, api="json"):
    params = {}
    if limit: params["limit"] = limit
    if offset: params["offset"] = offset
    return _do_mb_query("collection", "%s/%s" % (collection, collection_type), [], params, api=api)

def get_artists_in_collection(collection, limit=None, offset=None, api="json"):
    ...
    """
    return _do_collection_query(collection, "artists", limit, offset, api=api)

def get_releases_in_collection(collection, limit=None, offset=None, api="json"):
    """List the releases in a collection.
    Returns a dict with a 'collection' key, which again has a 'release-list'.

    See `Browsing`_ for how to use `limit` and `offset`.
    """
    return _do_collection_query(collection, "releases", limit, offset, api=api)

def get_events_in_collection(collection, limit=None, offset=None, api="json"):
    """List the events in a collection.
    Returns a dict with a 'collection' key, which again has a 'event-list'.

    See `Browsing`_ for how to use `limit` and `offset`.
    """
    return _do_collection_query(collection, "events", limit, offset, api=api)

def get_places_in_collection(collection, limit=None, offset=None, api="json"):
    """List the places in a collection.
    Returns a dict with a 'collection' key, which again has a 'place-list'.

    See `Browsing`_ for how to use `limit` and `offset`.
    """
    return _do_collection_query(collection, "places", limit, offset, api=api)

def get_recordings_in_collection(collection, limit=None, offset=None, api="json"):
    """List the recordings in a collection.
    Returns a dict with a 'collection' key, which again has a 'recording-list'.

    See `Browsing`_ for how to use `limit` and `offset`.
    """
    return _do_collection_query(collection, "recordings", limit, offset, api=api)

def get_works_in_collection(collection, limit=None, offset=None, api="json"):
    """List the works in a collection.
    Returns a dict with a 'collection' key, which again has a 'work-list'.

    See `Browsing`_ for how to use `limit` and `offset`.
    """
    return _do_collection_query(collection, "works", limit, offset, api=api)
...
def submit_barcodes(release_barcode,
    ...
    query = mbsubmission.make_barcode_request(release_barcode)
...
def submit_isrcs(recording_isrcs,
    ...
    query = mbsubmission.make_isrc_request(rec2isrcs)
    return _do_mb_post("recording", query,
                       user=user, password=password)

def submit_tags(user=None, password=None, client_id=None, access_token=None, **kwargs):
    """Submit user tags.
    ...
    query = mbsubmission.make_tag_request(**kwargs)
    return _do_mb_post("tag", query,
                       user=user, password=password, client_id=client_id, access_token=access_token)

def submit_ratings(user=None, password=None, client_id=None, access_token=None, **kwargs):
    """Submit user ratings.
    ...
    """
    query = mbsubmission.make_rating_request(**kwargs)
