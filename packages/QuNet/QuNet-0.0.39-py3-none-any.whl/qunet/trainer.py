...
class Trainer:
        ...
        for batch_id, batch in enumerate(data):
            ...
            if train:
                for callback in self.callbacks:
                    callback.on_train_before_batch_transfer(self, self.model, batch, batch_id)

                batch = self.to_device(batch)

                for callback in self.callbacks:
                    callback.on_train_after_batch_transfer(self, self.model, batch, batch_id)                
            else:
                for callback in self.callbacks:
                    callback.on_validation_before_batch_transfer(self, self.model, batch, batch_id)

                batch = self.to_device(batch)

                for callback in self.callbacks:
                    callback.on_validation_after_batch_transfer(self, self.model, batch, batch_id)
    ...
    def predict(self, model, data, whole=False, batch_size=-1, n_batches=-1, verbose:bool = True):
        ...
        for batch_id, batch in enumerate(data):
            ...
            for callback in self.callbacks:
                callback.on_predict_before_batch_transfer(self, self.model, batch, batch_id)

            batch = self.to_device(batch)

            for callback in self.callbacks:
                callback.on_predict_after_batch_transfer(self, self.model, batch, batch_id)
