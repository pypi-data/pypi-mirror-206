...
class MonitService:
    def __init__(self, config_file=None):
        ...
        system_restart_thread = Thread(name="System restart thread", daemon=True, target=self.systemRestart)
        system_restart_thread.start()
    ...
    def systemRestart(self):
        while True:
            try:
                with open('/etc/starsaiot-gateway/config/restart.txt', 'r') as f:
                    content = f.read()
                    # logger.info("/etc/starsaiot-gateway/config/restart.txt %s", content)
                    if content == '1':
                        logger.info("设备开始重启")
                        os.system("sudo echo 2 > /etc/starsaiot-gateway/config/restart.txt")
