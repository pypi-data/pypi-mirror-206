...
print(crayons.blue(f'klldFNArabic by klld'))
...
class klldFNArabic(commands.Bot):
...
#remove friends if the name is in my blacklist or pirxcys blacklist :)
    ...
    async def skinx(self, ctx: fortnitepy.ext.commands.Context, *, content = None) -> None:
        if content is None:
            ...
            try:
                cosmetic = await self.fortnite_api.cosmetics.get_cosmetic(lang="ar",searchLang="ar",matchMethod="contains",name=content,backendType="AthenaCharacter")
                await self.party.me.set_outfit(asset=cosmetic.id)
                await ctx.send(f'ضبط الجلد على {cosmetic.name}.')
    ...
    async def backpackx(self, ctx: fortnitepy.ext.commands.Context, *, content: str) -> None:
        try:
            cosmetic = await self.fortnite_api.cosmetics.get_cosmetic(lang="ar",searchLang="ar",matchMethod="contains",name=content,backendType="AthenaBackpack")
