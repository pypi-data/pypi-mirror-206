...
class InspectionWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.path_gm = self.parent.path_gm + f"/tile{self.parent.n_page}.jpg"

        self.view = self.setImg(self.path_target, 0)
    ...
    def setImg(self, path, index, contrast=1.3, brightness=15.0):
        ...
        pixItem.setPixmap(pixItem.pixmap().scaled(800, 800, Qt.KeepAspectRatio))
    ...
    def image_update(self):
        ...
        if is_anomaly:
            if str(self.parent.n_page) not in self.parent.img_dic:
                self.parent.img_dic[
                    str(self.parent.n_page)
                ] = f"{self.parent.temp_dir_path}/tile_img_{self.parent.mode}_{self.parent.n_page}.jpg"
        else:
    ...
    def split_img(self, _img, n_page):
        col, row = divmod(n_page, self.parent.nsplit)
        ...
        y1 = grid_size_h * row
        y2 = grid_size_h * (row + 1)
        x1 = grid_size_w * col
        x2 = grid_size_w * (col + 1)
        ...
        log.info(
            f"split_img(): {self.parent.n_page} / {self.parent.tot_page}: [{y1}:{y2}, {x1}:{x2}] "
        )
        ...
        return f"{self.parent.temp_dir_path}/tile_img_{self.parent.mode}_{self.parent.n_page}.jpg"
