...
if sys.platform in ['win32', 'cygwin', 'windows']:
    ...
    code = 1
    try:
        code = subprocess.call(['pip', 'install', torch_version, torchvision_version, '-f',
                                'https://download.pytorch.org/whl/torch_stable.html'])
        if code != 0:
            raise Exception('PyTorch and Torchvision installation failed !')
    except:
        try:
            code = subprocess.call(['pip3', 'install', torch_version, torchvision_version, '-f',
                                    'https://download.pytorch.org/whl/torch_stable.html'])
            if code != 0:
                raise Exception('PyTorch and Torchvision installation failed !')
        except:
            print(
                'Failed to install PyTorch, please install PyTorch and Torchvision manually following the simple instructions at: https://pytorch.org/get-started/')
    if code == 0:
        print(
            'Successfully installed PyTorch and torchvision! (If you need the GPU version, please install it manually, checkout the mindsdb docs and the pytorch docs if you need help)')
...
setup(
    ...
    name='semisup-segmentation',  # Required
    ...
    url='https://github.com/adines/SemiSeg',  # Optional
    ...
    keywords='segmentation, deep learning, semi-supervised',  # Optional
