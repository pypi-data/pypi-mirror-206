...
class CertoraBuildGenerator:
    ...
    def collect_for_file(self,
        ...
        for lib in self.__find_all_libraries_for_file(build_arg_contract_file, file_abs_path):
            self.library_addresses.append(self.find_contract_address_str(file_abs_path, lib, contracts_with_chosen_addresses))
    ...
    def __find_all_libraries_for_file(self, build_arg_contract_file: str, file_abs_path: str) -> List[str]:
        to_ret = []
        for node in self.asts[build_arg_contract_file][file_abs_path].values():
            if node['nodeType'] == 'ContractDefinition' and node['contractKind'] == 'library':
                to_ret.append(node['canonicalName'])
        return to_ret
