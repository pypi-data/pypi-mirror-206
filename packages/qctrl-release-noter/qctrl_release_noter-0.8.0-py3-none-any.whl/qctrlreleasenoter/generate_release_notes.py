...
# Commits from these authors are ignored.
IGNORED_AUTHORS = ["Q-CTRL Robot"]


class _PrType(Enum):
    FEATURES = ("feat",)
    FIXES = ("fix",)
    IMPROVEMENTS = ("perf", "revert", "build")
...
class NoteBody:
    ...
    breakings: list[str] = field(default_factory=list)
    ...
    def add_item(self, item: _CommitInfo) -> None:
        ...
        if item.breaking_change_message is not None:
            self.breakings.append(item.breaking_change_message)
...
def generate_note(branch, ignore, github, phase, repo):
    ...
    latest_tag = max(tags, key=lambda tag: tag.commit.committed_date, default=None)
    if latest_tag is not None:
        latest_tag_name = latest_tag.name
        commits = repo.iter_commits(f"{latest_tag_name}..{branch}", reverse=True)
    else:
        # Meaning this is the first release of the target repo.
        latest_tag_name = None
        commits = repo.iter_commits(reverse=True)

    note_body = generate_note_from_commits(commits, ignore)
    if note_body.is_empty():
        print_warning("No correctly formatted changes detected.")
        return
    ...
    query_parameters = {
        "tag": get_new_version_tag(
            ...
            has_breaking_changes=len(note_body.breakings) > 0,
            has_features=len(note_body.features) > 0,
        )
    }

    click.echo(f"Suggested version: {query_parameters['tag']}")
    if project_name is not None:
        query_parameters["title"] = f"{project_name} {query_parameters['tag']}"
        click.echo(f"Suggested title: {query_parameters['title']}")
    click.echo("")

    # Create summary of changes.
    change_list = []
    for items, title in [
        (note_body.unknowns, "UNKNOWN CHANGES"),
        (note_body.breakings, "Breaking changes"),
        (note_body.features, "New features"),
        (note_body.fixes, "Bug fixes"),
        (note_body.improvements, "Improvements"),
    ]:
        if not items:
            continue
        change_list += [title]
    ...
    click.echo(query_parameters["body"])
    click.echo("")
    ...
    if github:
        ...
        if project_name is None:
            print_warning("Project name not found, cannot redirect to GitHub repo.")
            return
        local_path = repo.working_tree_dir
        assert local_path is not None
        repo_name = str(local_path).rsplit("/", maxsplit=1)[-1]
        webbrowser.open(
            f"https://github.com/qctrl/{repo_name}/releases/new?{parsed_parameters}"
        )
...
def generate_note_from_commits(commits: Iterator[Commit], ignore: bool) -> NoteBody:
    """
    Iterate through commits, collecting information about breaking changes,
    new features, and bug fixes.
    """

    note_body = NoteBody()
    for commit in commits:
        # Ignore commits from specific authors.
        if ignore and commit.author.name in IGNORED_AUTHORS:
            continue
...
def _pre_release(
    ...
    return f"{_stable_release(current_version, has_breaking_changes, has_features)}-{phase.value}"


def _stable_release(
    current_version: Version, has_breaking_changes: bool, has_features: bool
) -> str:
    # pre -> stable
    if current_version.is_prerelease:
