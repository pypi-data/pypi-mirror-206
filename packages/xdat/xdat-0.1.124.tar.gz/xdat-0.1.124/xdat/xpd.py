...
def x_match(self, values, exclude=False, fnmatch=True):
    """
    ...
    for keep_pattern in values:
        if fnmatch and isinstance(keep_pattern, str):
...
def x_filter_by(self, values, col_name, exclude=False, dropna=True, ignore_index=None, sanity_checks=None, fnmatch=True):
    """
    Filter DataFrame by column values
    ...
    idxs = x_match(self[col_name], values, exclude=exclude, fnmatch=fnmatch)
...
def x_drop_by_counts(self, count_on, keep_count=None, min_count=None, max_count=None):
    ...
    self = x_filter_by(self, sa_counts.index.values, count_on, fnmatch=False)
    return self
