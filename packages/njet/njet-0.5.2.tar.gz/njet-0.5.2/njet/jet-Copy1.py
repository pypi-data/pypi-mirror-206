...
class jet:
    ...
    def __neg__(self):
        result_array = [-self.array(n) for n in range(self.order + 1)]
        result = self.__class__(*result_array, n=self.order, graph=[(1, '-'), self.graph])
        return result
    ...
    def __mul__(self, other):
        ...
        result_array = [glr[n] for n in range(max_order + 1)]
        result = self.__class__(*result_array, n=max_order, graph=[(2, '*'), self.graph, other.graph])
    ...
    def conjugate(self):
        ...
        result_array = [self.array(n).conjugate() for n in range(self.order + 1)]
        result = self.__class__(*result_array, n=self.order, graph=[(1, 'cg'), self.graph])
        return result
