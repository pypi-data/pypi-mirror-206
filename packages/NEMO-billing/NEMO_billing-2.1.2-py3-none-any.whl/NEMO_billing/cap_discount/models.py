...
class CAPDiscountConfiguration(BaseModel):
    ...
    def get_or_create_cap_discount(self, account: Account, username: str, start: date):
        try:
            ...
            self.initialize_cap_discount(new_cap_discount)
            if self.start_on_first_charge:
                assert start is not None
                new_cap_discount.start = start
            new_cap_discount.save()
            # Now that we saved our new cap discount, initialize amounts
    ...
    def initialize_cap_discount(self, cap_discount):
        cap_discount.amount = self.amount
        cap_discount.discount = self.discount
...
class CAPDiscount(BaseModel):
    ...
    def reset(self):
        ...
        self.configuration.initialize_cap_discount(self)
