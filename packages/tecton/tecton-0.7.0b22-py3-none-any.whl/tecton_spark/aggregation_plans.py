    ...
    * intermediate_column_prefixes
    ...
    Attributes:
        ...
        partial_aggregation_preprocessor: Creates intermediate columns to be used by the partial_aggregation_transform.
            ...
            a UDAF, as is the case for the approx count distinct aggregation.
...
def ApproxCountDistinctFullAgg(indices: str, registers: str, precision: int) -> Column:
    ...
    udf_name = f"tecton_approx_count_distinct_full_precision_{precision}"
...
def ApproxCountDistinctPartialAgg(index: str, register: str) -> List[Column]:
    ...
    return [columns.indices.cast(ArrayType(LongType())), columns.registers.cast(ArrayType(LongType()))]
...
_approx_count_distinct_materialized_column_prefixes = [
...
_approx_count_distinct_intermediate_column_prefixes = ["index", "register"]
...
AGGREGATION_PLANS = {
    ...
    # TODO(TEC-14015): Support continuous mode for approx_count_distinct.
    afpb.AGGREGATION_FUNCTION_APPROX_COUNT_DISTINCT: lambda time_key, params, _: AggregationPlan(
        ...
        materialized_column_prefixes=_approx_count_distinct_materialized_column_prefixes,
