...
class CallibriSensor(MemsSensor, RespirationSensor, SignalSensor, EnvelopeSensor, ElectrodeSensor):
    def __init__(self, ptr):
        ...
        if self.is_supported_feature(SensorFeature.FeatureMEMS):
    ...
    def ext_sw_input(self, ext_sw_inp: SensorExternalSwitchInput):
        status = OpStatus()
        _neuro_lib.writeDataOffsetSensor(self.sensor_ptr, ext_sw_inp.value, byref(status))
        raise_exception_if(status)
    ...
    def adc_input(self, adc_inp: SensorADCInput):
        status = OpStatus()
        _neuro_lib.writeExternalSwitchSensor(self.sensor_ptr, adc_inp.value, byref(status))
        raise_exception_if(status)

    @MemsSensor.acc_sens.setter
    def acc_sens(self, acc_sens: SensorAccelerometerSensitivity):
        status = OpStatus()
        _neuro_lib.writeADCInputSensor(self.sensor_ptr, acc_sens.value, byref(status))
        raise_exception_if(status)
    ...
    @property
    def stimulator_ma_state_callibri(self) -> CallibriStimulatorMAState:
        status = OpStatus()
    ...
    @property
    def stimulator_param_callibri(self) -> CallibriStimulationParams:
        status = OpStatus()
    ...
    @stimulator_param_callibri.setter
    def stimulator_param_callibri(self, params: CallibriStimulationParams):
        status = OpStatus()
    ...
    @property
    def motion_assistant_param_callibri(self) -> CallibriMotionAssistantParams:
        status = OpStatus()
    ...
    @motion_assistant_param_callibri.setter
    def motion_assistant_param_callibri(self, params: CallibriMotionAssistantParams):
        status = OpStatus()
    ...
    @property
    def motion_counter_param_callibri(self) -> CallibriMotionCounterParam:
        status = OpStatus()
    ...
    @motion_counter_param_callibri.setter
    def motion_counter_param_callibri(self, params: CallibriMotionCounterParam):
        status = OpStatus()
    ...
    @property
    def motion_counter_callibri(self) -> int:
        status = OpStatus()
    ...
    @property
    def color_callibri(self) -> CallibriColorType:
        status = OpStatus()
    ...
    @property
    def signal_type_callibri(self) -> CallibriSignalType:
        status = OpStatus()
    ...
    @signal_type_callibri.setter
    def signal_type_callibri(self, signal_type: CallibriSignalType):
        status = OpStatus()
        _neuro_lib.setSignalSettingsCallibri(self.sensor_ptr, signal_type.value, byref(status))
        raise_exception_if(status)

    @property
    def mems_calibrate_state_callibri(self) -> bool:
        status = OpStatus()
