...
def listPackageAction(name: Optional[str], **_) -> None:
  ...
  packageList = [[p.name, p.version, timeago(p.createdAtMs or 0),
                  sizeOfFmt(p.size)] for p in auth_and_list_package(name)]
...
def initializeParser() -> argparse.ArgumentParser:
  ...
  delete_pkg_parser.add_argument('name', nargs='?')
  delete_pkg_parser.add_argument('version', nargs='?')
