...
class Command:
    ...
    def run_command(
        self, context_data, org_slug, target_id, action, metadata, payload, ttl_sec
    ):
        if org_slug is None:
        ...
        if response.status_code == 200:
            ...
            while time_elapsed <= ttl_sec:
                time.sleep(1)
                time_elapsed += 1
                command_response = requests.request(
                    "GET", command_url, headers=command_headers, verify=False
                )
