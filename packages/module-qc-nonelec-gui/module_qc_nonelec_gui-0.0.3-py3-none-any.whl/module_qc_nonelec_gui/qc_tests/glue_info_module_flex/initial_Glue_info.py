...
class InitialWindow(QWidget):
    def __init__(self, parent=None):
        ...
        #        grid_edit.addLayout(self.make_timeform_layout(),5,1)
    ...
    def make_timeform_layout(self):
        grid_adhesion = QGridLayout()
        ...
        self.edit_d.setAlignment(Qt.AlignRight)
        self.edit_d.setText(str(self.parent.result_info["adhesion"]["day"]))
        self.edit_h = QLineEdit()
        self.edit_h.setAlignment(Qt.AlignRight)
        self.edit_h.setText(str(self.parent.result_info["adhesion"]["hour"]))
        self.edit_m = QLineEdit()
        self.edit_m.setAlignment(Qt.AlignRight)
        self.edit_m.setText(str(self.parent.result_info["adhesion"]["minute"]))
        self.edit_s = QLineEdit()
        self.edit_s.setAlignment(Qt.AlignRight)
        self.edit_s.setText(str(self.parent.result_info["adhesion"]["second"]))

        grid_adhesion.addWidget(self.edit_d, 0, 0)
        grid_adhesion.addWidget(label_d, 0, 1)
        grid_adhesion.addWidget(self.edit_h, 0, 2)
        grid_adhesion.addWidget(label_h, 0, 3)
        grid_adhesion.addWidget(self.edit_m, 0, 4)
        grid_adhesion.addWidget(label_m, 0, 5)
        grid_adhesion.addWidget(self.edit_s, 0, 6)
        grid_adhesion.addWidget(label_s, 0, 7)

        return grid_adhesion
    ...
    def pass_result(self):
        ...
        try:
            result_dict = {
                ...
                #                'adhesion'   :{
                #                    'day'        :int( self.edit_d.text() ),
                #                    'hour'       :int( self.edit_h.text() ),
                #                    'minute'     :int( self.edit_m.text() ),
                #                    'second'     :int( self.edit_s.text() )
                #                }
            }
            #            if int( self.edit_d.text() )<0 or int( self.edit_h.text() ) <0 or int( self.edit_m.text() )<0 or int( self.edit_s.text() )<0:
            #                QMessageBox.warning(None, 'Warning','Adhesion time should be positive value.', QMessageBox.Ok)
            #            elif int( self.edit_h.text() )>=24 or int( self.edit_m.text() )>=60 or int( self.edit_s.text() )>=60:
            #                QMessageBox.warning(None, 'Warning','Please round up the time values .', QMessageBox.Ok)
            #            else:
            #                token=1
            token = 1
        except Exception:
