...
class OverCallersRebid(Hand):
    ...
    def _bid_after_nt_agreed(self):
        ...
        if self._strong_with_biddable_five_card_major():
            ...
            if self.second_suit in self.opponents_suits:
                ...
                if self.hcp >= 14:
                    raise_level = 1
            bid = self.next_level_bid(suit, '2517', raise_level)
        else:
            bid = Pass('2518')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _rebid_after_double(self):
        ...
        if self._weak_hand_after_opener_bids_weak_two():
            ...
            bid = self.next_level_bid(self.advancer_bid_one.denomination, '2539')
        elif self._suit_for_rebid_is_minor_after_double(suit_for_rebid):
            bid = self.next_level_bid(self.advancer_bid_one.denomination, '2540')
    ...
    def _no_long_suit_support_partner(self):
        ...
        if self.advancer_bid_one.level >= 4:
            if self.hcp + self.distribution_points >= 21:
                bid = self.next_level_bid(self.advancer_bid_one.denomination, '2544')
            else:
                bid = Pass('2545')
            ...
            if self._can_raise_advancer_after_double():
                ...
                raise_level = 0
            bid = self.next_level_bid(self.advancer_bid_one.denomination, '2548', raise_level)
    ...
    def _partner_bid_suit(self):
        ...
        if self.bid_one.is_double:
            ...
            if self.shape[0] >= 7:
                ...
                level = 1
            bid = self.next_level_bid(self.bid_one.denomination, '2579', raise_level=level)
    ...
    def _partner_shows_support(self):
        ...
        if self.hcp >= 19:
            ...
            bid = self.next_level_bid(self.bid_one.denomination, '2612')
        else:
    ...
    def _support_for_partner(self):
        ...
        if self._advancer_has_bid_clubs_and_can_support(level):
            bid = self.suit_bid(level, self.advancer_bid_one.denomination, '2598')
        else:
            bid = Pass('2599')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _balanced_overcall_nt(self):
        ...
        if (self.advancer_bid_one.denomination == self.club_suit and
            ...
            bid = self.next_level_bid(self.advancer_bid_one.denomination, '2607')
        else:
            bid = Pass('2608')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
