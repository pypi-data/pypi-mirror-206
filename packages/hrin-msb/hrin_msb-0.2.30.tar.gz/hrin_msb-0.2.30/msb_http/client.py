...
class ApiRequestParameter:
	...
	def __initalize_using(self, request: RestRequest):
	...
	def __init__(self, api_host: str = '', request: RestRequest = None, **kwargs):
		self.headers = dict()
		self.cookies = None
		...
		self.__initalize_using(request=request)
...
class ApiRequest(ApiRequestParameter):
	...
	def _format_response(self, response: ApiResponseWrapper) -> Dict:
		return response.to_json()

	def get_result(self) -> Dict:
		try:
			_response = self._execute()
		except Exception as e:
			_response = ApiResponseWrapper(Response())
		return _response.to_json()
